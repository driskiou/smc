...
void cLevel_Entry ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // type
  Set_Type(static_cast<Level_Entry_type>(
      attributes.getValueAsInteger("type", m_entry_type)));
  // name
...
void cLevel_Entry ::Activate(void) {
  ...
  if (m_entry_type == LEVEL_ENTRY_WARP) {
    ...
    // set state to linked to stop checking for on ground objects which
    // sometimes changes the position
    ...
    while (1) {
      if (m_direction == DIR_DOWN) {
        ...
        if (pLevel_Player->m_col_rect.m_y + pLevel_Player->m_col_rect.m_h <
            m_rect.m_y) {
          break;
        }
      } else if (m_direction == DIR_RIGHT) {
        ...
        if (pLevel_Player->m_col_rect.m_x + pLevel_Player->m_col_rect.m_w <
            m_rect.m_x) {
          break;
        }
      } else {
...
float cLevel_Entry ::Get_Player_Pos_X(void) const {
  if (m_entry_type == LEVEL_ENTRY_WARP) {
    ...
    // right
      ...
      return m_col_rect.m_x - pLevel_Player->m_col_pos.m_y - m_col_rect.m_w -
             pLevel_Player->m_col_rect.m_w;
    }

    // up/down
    return m_col_rect.m_x - pLevel_Player->m_col_pos.m_x +
           (m_col_rect.m_w * 0.5f) - (pLevel_Player->m_col_rect.m_w * 0.5f);
  } else if (m_entry_type == LEVEL_ENTRY_BEAM) {
    return m_col_rect.m_x + (m_col_rect.m_w * 0.5f) -
           pLevel_Player->m_col_pos.m_y -
...
float cLevel_Entry ::Get_Player_Pos_Y(void) const {
  if (m_entry_type == LEVEL_ENTRY_WARP) {
    ...
    // down
      ...
      return m_col_rect.m_y - pLevel_Player->m_col_pos.m_y - 5 -
             pLevel_Player->m_rect.m_h;
    }

    // left/right
    return m_col_rect.m_y - pLevel_Player->m_col_pos.m_y +
           (m_col_rect.m_h * 0.5f) - (pLevel_Player->m_col_rect.m_h * 0.5f);
  } else if (m_entry_type == LEVEL_ENTRY_BEAM) {
    return m_col_rect.m_y + m_col_rect.m_h - pLevel_Player->m_col_pos.m_y -
           pLevel_Player->m_col_rect.m_h;
  }
...
void cLevel_Entry ::Set_Name(const std::string &str_name) {
  ...
  m_editor_entry_name =
      pFont->Render_Text(pFont->m_font_small, m_entry_name, white);
}
...
void cLevel_Entry ::Editor_Activate(void) {
  ...
  if (m_entry_type == LEVEL_ENTRY_WARP) {
    ...
    CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
        wmgr.createWindow("TaharezLook/Combobox", "level_entry_direction"));
    Editor_Add(UTF8_("Direction"), UTF8_("The direction to come out"), combobox,
               100, 105);
    ...
    combobox->subscribeEvent(
        ...
        CEGUI::Event::Subscriber(&cLevel_Entry::Editor_Direction_Select, this));
  }

  // destination entry
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "level_entry_name"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cLevel_Entry::Editor_Name_Text_Changed, this));

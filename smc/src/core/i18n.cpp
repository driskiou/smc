...
void I18N_Init(void) {
  ...
  const char *textdomain_directory =
      bindtextdomain(CAPTION, DATA_DIR "/" GAME_TRANSLATION_DIR);
...
void I18N_Set_Language(const std::string &language) {
  ...
  bool success = SUCCEEDED(SetThreadLocale(
      MAKELCID(MAKELANGID(primary_language, sub_language), SORT_DEFAULT)));

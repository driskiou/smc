...
void cBaseBox ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  if (box_type != TYPE_SPIN_BOX && box_type != TYPE_TEXT_BOX) {
    // animation
    Set_Animation_Type(
        attributes.getValueAsString("animation", m_anim_type).c_str());
  }
  // invisible
  Set_Invisible(static_cast<Box_Invisible_Type>(
      attributes.getValueAsInteger("invisible")));
  // useable count
  Set_Useable_Count(
      attributes.getValueAsInteger("useable_count", m_start_useable_count), 1);
}
...
void cBaseBox ::Load_From_Savegame(cSave_Level_Object *save_object) {
  // useable count
  int save_useable_count =
      string_to_int(save_object->Get_Value("useable_count"));
...
cSave_Level_Object *cBaseBox ::Save_To_Savegame(void) {
  ...
  // Useable Count
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "useable_count", int_to_string(m_useable_count)));
...
void cBaseBox ::Set_Invisible(Box_Invisible_Type type) {
  ...
  // was invisible
  if (type_old == BOX_INVISIBLE_MASSIVE ||
      type_old == BOX_INVISIBLE_SEMI_MASSIVE) {
    // was semi massive
...
void cBaseBox ::Update_Collision(void) {
  ...
  // invalid direction
  if (m_move_col_dir != DIR_UP && m_move_col_dir != DIR_DOWN &&
      m_move_col_dir != DIR_RIGHT && m_move_col_dir != DIR_LEFT) {
...
void cBaseBox ::Check_Collision(ObjectDirection col_direction) {
  ...
  cObjectCollisionType *col_list = Collision_Check_Relative(
      check_x, check_y, m_col_rect.m_w - (check_x * 0.5f),
      m_col_rect.m_h - (check_y * 0.5f));
  ...
  for (cObjectCollision_List::iterator itr = col_list->objects.begin();
    ...
    // send box collision
    col_obj->m_obj->Handle_Collision_Box(
        Get_Opposite_Direction(col_obj->m_direction), &m_col_rect);
...
void cBaseBox ::Col_Enemy(cSprite *obj) {
  ...
  // only valid enemies
  if (obj->m_type == TYPE_FURBALL || obj->m_type == TYPE_TURTLE ||
      obj->m_type == TYPE_KRUSH) {
    ...
    pHud_Points->Add_Points(enemy->m_kill_points, enemy->m_pos_x,
                            enemy->m_pos_y - 5.0f, "", static_cast<Uint8>(255),
                            1);
...
void cBaseBox ::Update(void) {
  ...
  if (m_box_invisible == BOX_VISIBLE ||
      (m_box_invisible == BOX_GHOST &&
       pLevel_Player->m_maryo_type == MARYO_GHOST) ||
      m_useable_count != m_start_useable_count) {
  ...
  // editor enabled
    ...
    if (m_box_invisible) {
      ...
      pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x,
                        m_start_pos_y - pActive_Camera->m_y, m_rect.m_w,
                        m_rect.m_h, m_pos_z, &color);
    }
    // visible box
    else {
      cAnimated_Sprite::Draw(request);
    ...
    if (m_item_image) {
      // auto position
      m_item_image->Blit(
          m_start_pos_x - ((m_item_image->m_w - m_rect.m_w) / 2) -
              pActive_Camera->m_x,
          m_start_pos_y - ((m_item_image->m_h - m_rect.m_h) / 2) -
              pActive_Camera->m_y,
          m_pos_z + 0.000003f);
    }
  }
...
void cBaseBox ::Generate_Activation_Particles(void) {
  ...
  if (m_type == TYPE_SPIN_BOX || m_type == TYPE_TEXT_BOX ||
      box_type == TYPE_GOLDPIECE) {
    return;
  }
  ...
  if (m_particle_counter_active > 0.0f) {
    ...
    if (box_type == TYPE_MUSHROOM_POISON) {
      anim->Set_Image(
          pVideo->Get_Surface("animation/particles/green_ghost_light.png"));
    ...
    anim->Set_Emitter_Rect(m_pos_x + 4.0f, m_pos_y + 8.0f, m_rect.m_w - 8.0f,
                           1.0f);
...
void cBaseBox ::Handle_Collision_Enemy(cObjectCollision *collision) {
  ...
  if (enemy->m_type == TYPE_TURTLE) {
    ...
    // if not shell
    if (turtle->m_turtle_state != TURTLE_SHELL_RUN &&
        turtle->m_turtle_state != TURTLE_SHELL_STAND) {
      return;
    }
  }
  // if turtle boss
    ...
    // if not shell
    if (turtleboss->m_turtle_state != TURTLEBOSS_SHELL_RUN &&
        turtleboss->m_turtle_state != TURTLEBOSS_SHELL_STAND) {
      return;
    }
  }
  // not a valid enemy
  ...
  if (collision->m_direction == DIR_RIGHT ||
      ...
      collision->m_direction == DIR_BOTTOM) {
...
void cBaseBox ::Editor_Activate(void) {
  ...
  if (m_type != TYPE_TEXT_BOX) {
    ...
    CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
        "TaharezLook/Editbox", "editor_basebox_useable_count"));
    ...
    editbox->subscribeEvent(
        ...
        CEGUI::Event::Subscriber(&cBaseBox::Editor_Useable_Count_Text_Changed,
                                 this));
  }

  // Invisible
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_basebox_invisible"));
  Editor_Add(
      UTF8_("Invisible"),
      UTF8_("Massive is invisible until activated.\nGhost is only visible and "
            "activateable if in ghost mode.\nSemi Massive is like Massive but "
            "only touchable in the activation direction."),
      combobox, 120, 100);
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cBaseBox::Editor_Invisible_Select, this));
...
bool cBaseBox ::Editor_Useable_Count_Text_Changed(

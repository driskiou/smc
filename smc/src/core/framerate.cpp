...
cPerformance_Timer ::cPerformance_Timer(void) { Reset(); }
...
cFramerate ::~cFramerate(void) {
  ...
  for (Performance_Timer_List::iterator itr = m_perf_timer.begin();
       itr != m_perf_timer.end(); ++itr) {
    delete *itr;
  }
...
void cFramerate ::Update(void) {
  ...
  if (!Is_Float_Equal(m_force_speed_factor, 0.0f)) {
    ...
    m_elapsed_ticks =
        static_cast<Uint32>((m_force_speed_factor * 1000) / m_fps_target);
  ...
  // measure speed factor
    ...
    m_speed_factor =
        static_cast<float>(m_elapsed_ticks / (1000 / m_fps_target));
  }

...
cAnimation_Goldpiece ::cAnimation_Goldpiece(cSprite_Manager *sprite_manager,
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    obj->Set_Pos(m_pos_x + Get_Random_Float(0.0f, m_rect.m_w),
                 m_pos_y + Get_Random_Float(0.0f, m_rect.m_h));
    obj->m_pos_z = m_pos_z;
    obj->Set_Scale_X(m_scale_x, 1);
    obj->Set_Scale_Y(m_scale_y, 1);
    obj->Set_Color(m_color);
    obj->Set_Color_Combine(m_combine_color[0], m_combine_color[1],
                           m_combine_color[2], m_combine_type);
...
cAnimation_Goldpiece ::~cAnimation_Goldpiece(void) {
  // clear
  for (BlinkPointList::iterator itr = m_objects.begin(); itr != m_objects.end();
       ++itr) {
    delete *itr;
  }
...
cAnimation_Fireball ::cAnimation_Fireball(cSprite_Manager *sprite_manager,
                                          float posx, float posy,
  ...
  for (unsigned int i = 0; i < power; i++) {
    cAnimation_Fireball_Item *obj =
        new cAnimation_Fireball_Item(sprite_manager);
...
cAnimation_Fireball ::~cAnimation_Fireball(void) {
  // clear
  for (FireAnimList::iterator itr = m_objects.begin(); itr != m_objects.end();
       ++itr) {
    delete *itr;
  }
...
void cAnimation_Fireball ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  for (FireAnimList::iterator itr = m_objects.begin(); itr != m_objects.end();
    ...
    obj->m_image->Blit(obj->m_pos_x - (pActive_Camera->m_x - m_pos_x),
                       obj->m_pos_y - (pActive_Camera->m_y - m_pos_y),
                       obj->m_pos_z, request);
...
void cParticle ::Update(void) {
  ...
  m_fade_pos -= ((static_cast<float>(speedfactor_fps) * 0.001f) *
...
void cParticle ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (m_parent->m_particle_based_on_emitter_pos > 0.0f) {
    request->m_pos_x +=
        (m_parent->m_pos_x * m_parent->m_particle_based_on_emitter_pos);
    request->m_pos_y +=
        (m_parent->m_pos_y * m_parent->m_particle_based_on_emitter_pos);
  }
  ...
  if (m_parent->m_fade_alpha) {
    request->m_color.alpha =
        static_cast<Uint8>(request->m_color.alpha * m_fade_pos);
  }
...
cParticle_Emitter ::~cParticle_Emitter(void) { cParticle_Emitter::Clear(); }
...
cParticle_Emitter *cParticle_Emitter ::Copy(void) const {
  cParticle_Emitter *particle_animation =
      new cParticle_Emitter(m_sprite_manager);
  particle_animation->Set_Based_On_Camera_Pos(m_emitter_based_on_camera_pos);
  particle_animation->Set_Particle_Based_On_Emitter_Pos(
      m_particle_based_on_emitter_pos);
  ...
  particle_animation->Set_Emitter_Iteration_Interval(
      m_emitter_iteration_interval);
  ...
  particle_animation->Set_Rotation(m_start_rot_x, m_start_rot_y, m_start_rot_z,
                                   1);
  particle_animation->Set_Start_Rot_Z_Uses_Direction(
      m_start_rot_z_uses_direction);
...
void cParticle_Emitter ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // position z
  Set_Pos_Z(attributes.getValueAsFloat("pos_z", m_pos_z),
            attributes.getValueAsFloat("pos_z_rand", m_pos_z_rand));
  // emitter based on camera pos
  Set_Based_On_Camera_Pos(attributes.getValueAsBool(
      "emitter_based_on_camera_pos", m_emitter_based_on_camera_pos));
  // particle based on emitter pos
  Set_Particle_Based_On_Emitter_Pos(attributes.getValueAsFloat(
      "particle_based_on_emitter_pos", m_particle_based_on_emitter_pos));
  // emitter rect
  Set_Emitter_Rect(static_cast<float>(attributes.getValueAsInteger(
                       "pos_x", static_cast<int>(m_pos_x))),
                   static_cast<float>(attributes.getValueAsInteger(
                       "pos_y", static_cast<int>(m_pos_y))),
                   static_cast<float>(attributes.getValueAsInteger(
                       "size_x", static_cast<int>(m_start_rect.m_w))),
                   static_cast<float>(attributes.getValueAsInteger(
                       "size_y", static_cast<int>(m_start_rect.m_h))));
  // emitter time to live
  Set_Emitter_Time_to_Live(attributes.getValueAsFloat("emitter_time_to_live",
                                                      m_emitter_time_to_live));
  // emitter interval
  Set_Emitter_Iteration_Interval(attributes.getValueAsFloat(
      "emitter_interval", m_emitter_iteration_interval));
  // quota/count
  Set_Quota(attributes.getValueAsInteger("quota", m_emitter_quota));
  // time for particle to live
  Set_Time_to_Live(
      attributes.getValueAsFloat("time_to_live", m_time_to_live),
      attributes.getValueAsFloat("time_to_live_rand", m_time_to_live_rand));
  // velocity
  Set_Speed(attributes.getValueAsFloat("vel", m_vel),
            attributes.getValueAsFloat("vel_rand", m_vel_rand));
  // start rotation
  Set_Rotation(attributes.getValueAsFloat("rot_x", m_start_rot_x),
               attributes.getValueAsFloat("rot_y", m_start_rot_y),
               attributes.getValueAsFloat("rot_z", m_start_rot_z), 1);
  Set_Start_Rot_Z_Uses_Direction(attributes.getValueAsBool(
      "start_rot_z_uses_direction", m_start_rot_z_uses_direction));
  // constant rotation x
  Set_Const_Rotation_X(
      attributes.getValueAsFloat("const_rot_x", m_const_rot_x),
      attributes.getValueAsFloat("const_rot_x_rand", m_const_rot_x_rand));
  // constant rotation y
  Set_Const_Rotation_Y(
      attributes.getValueAsFloat("const_rot_y", m_const_rot_y),
      attributes.getValueAsFloat("const_rot_y_rand", m_const_rot_y_rand));
  // constant rotation z
  Set_Const_Rotation_Z(
      attributes.getValueAsFloat("const_rot_z", m_const_rot_z),
      attributes.getValueAsFloat("const_rot_z_rand", m_const_rot_z_rand));
  // angle
  Set_Direction_Range(attributes.getValueAsFloat("angle_start", m_angle_start),
                      attributes.getValueAsFloat("angle_range", m_angle_range));
  // scale
  Set_Scale(attributes.getValueAsFloat("size_scale", m_size_scale),
            attributes.getValueAsFloat("size_scale_rand", m_size_scale_rand));
  // horizontal gravity
  Set_Horizontal_Gravity(
      attributes.getValueAsFloat("gravity_x", m_gravity_x),
      attributes.getValueAsFloat("gravity_x_rand", m_gravity_x_rand));
  // vertical gravity
  Set_Vertical_Gravity(
      attributes.getValueAsFloat("gravity_y", m_gravity_y),
      attributes.getValueAsFloat("gravity_y_rand", m_gravity_y_rand));
  // clip rect
  Set_Clip_Rect(static_cast<float>(attributes.getValueAsInteger(
                    "clip_x", static_cast<int>(m_clip_rect.m_x))),
                static_cast<float>(attributes.getValueAsInteger(
                    "clip_y", static_cast<int>(m_clip_rect.m_y))),
                static_cast<float>(attributes.getValueAsInteger(
                    "clip_w", static_cast<int>(m_clip_rect.m_w))),
                static_cast<float>(attributes.getValueAsInteger(
                    "clip_h", static_cast<int>(m_clip_rect.m_h))));
  // clip mode
  Set_Clip_Mode(static_cast<ParticleClipMode>(
      attributes.getValueAsInteger("clip_mode", m_clip_mode)));
}
...
void cParticle_Emitter ::Save_To_XML(CEGUI::XMLSerializer &stream) {
  ...
  // emitter based on camera pos
  Write_Property(stream, "emitter_based_on_camera_pos",
                 m_emitter_based_on_camera_pos);
  // particle based on emitter pos
  Write_Property(stream, "particle_based_on_emitter_pos",
                 m_particle_based_on_emitter_pos);
  // emitter rect
  ...
  Write_Property(stream, "start_rot_z_uses_direction",
                 m_start_rot_z_uses_direction);
  // constant rotation x
...
void cParticle_Emitter ::Pre_Update(void) {
  if (!m_image || m_emitter_quota == 0 ||
      Is_Float_Equal(m_time_to_live, 0.0f)) {
    return;
  }
...
void cParticle_Emitter ::Update(void) {
  ...
  m_emitter_living_time += pFramerate->m_speed_factor *
                           (static_cast<float>(speedfactor_fps) * 0.001f);
...
void cParticle_Emitter ::Update_Particles(void) {
  // update objects
  for (ParticleList::iterator itr = m_objects.begin();
       itr != m_objects.end();) {
    // get object pointer
    cParticle *obj = (*itr);
  ...
  if (m_emitter_living_time < m_emitter_time_to_live ||
      Is_Float_Equal(m_emitter_time_to_live, -1.0f)) {
    // emit
    ...
    m_emit_counter += pFramerate->m_speed_factor *
                      (static_cast<float>(speedfactor_fps) * 0.001f);
  }
...
void cParticle_Emitter ::Update_Position(void) {
  if (m_emitter_based_on_camera_pos && !editor_enabled) {
    Set_Pos(m_start_pos_x + pActive_Camera->m_x,
            m_start_pos_y + (pActive_Camera->m_y + game_res_h));
  }
...
void cParticle_Emitter ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (editor_enabled) {
    if (!m_spawned) {
      // draw emitter rect
      GL_rect color_rect = GL_rect(m_start_pos_x - pActive_Camera->m_x,
                                   m_start_pos_y - pActive_Camera->m_y,
      ...
      if (pMouseCursor->m_active_object == this) {
        ...
        if (m_clip_rect.m_w > 0.0f && m_clip_rect.m_h > 0.0f) {
          color_rect =
              GL_rect(m_start_pos_x + m_clip_rect.m_x - pActive_Camera->m_x,
                      m_start_pos_y + m_clip_rect.m_y - pActive_Camera->m_y,
                      m_clip_rect.m_w, m_clip_rect.m_h);

          rect_request = new cRect_Request();
          pVideo->Draw_Rect(&color_rect, m_editor_pos_z, &lightgrey,
                            rect_request);
          rect_request->m_filled = 0;
...
void cParticle_Emitter ::Keep_Particles_In_Rect(
    const GL_rect &clip_rect, ParticleClipMode mode /* = PCM_MOVE */) {
  // temporary obj rect
  GL_rect obj_rect;

  // find particles that are not visible and move them to the opposite screen
  // side
  for (ParticleList::iterator itr = m_objects.begin(); itr != m_objects.end();
       ++itr) {
    ...
    if (obj->m_scale_x != 1.0f) {
      obj_rect.m_x =
          obj->m_pos_x - ((obj->m_image->m_w * 0.5f) * (obj->m_scale_x - 1.0f));
    ...
    if (obj->m_scale_y != 1.0f) {
      obj_rect.m_y =
          obj->m_pos_y - ((obj->m_image->m_h * 0.5f) * (obj->m_scale_y - 1.0f));
...
void cParticle_Emitter ::Set_Emitter_Rect(float x, float y, float w /* = 0 */,
                                          float h /* = 0 */) {
  // hack: don't set x/y to 0 or the next Set_Pos call will overwrite the start
  // position
...
void cParticle_Emitter ::Set_Const_Rotation_X(float rot,
                                              float rot_random /* = 0 */) {
  m_const_rot_x = rot;
  m_const_rot_x_rand = rot_random;
}

void cParticle_Emitter ::Set_Const_Rotation_Y(float rot,
                                              float rot_random /* = 0 */) {
  m_const_rot_y = rot;
  m_const_rot_y_rand = rot_random;
}

void cParticle_Emitter ::Set_Const_Rotation_Z(float rot,
                                              float rot_random /* = 0 */) {
  m_const_rot_z = rot;
  m_const_rot_z_rand = rot_random;
}

void cParticle_Emitter ::Set_Direction_Range(float start,
                                             float range /* = 0 */) {
  m_angle_start = start;
  m_angle_range = range;
}
...
void cParticle_Emitter ::Set_Horizontal_Gravity(float start,
                                                float random /* = 0 */) {
  m_gravity_x = start;
  m_gravity_x_rand = random;
}

void cParticle_Emitter ::Set_Vertical_Gravity(float start,
                                              float random /* = 0 */) {
  m_gravity_y = start;
  m_gravity_y_rand = random;
}

void cParticle_Emitter ::Set_Color(
    const Color &col,
    const Color &col_rand /* = Color( static_cast<Uint8>(0) ) */) {
  m_color = col;
  m_color_rand = col_rand;

#ifdef _DEBUG
  if (m_color.red + col_rand.red > 255) {
    printf("cParticle_Emitter::Set_Color random color red (%d) is too high for "
           "%s\n",
...
void cParticle_Emitter ::Set_Fading_Size(bool enable) { m_fade_size = enable; }
...
void cParticle_Emitter ::Set_Blending(BlendingMode mode) { m_blending = mode; }
...
void cParticle_Emitter ::Editor_Activate(void) {
  ...
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_pos_z_base"));
  Editor_Add(UTF8_("Position z"),
             UTF8_("Initial depth position. Use values from 0.00011 to 0.12."),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Pos_Z_Base_Text_Changed, this));

  // position z rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_pos_z_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Pos_Z_Rand_Text_Changed, this));

  // image filename
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_image_filename"));
  Editor_Add(UTF8_("Filename"), UTF8_("Image filename"), editbox, 360);

  editbox->setText(m_image_filename.c_str());
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cParticle_Emitter::Editor_Filename_Text_Changed,
                               this));

  // emitter position based on camera pos
  CEGUI::ToggleButton *checkbox = static_cast<CEGUI::ToggleButton *>(
      wmgr.createWindow("emitter_based_on_camera_pos"));
  Editor_Add(UTF8_("Based on camera pos."),
             UTF8_("The emitter position is based on the camera position"),
             checkbox, 50);
  ...
  checkbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Emitter_Based_On_Camera_Pos_Changed,
          this));

  // particle position based on emitter pos
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "emitter_particle_based_on_emitter_pos"));
  Editor_Add(UTF8_("Particles Based on Emitter pos."),
             UTF8_("The particle position is based on the emitter position"),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Particle_Based_On_Emitter_Pos_Text_Changed,
          this));

  // emitter width
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_width"));
  Editor_Add(UTF8_("Emitter width"),
             UTF8_("Emitter width in which the particles spawn"), editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Emitter_Width_Text_Changed, this));

  // emitter height
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_height"));
  Editor_Add(UTF8_("Height"),
             UTF8_("Emitter height in which the particles spawn"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Emitter_Height_Text_Changed, this));

  // emitter time to live
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_time_to_live"));
  Editor_Add(UTF8_("Emitter TTL"),
             UTF8_("Emitter time to live. Set -1 for infinite."), editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Emitter_Time_To_Live_Text_Changed, this));

  // emitter interval
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_interval"));
  Editor_Add(UTF8_("Emitter interval"),
             UTF8_("Time between spawning particles. Amount is the quota."),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Emitter_Interval_Text_Changed, this));

  // quota
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_quota"));
  Editor_Add(UTF8_("Quota"),
             UTF8_("The amount of particles to spawn for an interval"), editbox,
             50, 28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cParticle_Emitter::Editor_Quota_Text_Changed,
                               this));

  // time to live
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_ttl_base"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cParticle_Emitter::Editor_TTL_Base_Text_Changed,
                               this));

  // time to live rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_ttl_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cParticle_Emitter::Editor_TTL_Rand_Text_Changed,
                               this));

  // velocity
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_vel_base"));
  Editor_Add(UTF8_("Velocity"), UTF8_("Initial particle velocity or speed"),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Velocity_Base_Text_Changed, this));

  // velocity rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_vel_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Velocity_Rand_Text_Changed, this));

  // start rotation x
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_rotation_x_base" )); Editor_Add(
  UTF8_("Rotation x"), UTF8_("Initial rotation x"), editbox, 150 );

  editbox->setValidationString( "[+]?[0-9]*\\.?[0-9]*" );
  editbox->setText( float_to_string( start_rotx, 6, 0 ) );
  editbox->subscribeEvent( CEGUI::Editbox::EventTextChanged,
  CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Rotation_X_Base_Text_Changed, this ) );
  */
  // start rotation y
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_rotation_y_base" )); Editor_Add(
  UTF8_("Rotation y"), UTF8_("Initial rotation y"), editbox, 150 );

  editbox->setValidationString( "[+]?[0-9]*\\.?[0-9]*" );
  editbox->setText( float_to_string( start_roty, 6, 0 ) );
  editbox->subscribeEvent( Editbox::EventTextChanged, CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Rotation_Y_Base_Text_Changed, this ) );
  */

  // start rotation z
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_rotation_z_base"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Rotation_Z_Base_Text_Changed, this));

  // start direction is added to the z rotation
  checkbox = static_cast<CEGUI::ToggleButton *>(
      wmgr.createWindow("emitter_start_rot_z_uses_direction"));
  Editor_Add(UTF8_("Add direction"),
             UTF8_("Start direction is added to the z rotation"), checkbox, 50,
             28, 0);
  ...
  checkbox->subscribeEvent(
      CEGUI::ToggleButton::EventSelectStateChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Start_Rot_Z_Uses_Direction_Changed, this));

  // constant rotation x
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_const_rotation_x_base" )); Editor_Add(
  UTF8_("Const. rotation x"), UTF8_("Initial constant rotation x"), editbox, 150
  );
  ...
  editbox->subscribeEvent( CEGUI::Editbox::EventTextChanged,
  CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Const_Rotation_X_Base_Text_Changed, this ) );
  */
  // constant rotation x rand
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_const_rotation_x_rand" )); Editor_Add(
  UTF8_("Random"), UTF8_("Additional random value"), editbox, 150, 28, 0 );

  editbox->setValidationString( "[+]?[0-9]*\\.?[0-9]*" );
  editbox->setText( float_to_string( m_const_rot_x_rand, 6, 0 ) );
  editbox->subscribeEvent( CEGUI::Editbox::EventTextChanged,
  CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Const_Rotation_X_Rand_Text_Changed, this ) );
  */
  // constant rotation y
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_const_rotation_y_base" )); Editor_Add(
  UTF8_("Const. rotation y"), UTF8_("Initial constant rotation y"), editbox, 150
  );
  ...
  editbox->subscribeEvent( CEGUI::Editbox::EventTextChanged,
  CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Const_Rotation_Y_Base_Text_Changed, this ) );
  */
  // constant rotation y rand
  /*editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
  "TaharezLook/Editbox", "emitter_const_rotation_y_rand" )); Editor_Add(
  UTF8_("Random"), UTF8_("Additional random value"), editbox, 150, 28, 0 );
  ...
  editbox->subscribeEvent( CEGUI::Editbox::EventTextChanged,
  CEGUI::Event::Subscriber(
  &cParticle_Emitter::Editor_Const_Rotation_Y_Rand_Text_Changed, this ) );
  */
  // constant rotation z
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "emitter_const_rotation_z_base"));
  Editor_Add(UTF8_("Const. rotation z"), UTF8_("Initial constant rotation z"),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Const_Rotation_Z_Base_Text_Changed, this));

  // constant rotation z rand
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "emitter_const_rotation_z_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Const_Rotation_Z_Rand_Text_Changed, this));

  // direction
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_direction_base"));
  Editor_Add(UTF8_("Direction"), UTF8_("Initial direction/angle"), editbox,
             150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Direction_Base_Text_Changed, this));

  // direction rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_direction_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Direction_Rand_Text_Changed, this));

  // scale
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_scale_base"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Scale_Base_Text_Changed, this));

  // scale rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_scale_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Scale_Rand_Text_Changed, this));

  // horizontal gravity
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_hor_gravity_base"));
  Editor_Add(UTF8_("Hor gravity"), UTF8_("Initial horizontal gravity"), editbox,
             150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Horizontal_Gravity_Base_Text_Changed,
          this));

  // horizontal gravity rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_hor_gravity_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Horizontal_Gravity_Rand_Text_Changed,
          this));

  // vertical gravity
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_ver_gravity_base"));
  Editor_Add(UTF8_("Ver gravity"), UTF8_("Initial vertical gravity"), editbox,
             150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Vertical_Gravity_Base_Text_Changed, this));

  // vertical gravity rand
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_ver_gravity_rand"));
  Editor_Add(UTF8_("Random"), UTF8_("Additional random value"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Vertical_Gravity_Rand_Text_Changed, this));

  // clip rect x
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_clip_rect_x"));
  Editor_Add(UTF8_("Clip rect x"), UTF8_("Clipping rectangle position x"),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Clip_Rect_X_Text_Changed, this));

  // clip rect w
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_clip_rect_w"));
  Editor_Add(UTF8_("Width"), UTF8_("Clipping rectangle width"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Clip_Rect_W_Text_Changed, this));

  // clip rect y
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_clip_rect_y"));
  Editor_Add(UTF8_("Clip rect y"), UTF8_("Clipping rectangle position y"),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Clip_Rect_Y_Text_Changed, this));

  // clip rect h
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "emitter_clip_rect_h"));
  Editor_Add(UTF8_("Height"), UTF8_("Clipping rectangle height"), editbox, 150,
             28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cParticle_Emitter::Editor_Clip_Rect_H_Text_Changed, this));

  // clip mode
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "emitter_clip_mode"));
  Editor_Add(UTF8_("Clip mode"),
             UTF8_("Clipping mode if particle is out the rectangle"), combobox,
             100, 105);
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cParticle_Emitter::Editor_Clip_Mode_Select,
                               this));
...
bool cParticle_Emitter ::Editor_Filename_Text_Changed(
...
bool cParticle_Emitter ::Editor_Pos_Z_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Pos_Z_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Emitter_Based_On_Camera_Pos_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  bool enabled =
      static_cast<CEGUI::ToggleButton *>(windowEventArgs.window)->isSelected();
...
bool cParticle_Emitter ::Editor_Particle_Based_On_Emitter_Pos_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Emitter_Width_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Emitter_Rect(m_start_pos_x, m_start_pos_y, string_to_float(str_text),
                   m_start_rect.m_h);
  Pre_Update();
...
bool cParticle_Emitter ::Editor_Emitter_Height_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Emitter_Rect(m_start_pos_x, m_start_pos_y, m_start_rect.m_w,
                   string_to_float(str_text));
  Pre_Update();

  return 1;
}

bool cParticle_Emitter ::Editor_Emitter_Time_To_Live_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Emitter_Interval_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Quota_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_TTL_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_TTL_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Velocity_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Velocity_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Rotation_X_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Rotation_Y_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Start_Rot_Z_Uses_Direction_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  bool enabled =
...
bool cParticle_Emitter ::Editor_Rotation_Z_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_X_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_X_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_Y_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_Y_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_Z_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Const_Rotation_Z_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Direction_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Direction_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Scale_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Scale_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Horizontal_Gravity_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Horizontal_Gravity_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Vertical_Gravity_Base_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Vertical_Gravity_Rand_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cParticle_Emitter ::Editor_Clip_Rect_X_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Clip_Rect(string_to_float(str_text), m_clip_rect.m_y, m_clip_rect.m_w,
                m_clip_rect.m_h);
  Pre_Update();
...
bool cParticle_Emitter ::Editor_Clip_Rect_Y_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Clip_Rect(m_clip_rect.m_x, string_to_float(str_text), m_clip_rect.m_w,
                m_clip_rect.m_h);
  Pre_Update();
...
bool cParticle_Emitter ::Editor_Clip_Rect_W_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Clip_Rect(m_clip_rect.m_x, m_clip_rect.m_y, string_to_float(str_text),
                m_clip_rect.m_h);
  Pre_Update();
...
bool cParticle_Emitter ::Editor_Clip_Rect_H_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
  ...
  Set_Clip_Rect(m_clip_rect.m_x, m_clip_rect.m_y, m_clip_rect.m_w,
                string_to_float(str_text));
  Pre_Update();
...
bool cParticle_Emitter ::Editor_Clip_Mode_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
...
cAnimation_Manager ::cAnimation_Manager(void) : cObject_Manager<cAnimation>() {}
...
void cAnimation_Manager ::Add(cAnimation *animation) {
   ...
   * many spawned particle emitters only emit one particle with an emitter which
   * is very slow
   */

...
: cStatusText( sprite_manager )
{
	...
	CEGUI::System::getSingleton().getDefaultGUIContext().getRootWindow()->addChild( m_window_debug_text );
	// debug text
	m_text_debug_text = static_cast<CEGUI::Window *>(CEGUI::System::getSingleton().getDefaultGUIContext().getRootWindow()->getChildRecursive ( "text_debugmessage" ));
	// hide
...
void cDebugDisplay :: Update( void )
{
	...
	if( m_text.compare( m_text_old ) != 0 )
	{
		...
		m_text_debug_text->setSize( CEGUI::Size<CEGUI::UDim>( CEGUI::UDim( 0, ( text_width + 15 ) * global_upscalex ), CEGUI::UDim( 0, ( text_height + 15 ) * global_upscaley ) ) );

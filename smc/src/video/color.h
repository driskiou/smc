...
public:
  ...
  inline CEGUI::Colour Get_cegui_Color(void) const {
    return CEGUI::Colour(
        static_cast<float>(red) / 255, static_cast<float>(green) / 255,
        static_cast<float>(blue) / 255, static_cast<float>(alpha) / 255);
  }
  ...
  inline Color operator+(const Color &c) const {
    return Color(static_cast<Uint8>(red + c.red), green + c.green,
  ...
  inline bool operator!=(const SDL_Color &c) const { return !(operator==(c)); }
...
static const Color whitealpha128 =
    Color(static_cast<Uint8>(255), 255, 255, 128);
...
static const Color lightgreenalpha64 =
    Color(static_cast<Uint8>(30), 230, 30, 64);

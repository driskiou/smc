...
namespace SMC
{
  ...
  bool cCamera ::Move_to_Position_Gradually(
  ...
  bool cCamera ::Step_to_Position_Gradually(const float pos_x,
                                            const float pos_y)
  {
  ...
  {
    return pActive_Player->m_col_rect.m_y + pActive_Player->m_col_rect.m_h -
           5.0f - (game_res_h * 0.5f);
  }

  GL_rect cCamera ::Get_Rect(void) const
  {
    return GL_rect(m_x, m_y, static_cast<float>(game_res_w),
    ...
    if (Game_Mode == MODE_LEVEL)
    {
      ...
      if (!editor_enabled)
      {
        ...
        if (pLevel_Player->m_vely != 0 && m_ver_offset_speed > 0.0f)
        {
          ...
          if ((m_y_offset < 100.0f && pLevel_Player->m_vely > 0.0f) ||
              (m_y_offset > -100.0f && pLevel_Player->m_vely < 0.0f))
          {
            m_y_offset += (pLevel_Player->m_vely * m_ver_offset_speed) *
        ...
        else
        {
          ...
          if (pLevel_Player->m_no_vely_counter > 10.0f &&
              (m_y_offset > 20.0f || m_y_offset < -20.0f))
          {
        ...
        else
        {
          ...
          if (pLevel_Player->m_velx != 0 && m_hor_offset_speed > 0.0f)
          {
            ...
            if ((m_x_offset < 200.0f && pLevel_Player->m_velx > 0.0f) ||
                (m_x_offset > -200.0f && pLevel_Player->m_velx < 0.0f))
            {
              m_x_offset += (pLevel_Player->m_velx * m_hor_offset_speed) *
                            pFramerate->m_speed_factor;
            }
          }
          // slowly remove offset
          else
          {
            ...
            if (pLevel_Player->m_no_velx_counter > 10.0f &&
                (m_x_offset > 40.0f || m_x_offset < -40.0f))
            {

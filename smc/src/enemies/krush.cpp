...
cKrush ::cKrush(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cKrush::Init();
}
...
void cKrush ::Load_From_Savegame(cSave_Level_Object *save_object) {
  ...
  if (save_object->exists("state")) {
    Moving_state mov_state = static_cast<Moving_state>(
...
void cKrush ::DownGrade(bool force /* = 0 */) {
  ...
  if (!force) {
    ...
    if (m_state == STA_WALK) {
      ...
      Col_Move(0.0f,
               m_images[3].m_image->m_col_h - m_images[4].m_image->m_col_h, 1,
               1);
...
void cKrush ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_krush_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cKrush::Editor_Direction_Select, this));

...
public:
   ...
   */
  cObjectCollision *Create_Collision_Object(const cSprite *base, cSprite *col,
                                            Col_Valid_Type valid_type) const;
  /* Add a collision object to the collision list
   * returns true if successful
   * add_if_new : add the collision only if the collision object doesn't already
   * collide with us and is deleted if it does
   */
  bool Add_Collision(cObjectCollision *collision, bool add_if_new = 0);
  /* Add the collision objects to the collision list
   * add_if_new : add each collision only if the collision object doesn't
   * already collide with us and is deleted if it does
   */
...
public:
  ...
  // save to savegame
  virtual cSave_Level_Object *Save_To_Savegame(void) { return NULL; };

  /* Sets the image for drawing
   * if new_start_image is set the default start_image will be set to the given
   * image if del_img is set the given image will be deleted
   */
  virtual void Set_Image(cGL_Surface *new_image, bool new_start_image = 0,
                         bool del_img = 0);
   ...
   */
  virtual void Set_Spawned(bool enable = 0) { m_spawned = enable; };
   ...
   */
  inline void Set_Shadow_Pos(const float pos) { m_shadow_pos = pos; };
  // Set the shadow color
  inline void Set_Shadow_Color(const Color &shadow) {
    m_shadow_color = shadow;
  };
  // Set image color
  inline void Set_Color(const Uint8 red, const Uint8 green, const Uint8 blue,
  ...
  inline void Set_Color(const Color &col) { m_color = col; };
   ...
   */
  void Set_Color_Combine(const float red, const float green, const float blue,
                         const GLint com_type);

  /* Set if rotation affects the collision rect
   * only supports 90� steps currently
   * if enabled col_pos, col_rect and rect must be reset manually before
   * changing rotation
   */
  ...
  // Add X rotation
  inline void Add_Rotation_X(float rot) { Set_Rotation_X(m_rot_x + rot); };
  // Add Y rotation
  inline void Add_Rotation_Y(float rot) { Set_Rotation_Y(m_rot_y + rot); };
  // Add Z rotation
  inline void Add_Rotation_Z(float rot) { Set_Rotation_Z(m_rot_z + rot); };
  // Add rotation
   ...
   * if enabled previous scale is always undone from rect before setting the new
   ...
   * if all are set scaling is centered and if all are not set scaling is
   * disabled todo : if enabled with scale_affects_rect it should also scale the
   * rect position
   */
  inline void Set_Scale_Directions(const bool up = 0, const bool down = 1,
                                   const bool left = 0, const bool right = 1) {
    m_scale_up = up;
    m_scale_down = down;
    m_scale_left = left;
    m_scale_right = right;
  ...
  // Add scale
  inline void Add_Scale_X(const float val) { Set_Scale_X(m_scale_x + val); };
  inline void Add_Scale_Y(const float val) { Set_Scale_Y(m_scale_y + val); };
  ...
  inline bool Is_Basic_Sprite(void) const {
    if (m_type == TYPE_PASSIVE || m_type == TYPE_FRONT_PASSIVE ||
        m_type == TYPE_MASSIVE || m_type == TYPE_CLIMBABLE ||
        m_type == TYPE_HALFMASSIVE) {
      return 1;
    }
  ...
  inline bool Is_Sprite_Managed(void) const {
    if (m_sprite_array == ARRAY_MASSIVE || m_sprite_array == ARRAY_PASSIVE ||
        m_sprite_array == ARRAY_ENEMY || m_sprite_array == ARRAY_ACTIVE) {
      return 1;
    }
  ...
  void Editor_Add(const CEGUI::String &name, const CEGUI::String &tooltip,
                  CEGUI::Window *window_setting, float obj_width,
                  float obj_height = 28, bool advance_row = 1);
  // editor activation
  ...
  bool m_delete_image;
  // if this can not be auto-deleted because the object is controlled from
  // elsewhere

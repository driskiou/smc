...
public:
  ...
  // unary - operator
  inline GL_Vector operator-() const { return GL_Vector(-x, -y, -z, -w); }
  ...
  inline bool operator==(const GL_Vector &v) const {
    return (Is_Float_Equal(x, v.x) && Is_Float_Equal(y, v.y) &&
            Is_Float_Equal(z, v.z) && Is_Float_Equal(w, v.w));
  }

  // Returns false if current vector equals v
  // v : other vector
  inline bool operator!=(const GL_Vector &v) const { return !(operator==(v)); }

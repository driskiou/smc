...
cSave_Overworld_Waypoint ::cSave_Overworld_Waypoint(void) { m_access = 0; }
...
cSave_Level_Object_Property ::cSave_Level_Object_Property(
...
cSave_Level_Object ::cSave_Level_Object(void) { m_type = TYPE_UNDEFINED; }

cSave_Level_Object ::~cSave_Level_Object(void) { m_properties.clear(); }

bool cSave_Level_Object ::exists(const std::string &val_name) {
  for (Save_Level_Object_ProprtyList::iterator itr = m_properties.begin();
       itr != m_properties.end(); ++itr) {
...
cSave_Level ::~cSave_Level(void) {
  for (Save_Level_ObjectList::iterator itr = m_level_objects.begin();
       itr != m_level_objects.end(); ++itr) {
    delete *itr;
  }

  m_level_objects.clear();

  for (cSprite_List::iterator itr = m_spawned_objects.begin();
       itr != m_spawned_objects.end(); ++itr) {
    delete *itr;
  }
...
cSave ::cSave(void) { Init(); }

cSave ::~cSave(void) {
  for (Save_LevelList::iterator itr = m_levels.begin(); itr != m_levels.end();
       ++itr) {
    delete *itr;
  }
  ...
  for (Save_OverworldList::iterator itr = m_overworlds.begin();
       itr != m_overworlds.end(); ++itr) {
    delete *itr;
  }
...
std::string cSave ::Get_Active_Level(void) {
  ...
  for (Save_LevelList::iterator itr = m_levels.begin(); itr != m_levels.end();
    ...
    // if active level
    if (!Is_Float_Equal(save_level->m_level_pos_x, 0.0f) &&
        !Is_Float_Equal(save_level->m_level_pos_y, 0.0f)) {
...
int cSavegame ::Load_Game(unsigned int save_slot) {
  ...
  if (!savegame->m_overworlds.empty()) {
    for (Save_OverworldList::iterator itr = savegame->m_overworlds.begin();
         itr != savegame->m_overworlds.end(); ++itr) {
      // get savegame overworld pointer
      cSave_Overworld *save_overworld = (*itr);

      // get overworld
      cOverworld *overworld =
          pOverworld_Manager->Get_from_Name(save_overworld->m_name);

      if (!overworld) {
        printf("Warning : Savegame %d : Overworld %s not found\n", save_slot,
               save_overworld->m_name.c_str());
        continue;
      }

      for (Save_Overworld_WaypointList::iterator wp_itr =
               save_overworld->m_waypoints.begin();
           wp_itr != save_overworld->m_waypoints.end(); ++wp_itr) {
        // get savegame waypoint pointer
        cSave_Overworld_Waypoint *save_waypoint = (*wp_itr);

        // get overworld waypoint
        cWaypoint *waypoint = overworld->Get_Waypoint(
            overworld->Get_Waypoint_Num(save_waypoint->m_destination));

        // not found
        if (!waypoint) {
          printf("Warning : Savegame %d : Overworld %s Waypoint %s not found\n",
                 save_slot, save_overworld->m_name.c_str(),
                 save_waypoint->m_destination.c_str());
          continue;
        }
  ...
  if (!savegame->m_overworld_active.empty()) {
    ...
    // Current waypoint
    if (!pOverworld_Player->Set_Waypoint(
            savegame->m_overworld_current_waypoint)) {
      printf(
          "Warning : Savegame %d : Overworld Current Waypoint %d is invalid\n",
  ...
  if (savegame->m_version < 8) {
    pLevel_Player->Set_Type(static_cast<Maryo_type>(savegame->m_player_state),
                            0, 0);
  } else {
    pLevel_Player->Set_Type(static_cast<Maryo_type>(savegame->m_player_type), 0,
                            0);
    pLevel_Player->m_state =
        static_cast<Moving_state>(savegame->m_player_state);
  }
  ...
  if (!savegame->m_levels.empty()) {
    for (Save_LevelList::iterator itr = savegame->m_levels.begin();
         itr != savegame->m_levels.end(); ++itr) {
      cSave_Level *save_level = (*itr);
      ...
      if (!Is_Float_Equal(save_level->m_level_pos_x, 0.0f) &&
        ...
        // if below version 9 : move y coordinate bottom to 0 and remove screen
        // height adjustment
        ...
        // position
        pLevel_Player->Set_Pos(save_level->m_level_pos_x,
                               save_level->m_level_pos_y);
      ...
      // spawned objects
      for (cSprite_List::iterator itr = save_level->m_spawned_objects.begin();
           itr != save_level->m_spawned_objects.end(); ++itr) {
        cSprite *sprite = (*itr);
      ...
      // objects data
      for (Save_Level_ObjectList::iterator itr =
               save_level->m_level_objects.begin();
           itr != save_level->m_level_objects.end(); ++itr) {
        ...
        // get level object
        cSprite *level_object = level->m_sprite_manager->Get_from_Position(
            posx, posy, save_object->m_type, 1);

        // if not anymore available
        if (!level_object) {
          printf(
              "Warning : Savegame object type %d on x %d, y %d not available\n",
  ...
  pHud_Debug->Set_Text(_("Savegame ") + int_to_string(save_slot) +
                       _(" loaded"));
  pHud_Manager->Update();
...
bool cSavegame ::Save_Game(unsigned int save_slot, std::string description) {
  ...
  if (pActive_Level->Is_Loaded()) {
    ...
    for (vector<cLevel *>::iterator itr = pLevel_Manager->objects.begin();
         itr != pLevel_Manager->objects.end(); ++itr) {
      cLevel *level = (*itr);
      ...
      // spawned objects
      for (cSprite_List::iterator itr =
               level->m_sprite_manager->objects.begin();
           itr != level->m_sprite_manager->objects.end(); ++itr) {
        cSprite *obj = (*itr);
  ...
  for (vector<cOverworld *>::iterator itr = pOverworld_Manager->objects.begin();
    ...
    // Waypoints
    for (cSprite_List::iterator wp_itr =
             overworld->m_sprite_manager->objects.begin();
         wp_itr != overworld->m_sprite_manager->objects.end(); ++wp_itr) {
      // get waypoint
  ...
  if (pActive_Overworld && Game_Mode_Type != MODE_TYPE_LEVEL_CUSTOM) {
    ...
    if (pOverworld_Player->m_current_waypoint >= 0) {
      ...
      savegame->m_overworld_current_waypoint =
          pOverworld_Player->m_current_waypoint;
    }
  }
...
cSave *cSavegame ::Load(unsigned int save_slot) {
  std::string filename =
      m_savegame_dir + "/" + int_to_string(save_slot) + ".smcsav";
...
// fixme : Check if there is a more portable way f.e. with imbue()
  ...
  if (!file.is_open()) {
    printf("Error : Couldn't open savegame file for saving. Is the file "
           "read-only ?");
    pHud_Debug->Set_Text(_("Couldn't save savegame ") + filename,
  ...
  Write_Property(stream, "level_engine_version",
                 savegame->m_level_engine_version);
  Write_Property(stream, "save_time",
                 static_cast<Uint64>(savegame->m_save_time));
  ...
  Write_Property(stream, "overworld_current_waypoint",
                 savegame->m_overworld_current_waypoint);
  // end player
  stream.closeTag();
  ...
  for (Save_LevelList::iterator itr = savegame->m_levels.begin();
    ...
    // save position if active level
    if (!Is_Float_Equal(level->m_level_pos_x, 0.0f) &&
        !Is_Float_Equal(level->m_level_pos_y, 0.0f)) {
    ...
    for (cSprite_List::iterator itr = level->m_spawned_objects.begin();
         itr != level->m_spawned_objects.end(); ++itr) {
      cSprite *obj = (*itr);
    ...
    for (Save_Level_ObjectList::iterator itr = level->m_level_objects.begin();
         itr != level->m_level_objects.end(); ++itr) {
      ...
      // Properties
      for (Save_Level_Object_ProprtyList::iterator prop_itr =
               obj->m_properties.begin();
           prop_itr != obj->m_properties.end(); ++prop_itr) {
  ...
  for (Save_OverworldList::iterator itr = savegame->m_overworlds.begin();
    ...
    for (Save_Overworld_WaypointList::iterator wp_itr =
             overworld->m_waypoints.begin();
         wp_itr != overworld->m_waypoints.end(); ++wp_itr) {
...
std::string cSavegame ::Get_Description(unsigned int save_slot,
                                        bool only_description /* = 0 */) {
  ...
  if (!only_description) {
    ...
    str_description += _(" - Date ") + Time_to_String(savegame->m_save_time,
                                                      "%Y-%m-%d  %H:%M:%S");
  }
...
bool cSavegame ::Is_Valid(unsigned int save_slot) const {
  return (
      File_Exists(m_savegame_dir + "/" + int_to_string(save_slot) +
                  ".smcsav") ||
      File_Exists(m_savegame_dir + "/" + int_to_string(save_slot) + ".save"));
}
...
// again if CEGUI 0.8 works with std::string utf8
    ...
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, (const CEGUI::utf8 *)filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/" + xsd_name, "");
#else
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, filename.c_str(), DATA_DIR "/" GAME_SCHEMA_DIR "/" + xsd_name,
        "");
#endif
  }
  // catch CEGUI Exceptions
    ...
    printf("Loading Savegame %s CEGUI Exception %s\n", filename.c_str(),
           ex.getMessage().c_str());
    pHud_Debug->Set_Text(_("Savegame Loading failed : ") +
...
void cSavegame_XML_Handler ::elementStart(
    const CEGUI::String &element, const CEGUI::XMLAttributes &attributes) {
  if (element == "property") {
...
void cSavegame_XML_Handler ::elementEnd(const CEGUI::String &element) {
  ...
  if (element == "information" || element == "Information") {
    ...
    m_savegame->m_level_engine_version = m_xml_attributes.getValueAsInteger(
        "level_engine_version", m_savegame->m_level_engine_version);
    m_savegame->m_save_time =
        string_to_int64(m_xml_attributes.getValueAsString("save_time").c_str());
    m_savegame->m_description =
        m_xml_attributes.getValueAsString("description").c_str();
...
void cSavegame_XML_Handler ::Handle_Level(
...
void cSavegame_XML_Handler ::Handle_Level_Object(
    const CEGUI::XMLAttributes &attributes) {
  ...
  for (unsigned int i = 0; i < m_xml_attributes.getCount(); i++) {
    ...
    if (property_name.compare("level_name") == 0 ||
        property_name.compare("player_posx") == 0 ||
        property_name.compare("player_posy") == 0) {
      continue;
    }

    object->m_properties.push_back(cSave_Level_Object_Property(
        property_name, m_xml_attributes.getValue(i).c_str()));
  }

  // remove used properties
  for (Save_Level_Object_ProprtyList::iterator prop_itr =
           object->m_properties.begin();
       prop_itr != object->m_properties.end(); ++prop_itr) {
    cSave_Level_Object_Property Property = (*prop_itr);
...
void cSavegame_XML_Handler ::Handle_Level_Spawned_Object(
    const CEGUI::String &element, CEGUI::XMLAttributes &attributes) {
  cSprite *sprite = Create_Level_Object_From_XML(
      element, attributes, m_savegame->m_level_engine_version,
      pActive_Level->m_sprite_manager);
...
void cSavegame_XML_Handler ::Handle_Player(
    const CEGUI::XMLAttributes &attributes) {
  m_savegame->m_lives = m_xml_attributes.getValueAsInteger("lives");
  m_savegame->m_points =
      string_to_long(m_xml_attributes.getValueAsString("points", "0").c_str());
  ...
  m_savegame->m_itembox_item =
      m_xml_attributes.getValueAsInteger("itembox_item");
  // new in V.11
  m_savegame->m_level_time = m_xml_attributes.getValueAsInteger("level_time");
  if (!m_old_format) {
    m_savegame->m_overworld_active =
        m_xml_attributes.getValueAsString("overworld_active").c_str();
    m_savegame->m_overworld_current_waypoint =
        m_xml_attributes.getValueAsInteger("overworld_current_waypoint");
  }
}

void cSavegame_XML_Handler ::Handle_Overworld_Data(
    const CEGUI::XMLAttributes &attributes) {
  // savegame format V.10 and lower
  m_savegame->m_overworld_active =
      m_xml_attributes.getValueAsString("active").c_str();
  m_savegame->m_overworld_current_waypoint =
      m_xml_attributes.getValueAsInteger("current_waypoint");
}

void cSavegame_XML_Handler ::Handle_Overworld(
    const CEGUI::XMLAttributes &attributes) {
...
void cSavegame_XML_Handler ::Handle_Overworld_Waypoint(
    const CEGUI::XMLAttributes &attributes) {
  ...
  if (m_xml_attributes.exists("world_name")) {
    waypoint->m_destination =
        m_xml_attributes.getValueAsString("world_name").c_str();
  } else if (m_xml_attributes.exists("level_name")) {
    waypoint->m_destination =
        m_xml_attributes.getValueAsString("level_name").c_str();
  }
  // default
  else {
    waypoint->m_destination =
        m_xml_attributes.getValueAsString("destination").c_str();
  }

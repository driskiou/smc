...
cMoving_Platform ::cMoving_Platform(cSprite_Manager *sprite_manager)
    : cAnimated_Sprite(sprite_manager, "moving_platform"),
      m_path_state(sprite_manager) {
...
void cMoving_Platform ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // move type
  Set_Move_Type(static_cast<Moving_Platform_Type>(
      attributes.getValueAsInteger("move_type", m_move_type)));
  // massive type
  Set_Massive_Type(Get_Massive_Type_Id(
      attributes
          .getValueAsString("massive_type",
                            Get_Massive_Type_Name(m_massive_type))
          .c_str()));
  // if move type is line or circle
  if (m_move_type == MOVING_PLATFORM_TYPE_LINE ||
      m_move_type == MOVING_PLATFORM_TYPE_CIRCLE) {
    // direction
    Set_Direction(Get_Direction_Id(
                      attributes
                          .getValueAsString("direction", Get_Direction_Name(
                                                             m_start_direction))
                          .c_str()),
                  1);
    // max distance
    Set_Max_Distance(
        attributes.getValueAsInteger("max_distance", m_max_distance));
  }
  // path identifier
  if (m_move_type == MOVING_PLATFORM_TYPE_PATH ||
      m_move_type == MOVING_PLATFORM_TYPE_PATH_BACKWARDS) {
  ...
  // touch move time
  Set_Touch_Move_Time(
      attributes.getValueAsFloat("touch_move_time", m_touch_move_time));
  // middle image count
  Set_Middle_Count(
      attributes.getValueAsInteger("middle_img_count", m_middle_count));
  // image top left
  Set_Image_Top_Left(pVideo->Get_Surface(
      attributes
          .getValueAsString("image_top_left",
                            m_images[0].m_image->Get_Filename())
          .c_str()));
  // image top middle
  Set_Image_Top_Middle(pVideo->Get_Surface(
      attributes
          .getValueAsString("image_top_middle",
                            m_images[1].m_image->Get_Filename())
          .c_str()));
  // image top right
  Set_Image_Top_Right(pVideo->Get_Surface(
      attributes
          .getValueAsString("image_top_right",
                            m_images[2].m_image->Get_Filename())
          .c_str()));
}
...
void cMoving_Platform ::Save_To_XML(CEGUI::XMLSerializer &stream) {
  ...
  // image top left
  Write_Property(stream, "image_top_left",
                 m_images[0].m_image->Get_Filename(1));
  // image top middle
  Write_Property(stream, "image_top_middle",
                 m_images[1].m_image->Get_Filename(1));
  // image top right
  Write_Property(stream, "image_top_right",
                 m_images[2].m_image->Get_Filename(1));
...
void cMoving_Platform ::Load_From_Savegame(cSave_Level_Object *save_object) {
  ...
  if (save_object->exists("direction")) {
    m_direction = static_cast<ObjectDirection>(
        string_to_int(save_object->Get_Value("direction")));
  }

  // platform state
  if (save_object->exists("platform_state")) {
    m_platform_state = static_cast<Moving_Platform_State>(
        string_to_int(save_object->Get_Value("platform_state")));
  }
...
cSave_Level_Object *cMoving_Platform ::Save_To_Savegame(void) {
  ...
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "posx", int_to_string(static_cast<int>(m_start_pos_x))));
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "posy", int_to_string(static_cast<int>(m_start_pos_y))));

  // direction
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("direction", int_to_string(m_direction)));

  // platform state
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "platform_state", int_to_string(m_platform_state)));

  // new position ( only save if needed )
  if (m_start_pos_x != m_pos_x || m_start_pos_y != m_pos_y) {
    save_object->m_properties.push_back(cSave_Level_Object_Property(
        "new_posx", int_to_string(static_cast<int>(m_pos_x))));
    save_object->m_properties.push_back(cSave_Level_Object_Property(
        "new_posy", int_to_string(static_cast<int>(m_pos_y))));
  }

  // velocity
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("velx", float_to_string(m_velx)));
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("vely", float_to_string(m_vely)));

  // active ( only save if needed )
  if (!m_active) {
    save_object->m_properties.push_back(
        cSave_Level_Object_Property("active", int_to_string(m_active)));
  }
...
void cMoving_Platform ::Set_Direction(const ObjectDirection dir,
...
void cMoving_Platform ::Update(void) {
  ...
  if (m_touch_time > 0.0f) {
    if (m_platform_state == MOVING_PLATFORM_TOUCHED &&
      ...
      if (m_shake_counter > m_shake_time) {
        ...
        // todo : change massivetype so no object can collide with it anymore
        // but this needs a start_massivetype for saving
      }
      ...
      if (m_col_rect.m_y >
          pActive_Camera->m_limit_rect.m_y + game_res_h + 300.0f) {
        m_platform_state = MOVING_PLATFORM_STAY;
        m_can_be_ground = 0;
        Set_Active(0);
      }
  ...
  // if moving
  if (m_speed && m_platform_state != MOVING_PLATFORM_FALL &&
      (!m_touch_move_time || m_touch_counter > m_touch_move_time)) {
    if (m_move_type == MOVING_PLATFORM_TYPE_LINE) {
      ...
      float diff_x = (m_start_pos_x +
                      (m_max_distance * sin(m_moving_angle * deg_to_rad))) -
                     m_pos_x;
      float diff_y = (m_start_pos_y - m_max_distance +
                      (m_max_distance * cos(m_moving_angle * deg_to_rad))) -
                     m_pos_y;

      // move to position
      Set_Velocity(diff_x, diff_y);
    } else if (m_move_type == MOVING_PLATFORM_TYPE_PATH ||
               m_move_type == MOVING_PLATFORM_TYPE_PATH_BACKWARDS) {
      if (m_path_state.m_path) {
        ...
        // get difference
        float diff_x =
            (m_path_state.m_path->m_start_pos_x + m_path_state.m_pos_x) -
            m_pos_x;
        float diff_y =
            (m_path_state.m_path->m_start_pos_y + m_path_state.m_pos_y) -
            m_pos_y;
  ...
  if (editor_level_enabled && m_speed) {
    if (m_move_type == MOVING_PLATFORM_TYPE_LINE) {
      if (m_start_direction == DIR_RIGHT) {
        pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x,
                          m_start_pos_y - pActive_Camera->m_y,
                          m_max_distance + m_col_rect.m_w, 2, m_pos_z - 0.0001f,
                          &m_editor_color);
      } else if (m_start_direction == DIR_DOWN) {
        pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x,
                          m_start_pos_y - pActive_Camera->m_y, 2,
                          m_max_distance + m_col_rect.m_h, m_pos_z - 0.0001f,
                          &m_editor_color);
      }
      // added in version 1.7
      else if (m_start_direction == DIR_UP) {
        pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x,
                          m_start_pos_y - pActive_Camera->m_y - m_max_distance,
                          2, m_max_distance + m_col_rect.m_h, m_pos_z - 0.0001f,
                          &m_editor_color);
      }
      // added in version 1.7
      else if (m_start_direction == DIR_LEFT) {
        pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x - m_max_distance,
                          m_start_pos_y - pActive_Camera->m_y,
                          m_max_distance + m_col_rect.m_w, 2, m_pos_z - 0.0001f,
                          &m_editor_color);
      }
      ...
      pVideo->Draw_Circle(m_start_pos_x - pActive_Camera->m_x,
                          m_start_pos_y - m_max_distance - pActive_Camera->m_y,
                          static_cast<float>(m_max_distance), m_pos_z - 0.0001f,
                          &m_editor_color, circle_request);
...
void cMoving_Platform ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_moving_platform_move_type"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cMoving_Platform::Editor_Move_Type_Select,
                               this));

  // path identifier
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_path_identifier"));
  Editor_Add(UTF8_("Path Identifier"), UTF8_("Name of the Path to move along."),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Path_Identifier_Text_Changed, this));

  // direction
  combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_moving_platform_direction"));
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMoving_Platform::Editor_Direction_Select,
                               this));

  // max distance
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_max_distance"));
  Editor_Add(UTF8_("Distance"),
             UTF8_("Movable distance into its direction if type is line or "
                   "radius if circle."),
             editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Max_Distance_Text_Changed, this));

  // speed
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_moving_platform_speed"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cMoving_Platform::Editor_Speed_Text_Changed,
                               this));

  // touch time
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_touch_time"));
  Editor_Add(UTF8_("Touch time"),
             UTF8_("Time when touched until shaking starts"), editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Touch_Time_Text_Changed, this));

  // shake time
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_shake_time"));
  Editor_Add(UTF8_("Shake time"), UTF8_("Time it's shaking until falling"),
             editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Shake_Time_Text_Changed, this));

  // touch move time
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_move_time"));
  Editor_Add(
      UTF8_("Touch move time"),
      UTF8_("If set does not move until this time has elapsed after touched"),
      editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Touch_Move_Time_Text_Changed, this));

  // horizontal middle image count
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_hor_middle_count"));
  Editor_Add(UTF8_("Hor image count"), UTF8_("Horizontal middle image count"),
             editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Hor_Middle_Count_Text_Changed, this));

  // image top left
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_image_top_left"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Image_Top_Left_Text_Changed, this));

  // image top middle
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_image_top_middle"));
  Editor_Add(UTF8_("Image top middle"), UTF8_("Image top middle"), editbox,
             200);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Image_Top_Middle_Text_Changed, this));

  // image top right
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_moving_platform_image_top_right"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cMoving_Platform::Editor_Image_Top_Right_Text_Changed, this));
...
void cMoving_Platform ::Editor_State_Update(void) {
  ...
  // path identifier
  CEGUI::Editbox *editbox_path_identifier = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_moving_platform_path_identifier"));
  // direction
  CEGUI::Combobox *combobox_direction = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_moving_platform_direction"));
  // max distance
  CEGUI::Editbox *editbox_max_distance = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_moving_platform_max_distance"));
  ...
  if (m_move_type == MOVING_PLATFORM_TYPE_PATH ||
    ...
    if (m_move_type == MOVING_PLATFORM_TYPE_CIRCLE) {
      CEGUI::ListboxItem *item =
          combobox_direction->findItemWithText("up", NULL);
...
bool cMoving_Platform ::Editor_Path_Identifier_Text_Changed(
...
bool cMoving_Platform ::Editor_Max_Distance_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Speed_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Touch_Time_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Shake_Time_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Touch_Move_Time_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Hor_Middle_Count_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Image_Top_Left_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Image_Top_Middle_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cMoving_Platform ::Editor_Image_Top_Right_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
void cMoving_Platform ::Create_Name(void) {
  ...
  m_name += _("Platform - ") + Get_Direction_Name(m_start_direction) + " - " +
            Get_Massive_Type_Name(m_massive_type);
}

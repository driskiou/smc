...
void cTurtleBoss ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  Set_Direction(Get_Direction_Id(
                    attributes
                        .getValueAsString("direction",
                                          Get_Direction_Name(m_start_direction))
                        .c_str()),
                1);
  // color
  ...
  // max downgrade count
  Set_Max_Downgrade_Counts(attributes.getValueAsInteger("max_downgrade_count",
                                                        m_max_downgrade_count));
  // shell time
  Set_Shell_Time(attributes.getValueAsFloat("shell_time", m_shell_time));
  // level ends if killed
  Set_Level_Ends_If_Killed(attributes.getValueAsBool("level_ends_if_killed",
                                                     m_level_ends_if_killed));
}
...
void cTurtleBoss ::Set_Direction(const ObjectDirection dir,
...
void cTurtleBoss ::Update(void) {
  ...
  // moving shell
    ...
    if (static_cast<int>(m_counter) % 2 == 1) {
      ...
      cMovingSprite::Set_Color(Color(static_cast<Uint8>(255),
                                     250 - static_cast<Uint8>(m_counter * 1.5f),
                                     250 - static_cast<Uint8>(m_counter * 4)));
    }
    // default
    else {
    ...
    if (m_counter > 60.0f) {
      ...
      Set_Turtle_Moving_State(TURTLEBOSS_SHELL_RUN);
      Col_Move(0.0f,
               m_images[0].m_image->m_col_h - m_images[5].m_image->m_col_h, 1,
               1);
...
void cTurtleBoss ::Stand_Up(void) {
  if (m_turtle_state != TURTLEBOSS_SHELL_STAND &&
      m_turtle_state != TURTLEBOSS_SHELL_RUN) {
    return;
  }
...
bool cTurtleBoss ::Hit_Enemy(cEnemy *enemy) const {
  ...
  pHud_Points->Add_Points(enemy->m_kill_points, m_pos_x + m_image->m_w / 3,
                          m_pos_y - 5.0f, "", static_cast<Uint8>(255), 1);
  enemy->DownGrade(1);
...
void cTurtleBoss ::Throw_Fireballs(unsigned int amount /* = 6 */) {
  ...
  cAnimation_Fireball *anim =
      new cAnimation_Fireball(m_sprite_manager, m_pos_x + (m_col_rect.m_w / 2),
...
void cTurtleBoss ::Generate_Stars(unsigned int amount /* = 1 */,
                                  float particle_scale /* = 0.4f */) const {
  // animation
...
void cTurtleBoss ::Handle_Collision_Player(cObjectCollision *collision) {
  if (collision->m_direction == DIR_UNDEFINED ||
      (m_turtle_state == TURTLEBOSS_SHELL_RUN && m_player_counter > 0.0f) ||
      m_state == STA_OBJ_LINKED) {
    return;
  }

  if (collision->m_direction == DIR_TOP && pLevel_Player->m_state != STA_FLY) {
    if (m_turtle_state == TURTLEBOSS_WALK) {
      pHud_Points->Add_Points(250, pLevel_Player->m_pos_x,
                              pLevel_Player->m_pos_y);
      ...
      pHud_Points->Add_Points(100, pLevel_Player->m_pos_x,
                              pLevel_Player->m_pos_y);
      pAudio->Play_Sound("enemy/turtle/shell/hit.ogg");
    } else if (m_turtle_state == TURTLEBOSS_SHELL_RUN) {
      pHud_Points->Add_Points(50, pLevel_Player->m_pos_x,
                              pLevel_Player->m_pos_y);
      pAudio->Play_Sound("enemy/turtle/shell/hit.ogg");
    }
    ...
    // animation
    cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
    anim->Set_Pos(m_pos_x + (m_col_rect.m_w / 2),
                  m_pos_y + (m_col_rect.m_h / 2));
    ...
    if (m_turtle_state == TURTLEBOSS_SHELL_RUN) {
      ...
      if ((pLevel_Player->m_col_rect.m_w / 2) + pLevel_Player->m_pos_x <
          (m_col_rect.m_w / 2) + m_pos_x) {
        Set_Direction(DIR_RIGHT);
        m_velx = m_velx_max;
      }
      // right side
    ...
    if (m_turtle_state == TURTLEBOSS_WALK) {
      ...
      if (collision->m_direction == DIR_RIGHT) {
        anim->Set_Pos(m_pos_x + m_col_pos.m_x + m_col_rect.m_w,
                      m_pos_y + (m_col_rect.m_h / 2));
        ...
        anim->Set_Pos(m_pos_x + (m_col_rect.m_w / 2),
                      m_pos_y + m_col_pos.m_y + m_col_rect.m_h);
  ...
  if (m_turtle_state == TURTLEBOSS_WALK) {
    ...
    if (collision->m_direction == DIR_RIGHT ||
      ...
      if (collision->m_direction == DIR_RIGHT) {
        ...
        anim->Set_Pos(col_object->m_pos_x + col_object->m_col_pos.m_x + 4,
                      m_pos_y + (m_col_rect.m_h / 1.35f), 1);
        anim->Set_Direction_Range(140.0f, 100.0f);
      } else {
        anim = new cParticle_Emitter(m_sprite_manager);
        anim->Set_Pos(col_object->m_pos_x + col_object->m_col_pos.m_x +
                          col_object->m_col_rect.m_w - 4,
                      m_pos_y + (m_col_rect.m_h / 1.35f), 1);
    ...
    if (m_vely < -5.0f) {
      ...
      if (collision->m_array == ARRAY_ACTIVE) {
        ...
        if (col_object->m_type == TYPE_BONUS_BOX ||
            col_object->m_type == TYPE_SPIN_BOX) {
          // get basebox
          cBaseBox *box = static_cast<cBaseBox *>(col_object);

          // if useable
          if (box->m_useable_count != 0) {
            pHud_Points->Add_Points(50, m_pos_x + m_image->m_w / 3,
                                    m_pos_y - 5.0f);
            Send_Collision(collision);
            DownGrade(1);
          }
...
void cTurtleBoss ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_turtle_boss_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cTurtleBoss::Editor_Direction_Select, this));

  // max hits
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_turtle_boss_max_hits"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cTurtleBoss::Editor_Max_Hits_Text_Changed,
                               this));

  // max downgrades
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_turtle_boss_max_downgrade_count"));
  Editor_Add(UTF8_("Downgrades"), UTF8_("Downgrades until death"), editbox,
             120);
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cTurtleBoss::Editor_Max_Downgrade_Counts_Text_Changed, this));

  // max shell time
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_turtle_boss_max_shell_time"));
  Editor_Add(UTF8_("Shell Time"), UTF8_("Time running as shell to rise again"),
             editbox, 200);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cTurtleBoss::Editor_Shell_Time_Text_Changed,
                               this));

  // level ends if killed
  combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_turtle_boss_level_ends_if_killed"));
  Editor_Add(UTF8_("End Level"), UTF8_("End the level if it is killed."),
             combobox, 100, 75);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cTurtleBoss::Editor_Level_Ends_If_Killed,
                               this));
...
bool cTurtleBoss ::Editor_Max_Downgrade_Counts_Text_Changed(
...
bool cTurtleBoss ::Editor_Shell_Time_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =

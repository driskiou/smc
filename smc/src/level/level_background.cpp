...
void cBackground ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  if (m_type == BG_GR_HOR || m_type == BG_GR_VER) {
    Set_Color_1(Color(
        static_cast<Uint8>(attributes.getValueAsInteger("bg_color_1_red")),
        attributes.getValueAsInteger("bg_color_1_green"),
        attributes.getValueAsInteger("bg_color_1_blue")));
    Set_Color_2(Color(
        static_cast<Uint8>(attributes.getValueAsInteger("bg_color_2_red")),
        attributes.getValueAsInteger("bg_color_2_green"),
        attributes.getValueAsInteger("bg_color_2_blue")));
  } else if (m_type == BG_IMG_BOTTOM || m_type == BG_IMG_TOP ||
             m_type == BG_IMG_ALL) {
    Set_Start_Pos(attributes.getValueAsFloat("posx"),
                  attributes.getValueAsFloat("posy"));
    Set_Pos_Z(attributes.getValueAsFloat("posz"));

    Set_Image(attributes.getValueAsString("image").c_str());
    Set_Scroll_Speed(attributes.getValueAsFloat("speedx"),
                     attributes.getValueAsFloat("speedy"));
...
void cBackground ::Save_To_XML(CEGUI::XMLSerializer &stream) {
  ...
  if (m_type == BG_GR_HOR || m_type == BG_GR_VER) {
    ...
    Write_Property(stream, "bg_color_1_green",
                   static_cast<int>(m_color_1.green));
    ...
    Write_Property(stream, "bg_color_2_green",
                   static_cast<int>(m_color_2.green));
...
void cBackground ::Set_Type(const BackgroundType type) { m_type = type; }
...
void cBackground ::Set_Color_1(const Color &color) { m_color_1 = color; }

void cBackground ::Set_Color_2(const Color &color) { m_color_2 = color; }
...
void cBackground ::Set_Image(const std::string &img_file_1) {
  ...
  if (m_image_1_filename.find(DATA_DIR "/" GAME_PIXMAPS_DIR "/") !=
      std::string::npos) {
...
void cBackground ::Set_Scroll_Speed(const float x /* = 1.0f */,
                                    const float y /* = 1.0f */) {
  m_speed_x = x;
  m_speed_y = y;
}
...
void cBackground ::Set_Pos_Z(const float val) { m_pos_z = val; }
...
void cBackground ::Draw(void) {
  ...
  // image
    ...
    float posy_final =
        m_pos_y - (((pActive_Camera->m_y + game_res_h) * 0.3f) * m_speed_y);
...
void cBackground ::Draw_Gradient(void) {
  ...
  if (m_color_1 == m_color_2) {
    ...
    if (camera_bottom < -0.1f) {
      ...
      color_start.red += static_cast<Uint8>(
          static_cast<float>(m_color_2.red - m_color_1.red) * power);
      color_start.green += static_cast<Uint8>(
          static_cast<float>(m_color_2.green - m_color_1.green) * power);
      color_start.blue += static_cast<Uint8>(
          static_cast<float>(m_color_2.blue - m_color_1.blue) * power);
    }

    if (m_type == BG_GR_VER) {
      pVideo->Draw_Gradient(NULL, m_pos_z, &color_start, &m_color_2,
                            DIR_VERTICAL);

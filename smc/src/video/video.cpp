...
void cVideo ::Init_CEGUI_Fake(void) const {
  ...
  CEGUI::String log_dump_dir =
      (const CEGUI::utf8 *)((Get_Temp_Directory() + "cegui.log").c_str());
#else
  CEGUI::String log_dump_dir = "/dev/null";
#endif
  // create fake system and renderer
  pGuiSystem = &CEGUI::System::create(CEGUI::NullRenderer::create(), rp, NULL,
                                      NULL, NULL, "", log_dump_dir);
}
...
void cVideo ::Init_CEGUI(void) const {
  ...
  try {
    ...
    pGuiRenderer = &CEGUI::OpenGLRenderer::bootstrapSystem(
        CEGUI::Sizef(windowWidth, windowHeight));
  }
  ...
  CEGUI::MouseCursor::setInitialMousePosition(
      CEGUI::Vector2f(mouse_x, mouse_y));
  // add custom widgets
...
// again if CEGUI 0.8 works with std::string utf8
#ifdef _WIN32
    pGuiSystem = &CEGUI::System::create(
        *pGuiRenderer, rp, NULL, NULL, NULL, "",
        (const CEGUI::utf8 *)((pResource_Manager->user_data_dir + "cegui.log")
                                  .c_str()));
#else
...
void cVideo ::Init_CEGUI_Data(void) const {
  ...
  CEGUI::Window *window_root =
      CEGUI::WindowManager::getSingleton().loadLayoutFromFile("default.layout");
...
void cVideo ::Init_Video(bool reload_textures_from_file /* = 0 */,
                         bool use_preferences /* = 1 */) {
  Render_Finish();
  ...
  // Note: As of SDL 1.2.10, if width and height are both 0, SDL_SetVideoMode
  // will use the desktop resolution.
  screen = SDL_CreateWindow(CAPTION, SDL_WINDOWPOS_UNDEFINED,
  ...
  if (use_preferences && pPreferences->m_video_fullscreen) {
    bool is_fullscreen = ((SDL_GetWindowFlags(screen) &
  ...
  if (m_initialised) {
    ...
    pGuiSystem->notifyDisplaySizeChanged(CEGUI::Sizef(
  ...
  // finished first initialization
    ...
    if (m_opengl_version < 1.4f) {
      if (m_opengl_version >= 1.3f) {
        printf("Info : OpenGL Version %.1f is below the optimal version 1.4 "
               "and higher\n",
               m_opengl_version);
      } else {
...
void cVideo ::Init_OpenGL(void) {
  ...
  glViewport(0, 0, pPreferences->m_video_screen_w,
             pPreferences->m_video_screen_h);
  ...
  glOrtho(0, static_cast<float>(pPreferences->m_video_screen_w),
          static_cast<float>(pPreferences->m_video_screen_h), 0, -1, 1);
...
void cVideo ::Init_Geometry(void) {
  ...
  if (m_geometry_quality > 0.5f) {
    ...
    // Polygon - does not display correctly with open source ATi drivers
    // ( 18.2.2008 )
...
void cVideo ::Init_Resolution_Scale(void) const {
  // up scale
  global_upscalex = static_cast<float>(pPreferences->m_video_screen_w) /
                    static_cast<float>(game_res_w);
  global_upscaley = static_cast<float>(pPreferences->m_video_screen_h) /
                    static_cast<float>(game_res_h);
  // down scale
  global_downscalex = static_cast<float>(game_res_w) /
                      static_cast<float>(pPreferences->m_video_screen_w);
  global_downscaley = static_cast<float>(game_res_h) /
...
void cVideo ::Init_Image_Cache(bool recreate /* = 0 */,
  ...
  std::string imgcache_dir_active =
      m_imgcache_dir + "/" + int_to_string(pPreferences->m_video_screen_w) +
      "x" + int_to_string(pPreferences->m_video_screen_h);
  ...
  if (recreate || pPreferences->m_game_version != smc_version) {
    ...
    if (Dir_Exists(m_imgcache_dir)) {
      ...
      // could happen if a file is locked or we have no write rights
        ...
        if (draw_gui) {
          // caching failed
          Loading_Screen_Draw_Text(
              _("Caching Images failed : Could not remove old images"));
          SDL_Delay(2000);
        }
      }
    }
  ...
  if (draw_gui) {
    // get progress bar
    progress_bar = static_cast<CEGUI::ProgressBar *>(
        CEGUI::System::getSingleton()
            .getDefaultGUIContext()
            .getRootWindow()
            ->getChildRecursive("progress_bar"));
  ...
  for (vector<std::string>::iterator itr = image_files.begin();
    ...
    /* don't cache if no image settings or images without the width and height
     * set as there is currently no support to get the old and real image size
     * and thus the scaled down (cached) image size is used which is wrong
     */
    if (!settings || !settings->m_width || !settings->m_height) {
      if (settings) {
        debug_print("Info : %s has no image settings image size set and will "
                    "not get cached\n",
    ...
    unsigned char *image_downsampled =
        new unsigned char[new_width * new_height * image_bpp];
    bool downsampled =
        Downscale_Image(static_cast<unsigned char *>(sdl_surface->pixels),
                        sdl_surface->w, sdl_surface->h, image_bpp,
                        image_downsampled, reduce_block_x, reduce_block_y);
    ...
    if (downsampled) {
      ...
      // save image
      Save_Surface(imgcache_dir_active + "/" + cache_filename,
                   image_downsampled, new_width, new_height, image_bpp);
    }
    ...
    if (draw_gui) {
      ...
      progress_bar->setProgress(static_cast<float>(loaded_files) /
                                static_cast<float>(file_count));

#ifdef _DEBUG
      // update filename
      cGL_Surface *surface_filename =
          pFont->Render_Text(pFont->m_font_small, filename, white);
      // draw filename
...
int cVideo ::Test_Video(int width, int height, int bpp,
                        int flags /* = 0 */) const {
...
vector<cSize_Int>
cVideo ::Get_Supported_Resolutions(int flags /* = 0 */) const {
...
void cVideo ::Render_From_Thread(void) {
  ...
  // under linux with sofware mesa 7.9 it only showed the rendered output with
  // SDL_GL_SwapWindow(screen)
...
void cVideo ::Render(bool threaded /* = 0 */) {
  ...
  if (threaded) {
    ...
    if (!pRenderer->m_render_data.empty()) {
      pRenderer_current->m_render_data.insert(
          pRenderer_current->m_render_data.begin(),
          pRenderer->m_render_data.begin(), pRenderer->m_render_data.end());
...
cGL_Surface *cVideo ::Get_Surface(std::string filename,
                                  bool print_errors /* = 1 */) {
...
cVideo::cSoftware_Image cVideo ::Load_Image(std::string filename,
                                            bool load_settings /* = 1 */,
                                            bool print_errors /* = 1 */) const {
  ...
  if (load_settings) {
    ...
    if (File_Exists(settings_file)) {
      ...
      std::string img_filename_cache =
          m_imgcache_dir + "/" + settings_file.substr(strlen(DATA_DIR "/")) +
          ".png";
      ...
      // image given in base settings
        ...
        std::string img_filename =
            filename.substr(0, filename.rfind("/") + 1) + settings->m_base;
        ...
        if (!File_Exists(img_filename)) {
          ...
          // pixmaps dir must be given
          if (img_filename.find(DATA_DIR "/" GAME_PIXMAPS_DIR "/") ==
              std::string::npos) {
  ...
  if (!sdl_surface && File_Exists(filename) &&
      (!settings || settings->m_base.empty())) {
...
cGL_Surface *cVideo ::Load_GL_Surface(std::string filename,
                                      bool use_settings /* = 1 */,
                                      bool print_errors /* = 1 */) {
  // pixmaps dir must be given
  if (filename.find(DATA_DIR "/" GAME_PIXMAPS_DIR "/") == std::string::npos) {
  ...
  // load software image
  cSoftware_Image software_image =
      Load_Image(filename, use_settings, print_errors);
  SDL_Surface *sdl_surface = software_image.m_sdl_surface;
  cImage_Settings_Data *settings = software_image.m_settings;
  ...
  if (settings) {
    ...
    image = pVideo->Create_Texture(sdl_surface, settings->m_mipmap,
...
SDL_Surface *
cVideo ::Convert_To_Final_Software_Image(SDL_Surface *surface) const {
...
cGL_Surface *
cVideo ::Create_Texture(SDL_Surface *surface, bool mipmap /* = 0 */,
                        unsigned int force_width /* = 0 */,
                        unsigned int force_height /* = 0 */) const {
  if (!surface) {
    return NULL;
  }
  ...
  /* todo : Make this a render request because it forces an early thread render
   * finish as opengl commands are used directly. Reduces performance if the
   * render thread is on. It's usually called from the text rendering in
   * cTimeDisplay::Update.
   */
  ...
  if (texture_width != surface->w || texture_height != surface->h) {
    ...
    // create scaled image
    unsigned char *new_pixels = static_cast<unsigned char *>(
        SDL_malloc(texture_width * texture_height * 4));
    Downscale_Image(static_cast<unsigned char *>(surface->pixels), surface->w,
                    surface->h, surface->format->BytesPerPixel, new_pixels,
...
void cVideo ::Create_GL_Texture(unsigned int width, unsigned int height,
                                const void *pixels,
                                bool mipmap /* = 0 */) const {
...
void cVideo ::Clear_Screen(void) const { pRenderer->Add(new cClear_Request()); }

void cVideo ::Draw_Rect(const GL_rect *rect, float z, const Color *color,
                        cRect_Request *request /* = NULL */) const {
  if (!rect) {
    Draw_Rect(0, 0, static_cast<float>(game_res_w),
              static_cast<float>(game_res_h), z, color, request);
  } else {
    Draw_Rect(rect->m_x, rect->m_y, rect->m_w, rect->m_h, z, color, request);
  }
}

void cVideo ::Draw_Rect(float x, float y, float width, float height, float z,
                        const Color *color,
                        cRect_Request *request /* = NULL */) const {
...
void cVideo ::Draw_Gradient(const GL_rect *rect, float z, const Color *color_1,
                            const Color *color_2, ObjectDirection direction,
                            cGradient_Request *request /* = NULL */) const {
  if (!rect) {
    Draw_Gradient(0, 0, static_cast<float>(game_res_w),
                  static_cast<float>(game_res_h), z, color_1, color_2,
                  direction, request);
  } else {
    Draw_Gradient(rect->m_x, rect->m_y, rect->m_w, rect->m_h, z, color_1,
                  color_2, direction, request);
  }
}

void cVideo ::Draw_Gradient(float x, float y, float width, float height,
                            float z, const Color *color_1, const Color *color_2,
...
void cVideo ::Draw_Circle(float x, float y, float radius, float z,
                          const Color *color,
                          cCircle_Request *request /* = NULL */) const {
  if (!color || radius <= 0) {
    return;
  }
...
void cVideo ::Draw_Line(const GL_line *line, float z, const Color *color,
                        cLine_Request *request /* = NULL */) const {
  if (!line) {
    return;
  }
...
void cVideo ::Draw_Line(float x1, float y1, float x2, float y2, float z,
                        const Color *color,
                        cLine_Request *request /* = NULL */) const {
  if (!color) {
    return;
...
float cVideo ::Get_Scale(const cGL_Surface *image, float width, float height,
                         bool only_downscale /* = 1 */) const {
  if (!image) {
    return 0;
  }
 ...
 * MIT license
 */
bool cVideo ::Downscale_Image(const unsigned char *const orig, int width,
                              int height, int channels,
                              unsigned char *resampled, int block_size_x,
                              int block_size_y) const {
  // error check
  if (width <= 0 || height <= 0 || channels <= 0 || orig == NULL ||
      resampled == NULL || block_size_x <= 0 || block_size_y <= 0) {
    // invalid argument
    return 0;
  }
...
void cVideo ::Save_Screenshot(void) {
  ...
  for (unsigned int i = 1; i < 1000; i++) {
    filename = pResource_Manager->user_data_dir + USER_SCREENSHOT_DIR "/" +
               int_to_string(i) + ".png";

    if (!File_Exists(filename)) {
      // create image data
      GLubyte *data = new GLubyte[pPreferences->m_video_screen_w *
                                  pPreferences->m_video_screen_h * 3];
      // read opengl screen
      glReadPixels(0, 0, pPreferences->m_video_screen_w,
                   pPreferences->m_video_screen_h, GL_RGB, GL_UNSIGNED_BYTE,
                   static_cast<GLvoid *>(data));
      // save
      Save_Surface(filename, data, pPreferences->m_video_screen_w,
                   pPreferences->m_video_screen_h, 3, 1);
      // clear data
      delete[] data;

      // show info
      pHud_Debug->Set_Text("Screenshot " + int_to_string(i) + _(" saved"),
                           speedfactor_fps * 2.5f);
...
void cVideo ::Save_Surface(const std::string &filename,
                           const unsigned char *data, unsigned int width,
...
void Draw_Effect_Out(Effect_Fadeout effect /* = EFFECT_OUT_RANDOM */,
                     float speed /* = 1 */) {
  if (effect == EFFECT_OUT_RANDOM) {
    effect =
        static_cast<Effect_Fadeout>((rand() % (EFFECT_OUT_AMOUNT - 1)) + 1);
  }
  ...
  case EFFECT_OUT_HORIZONTAL_VERTICAL: {
    ...
    while (pos > pos_end * 0.5f) {
      ...
      if (hor) {
        ...
        // left
        pVideo->Draw_Gradient(0, 0, pos_end, static_cast<float>(game_res_h),
                              0.9f, &color, &black, DIR_HORIZONTAL,
                              gradient_request);
      }
      // vertical
      else {
        ...
        // top
        pVideo->Draw_Gradient(0, 0, static_cast<float>(game_res_w), pos_end,
                              0.9f, &color, &black, DIR_VERTICAL,
                              gradient_request);
      }
      ...
      if (hor) {
        // right
        pVideo->Draw_Gradient(static_cast<float>(game_res_w) - pos_end, 0,
                              pos_end, static_cast<float>(game_res_h), 0.9f,
                              &color, &black, DIR_HORIZONTAL, gradient_request);
      }
      // vertical
      else {
        // down
        pVideo->Draw_Gradient(0, static_cast<float>(game_res_h) - pos_end,
                              static_cast<float>(game_res_w), pos_end, 0.9f,
                              &color, &black, DIR_VERTICAL, gradient_request);
      }
  ...
  case EFFECT_OUT_BIG_ITEM: {
    ...
    while (f < 50) {
      ...
      color = Color(
          static_cast<Uint8>(255 - (f * 4)), 255 - static_cast<Uint8>(f * 4),
          255 - static_cast<Uint8>(f * 4), 200 - static_cast<Uint8>(f * 4));

      // ## item
      // create request
      cSurface_Request *request = new cSurface_Request();
      image->Blit((game_res_w * 0.5f) - ((image->m_w * f) / 2),
                  game_res_h * 0.5f - ((image->m_h * f) / 2), 0.9f, request);
  ...
  case EFFECT_OUT_RANDOM_COLOR_BOOST: {
    ...
    while (rect_size < 200.0f) {
      ...
      for (unsigned int g = 0; g < 50; g++) {
        ...
        cRect_Request *request = new cRect_Request();
        pVideo->Draw_Rect(
            Get_Random_Float(-rect_size * 0.5f, game_res_w - rect_size * 0.5f),
            Get_Random_Float(-rect_size * 0.5f, game_res_h - rect_size * 0.5f),
            rect_size, rect_size, 0.9f, &rand_color, request);
  ...
  case EFFECT_OUT_BLACK_TILED_RECTS: {
    ...
    GL_rect dest(0, 0, static_cast<float>(game_res_w) / tiles_num_hor,
                 static_cast<float>(game_res_h) / tiles_num_ver);
    Color color = black;
    ...
    while (grid[selected_tile_y][selected_tile_x] < 60.0f) {
      ...
      for (unsigned int x = 0; x < tiles_num_hor; x++) {
        for (unsigned int y = 0; y < tiles_num_ver; y++) {
          ...
          request->m_rect.m_x -=
              (dest.m_w * 0.5f) * (request->m_scale_x - 1.0f);
          request->m_rect.m_y -=
              (dest.m_h * 0.5f) * (request->m_scale_y - 1.0f);
  ...
  case EFFECT_OUT_FIXED_COLORBOX: {
    ...
    while (rect_size < 35) {
      ...
      // draw rects as a net
      // horizontal
      for (rect.m_x = pos_x; rect.m_x < game_res_w;
           rect.m_x += 20 + (rect_size * color_mod)) {
        // vertical
        for (rect.m_y = pos_y; rect.m_y < game_res_h;
             rect.m_y += 20 + (rect_size * color_mod)) {
      ...
      rect_color.green =
          static_cast<Uint8>(start_color.green * 0.1f * color_mod);
...
void Draw_Effect_In(Effect_Fadein effect /* = EFFECT_IN_RANDOM */,
                    float speed /* = 1 */) {
  // Clear render cache
  pRenderer->Clear(1);
...
void Loading_Screen_Init(void) {
  if (CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("loading") != nullptr) {
  ...
  // hide all windows
  for (unsigned int i = 0, gui_windows = guisheet->getChildCount();
       i < gui_windows; i++) {
    guisheet->getChildAtIdx(i)->hide();
  }

  // Create loading window
  CEGUI::Window *loading_window =
      CEGUI::WindowManager::getSingleton().loadLayoutFromFile("loading.layout");
...
void Loading_Screen_Draw_Text(const std::string &str_info /* = "Loading" */) {
  // set info text
  CEGUI::Window *text_default =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("text_loading"));
  if (!text_default) {
    printf("Warning: Loading Screen not initialized.");
    return;
  }
  text_default->setText(
      reinterpret_cast<const CEGUI::utf8 *>(str_info.c_str()));
...
void Loading_Screen_Exit(void) {
  CEGUI::Window *loading_window = CEGUI::System::getSingleton()
                                      .getDefaultGUIContext()
                                      .getRootWindow()
                                      ->getChildRecursive("loading");

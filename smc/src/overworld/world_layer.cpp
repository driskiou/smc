...
cLayer_Line_Point ::cLayer_Line_Point(cSprite_Manager *sprite_manager,
...
void cLayer_Line_Point ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  pVideo->Draw_Rect(m_col_rect.m_x - pActive_Camera->m_x,
                    ...
                    m_col_rect.m_h, m_pos_z, &m_color);
}
...
void cLayer_Line_Point ::Destroy(void) {
  ...
  if (m_type == TYPE_OW_LINE_START) {
    m_overworld->m_layer->Delete(static_cast<cLayer_Line_Point_Start *>(this),
                                 0);
  }
...
cLayer_Line_Point_Start ::cLayer_Line_Point_Start(
    cSprite_Manager *sprite_manager, cOverworld *overworld)
    : cLayer_Line_Point(sprite_manager, overworld, TYPE_OW_LINE_START) {
  cLayer_Line_Point_Start::Init();
}

cLayer_Line_Point_Start ::cLayer_Line_Point_Start(
    CEGUI::XMLAttributes &attributes, cSprite_Manager *sprite_manager,
    cOverworld *overworld)
    : cLayer_Line_Point(sprite_manager, overworld, TYPE_OW_LINE_START) {
  cLayer_Line_Point_Start::Init();
...
void cLayer_Line_Point_Start ::Init(void) {
  ...
  // create end point
  m_linked_point =
      new cLayer_Line_Point(m_sprite_manager, m_overworld, TYPE_OW_LINE_END);
...
cLayer_Line_Point_Start *cLayer_Line_Point_Start ::Copy(void) const {
  ...
  cLayer_Line_Point_Start *layer_line =
      new cLayer_Line_Point_Start(m_sprite_manager, pWorld_Editor->m_overworld);
  // start position
...
void cLayer_Line_Point_Start ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  // Start
  Set_Pos(static_cast<float>(attributes.getValueAsInteger("X1")) - 2,
          static_cast<float>(attributes.getValueAsInteger("Y1")) - 2, 1);
  // End
  m_linked_point->Set_Pos(
      static_cast<float>(attributes.getValueAsInteger("X2")) - 2,
      static_cast<float>(attributes.getValueAsInteger("Y2")) - 2, 1);
  // origin
  m_origin = attributes.getValueAsInteger("origin");
}

void cLayer_Line_Point_Start ::Set_Sprite_Manager(
...
void cLayer_Line_Point_Start ::Draw(cSurface_Request *request /* = NULL */) {
  // not a valid draw
  if (m_auto_destroy || m_linked_point->m_auto_destroy ||
      !pOverworld_Manager->m_draw_layer) {
    return;
  }
  ...
  // if active
  if (pOverworld_Player->m_current_line >= 0 &&
      m_overworld->m_layer->objects[pOverworld_Player->m_current_line] ==
          this) {
    color = lightblue;
  }

  pVideo->Draw_Line(Get_Line_Pos_X() - pActive_Camera->m_x,
                    Get_Line_Pos_Y() - pActive_Camera->m_y,
                    m_linked_point->Get_Line_Pos_X() - pActive_Camera->m_x,
                    m_linked_point->Get_Line_Pos_Y() - pActive_Camera->m_y,
                    0.085f, &color, line_request);
...
GL_line cLayer_Line_Point_Start ::Get_Line(void) const {
  return GL_line(
      m_pos_x + (m_col_rect.m_w * 0.5f), m_pos_y + (m_col_rect.m_h * 0.5f),
      m_linked_point->m_pos_x + (m_linked_point->m_col_rect.m_w * 0.5f),
      m_linked_point->m_pos_y + (m_linked_point->m_col_rect.m_h * 0.5f));
}
...
cWaypoint *cLayer_Line_Point_Start ::Get_End_Waypoint(void) const {
  ...
  if (wp_num < 0) {
    cLayer_Line_Point_Start *line_col =
        m_overworld->m_layer->Get_Line_Collision_Start(
            m_linked_point->m_col_rect);
...
void cLayer_Line_Point_Start ::Editor_Activate(void) {
  ...
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "layer_line_origin"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cLayer_Line_Point_Start::Editor_Origin_Text_Changed, this));

  // init
  Editor_Init();
}

bool cLayer_Line_Point_Start ::Editor_Origin_Text_Changed(
...
cLayer ::cLayer(cOverworld *origin) { m_overworld = origin; }

cLayer ::~cLayer(void) { Delete_All(); }

void cLayer ::Add(cLayer_Line_Point_Start *line_point) {
  for (LayerLineList::iterator itr = objects.begin(); itr != objects.end();
       ++itr) {
    // get pointer
  ...
  // check if in sprite manager
  if (m_overworld->m_sprite_manager->Get_Array_Num(
          line_point->m_linked_point) == -1) {
    // add end point
...
// again if CEGUI 0.8 works with std::string utf8
    ...
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, (const CEGUI::utf8 *)filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/World/Lines.xsd", "");
#else
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/World/Lines.xsd", "");
  ...
  // catch CEGUI Exceptions
    ...
    printf("Loading Line Layer %s CEGUI Exception %s\n", filename.c_str(),
           ex.getMessage().c_str());
    pHud_Debug->Set_Text(_("Line Layer Loading failed : ") +
...
// fixme : Check if there is a more portable way f.e. with imbue()
  ...
  for (LayerLineList::iterator itr = objects.begin(); itr != objects.end();
    ...
    // end
    Write_Property(stream, "X2",
                   static_cast<int>(line->m_linked_point->Get_Line_Pos_X()));
    Write_Property(stream, "Y2",
                   static_cast<int>(line->m_linked_point->Get_Line_Pos_Y()));
    // origin
...
cLayer_Line_Point_Start *
cLayer ::Get_Line_Collision_Start(const GL_rect &line_rect) {
  for (LayerLineList::iterator itr = objects.begin(); itr != objects.end();
       ++itr) {
    // get pointer
    cLayer_Line_Point_Start *layer_line = (*itr);
...
cLine_collision cLayer ::Get_Line_Collision_Direction(
    float x, float y, ObjectDirection dir, float dir_size /* = 10 */,
    unsigned int check_size /* = 10 */) const {
  if (dir == DIR_UP) {
    y -= dir_size;
...
cLine_collision cLayer ::Get_Nearest(float x, float y,
                                     ObjectDirection dir /* = DIR_HORIZONTAL */,
                                     unsigned int check_size /* = 15 */,
                                     int only_origin_id /* = -1 */) const {
  for (LayerLineList::const_iterator itr = objects.begin();
       itr != objects.end(); ++itr) {
    // get pointer
    cLayer_Line_Point_Start *layer_line = (*itr);
...
cLine_collision
cLayer ::Get_Nearest_Line(cLayer_Line_Point_Start *map_layer_line, float x,
                          float y, ObjectDirection dir /* = DIR_HORIZONTAL */,
                          unsigned int check_size /* = 15  */) const {
  ...
  for (float csize = 0; csize < check_size; csize++) {
    ...
    if (pOverworld_Manager->m_debug_mode && pOverworld_Manager->m_draw_layer) {
      ...
      cLine_Request *line_request = new cLine_Request();
      pVideo->Draw_Line(
          line_1.m_x1 - pActive_Camera->m_x, line_1.m_y1 - pActive_Camera->m_y,
          line_1.m_x2 - pActive_Camera->m_x, line_1.m_y2 - pActive_Camera->m_y,
          map_layer_line->m_pos_z + 0.001f, &white, line_request);
      ...
      pVideo->Draw_Line(
          line_2.m_x1 - pActive_Camera->m_x, line_2.m_y1 - pActive_Camera->m_y,
          line_2.m_x2 - pActive_Camera->m_x, line_2.m_y2 - pActive_Camera->m_y,
          map_layer_line->m_pos_z + 0.001f, &black, line_request);
      line_request->m_line_width = 2;
      line_request->m_render_count = 50;
      // add request
      pRenderer->Add(line_request);
...
void cLayer ::elementStart(const CEGUI::String &element,
    ...
    if (m_overworld->m_engine_version < 2) {
      if (m_xml_attributes.exists("Y1")) {
        m_xml_attributes.add(
            "Y1", CEGUI::PropertyHelper<float>::toString(
                      m_xml_attributes.getValueAsFloat("Y1") - 600.0f));
      }
      if (m_xml_attributes.exists("Y2")) {
        m_xml_attributes.add(
            "Y2", CEGUI::PropertyHelper<float>::toString(
                      m_xml_attributes.getValueAsFloat("Y2") - 600.0f));
      }
    }

    // add layer line
    Add(new cLayer_Line_Point_Start(
        m_xml_attributes, m_overworld->m_sprite_manager, m_overworld));

...
void cWaypoint ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  // position
  Set_Pos(static_cast<float>(attributes.getValueAsInteger("x")),
          static_cast<float>(attributes.getValueAsInteger("y")), 1);
  // image
  /*if( attributes.exists( "image" ) )
  {
          Set_Image( pVideo->Get_Surface( attributes.getValueAsString( "image"
  ).c_str() ), 1 ) ;
  }*/
  // type
  m_waypoint_type = static_cast<Waypoint_type>(
      attributes.getValueAsInteger("type", WAYPOINT_NORMAL));
  // destination
  // pre 0.99.6 : world
  ...
  // backward direction
  Set_Direction_Backward(Get_Direction_Id(
      attributes.getValueAsString("direction_backward", "left").c_str()));
  // forward direction
  Set_Direction_Forward(Get_Direction_Id(
      attributes.getValueAsString("direction_forward", "right").c_str()));
...
void cWaypoint ::Save_To_XML(CEGUI::XMLSerializer &stream) {
  ...
  // direction backward
  Write_Property(stream, "direction_backward",
                 Get_Direction_Name(m_direction_backward));
  // direction forward
  Write_Property(stream, "direction_forward",
                 Get_Direction_Name(m_direction_forward));
  // access
...
void cWaypoint ::Draw(cSurface_Request *request /* = NULL  */) {
  ...
  if (pOverworld_Manager->m_debug_mode || editor_world_enabled) {
    ...
    if (m_direction_backward == DIR_RIGHT || m_direction_backward == DIR_LEFT ||
        m_direction_backward == DIR_UP || m_direction_backward == DIR_DOWN) {
    ...
    if (m_direction_forward == DIR_RIGHT || m_direction_forward == DIR_LEFT ||
        m_direction_forward == DIR_UP || m_direction_forward == DIR_DOWN) {
      x = m_rect.m_x - pActive_Camera->m_x;
      y = m_rect.m_y - pActive_Camera->m_y;
  ...
  // draw waypoint
  if ((m_access && (m_waypoint_type == 1 || m_waypoint_type == 2)) ||
      pOverworld_Manager->m_debug_mode || editor_world_enabled) {
    ...
    if (!pOverworld_Manager->m_debug_mode) {
      request->m_color = Color(static_cast<Uint8>(255),
                               100 + static_cast<Uint8>(m_glim_color), 10);
    }
    // change to debug color
    else {
      if (m_access) {
        request->m_color = Color(static_cast<Uint8>(255),
                                 100 + static_cast<Uint8>(m_glim_color), 200);
      } else {
        request->m_color = Color(static_cast<Uint8>(20),
                                 100 + static_cast<Uint8>(m_glim_color), 10);
      }
    }
...
void cWaypoint ::Set_Destination(std::string str) {
  ...
  // world waypoint
           ...
           str.find(DATA_DIR "/" GAME_OVERWORLD_DIR "/") != std::string::npos) {
...
std::string cWaypoint ::Get_Destination(bool with_dir /* = 0 */,
                                        bool with_end /* = 0 */) const {
  ...
  if (m_waypoint_type == WAYPOINT_NORMAL) {
    ...
    if (with_dir &&
        name.find(DATA_DIR "/" GAME_OVERWORLD_DIR "/") == std::string::npos) {
...
void cWaypoint ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "waypoint_type"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cWaypoint::Editor_Type_Select, this));

  // destination
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "waypoint_destination"));
  Editor_Add(UTF8_("Destination"), UTF8_("Destination level or world"), editbox,
             150);

  editbox->setText(Get_Destination());
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cWaypoint::Editor_Destination_Text_Changed,
                               this));

  // backward direction
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "waypoint_backward_direction"));
  Editor_Add(UTF8_("Backward Direction"), UTF8_("Backward Direction"), combobox,
             100, 105);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cWaypoint::Editor_Backward_Direction_Select,
                               this));

  // forward direction
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "waypoint_forward_direction"));
  Editor_Add(UTF8_("Forward Direction"), UTF8_("Forward Direction"), combobox,
             100, 105);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cWaypoint::Editor_Forward_Direction_Select,
                               this));

  // Access
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "waypoint_access"));
  Editor_Add(UTF8_("Default Access"),
             UTF8_("Enable if the Waypoint should be always accessible."),
             combobox, 120, 80);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cWaypoint::Editor_Access_Select, this));
...
bool cWaypoint ::Editor_Destination_Text_Changed(
...
bool cWaypoint ::Editor_Backward_Direction_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =
...
bool cWaypoint ::Editor_Forward_Direction_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =

...
cGee ::cGee(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cGee::Init();
}
...
void cGee ::Update(void) {
  ...
  // moving
    ...
    if ((!m_always_fly && m_fly_distance_counter > m_fly_distance) ||
        Is_At_Max_Distance()) {
      Stop();
    }
    ...
    if (m_start_direction == DIR_HORIZONTAL) {
      pVideo->Draw_Rect(m_start_pos_x - pActive_Camera->m_x - m_max_distance,
                        m_start_pos_y + (m_rect.m_h * 0.5f) - 5.0f -
                            pActive_Camera->m_y,
                        m_col_rect.m_w + (m_max_distance * 2.0f), 10.0f,
                        m_pos_z - 0.000001f, &whitealpha128);
      ...
      pVideo->Draw_Rect(m_start_pos_x + (m_rect.m_w * 0.5f) - 5.0f -
                        ...
                        10.0f, m_col_rect.m_h + (m_max_distance * 2.0f),
                        m_pos_z - 0.000001f, &whitealpha128);
    }
  }
...
void cGee ::Stop(void) { Set_Moving_State(STA_STAY); }
...
void cGee ::Generate_Particles(unsigned int amount /* = 4 */) const {
  ...
  anim->Set_Emitter_Rect(m_col_rect.m_x + (m_col_rect.m_w * 0.3f),
                         m_col_rect.m_y + (m_col_rect.m_h * 0.2f),
                         m_col_rect.m_w * 0.4f, m_col_rect.m_h * 0.3f);
...
void cGee ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_gee_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cGee::Editor_Direction_Select, this));

  // max distance
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_gee_max_distance"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cGee::Editor_Max_Distance_Text_Changed, this));

  // always fly
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_gee_always_fly"));
  Editor_Add(UTF8_("Always fly"),
             UTF8_("Move without stopping at the fly distance"), combobox, 120,
             80);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cGee::Editor_Always_Fly_Select, this));

  // wait time
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_gee_wait_time"));
  Editor_Add(UTF8_("Wait time"),
             UTF8_("Time to wait until moving again after a stop"), editbox,
             90);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cGee::Editor_Wait_Time_Text_Changed, this));

  // fly distance
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_gee_fly_distance"));
  Editor_Add(UTF8_("Fly distance"), UTF8_("The distance to move each time"),
             editbox, 90);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cGee::Editor_Fly_Distance_Text_Changed, this));

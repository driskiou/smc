...
bool cLevel ::New(std::string filename) {
  ...
  if (filename.empty()) {
    ...
    while (1) {
      // set name
      filename = pResource_Manager->user_data_dir + USER_LEVEL_DIR + "/new_" +
                 int_to_string(i) + ".smclvl";
    ...
    // set user directory
    if (filename.find(pResource_Manager->user_data_dir + USER_LEVEL_DIR +
                      "/") == std::string::npos) {
      filename.insert(0,
                      pResource_Manager->user_data_dir + USER_LEVEL_DIR + "/");
    }
  }
...
const CEGUI::String &cLevel ::getDefaultResourceGroup() const {
...
bool cLevel ::Load(std::string filename) {
  ...
  if (filename.rfind(".smclvl") != std::string::npos) {
    try {
// fixme : Workaround for std::string to CEGUI::String utf8 conversion. Check
// again if CEGUI 0.8 works with std::string utf8
#ifdef _WIN32
      CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
          *this, (const CEGUI::utf8 *)filename.c_str(),
          DATA_DIR "/" GAME_SCHEMA_DIR "/Level.xsd", "");
#else
      CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
          *this, filename.c_str(), DATA_DIR "/" GAME_SCHEMA_DIR "/Level.xsd",
          "");
#endif
    }
    // catch CEGUI Exceptions
      ...
      printf("Loading Level %s CEGUI Exception %s\n", filename.c_str(),
             ex.getMessage().c_str());
      pHud_Debug->Set_Text(_("Loading Level failed : ") +
                           (const std::string)ex.getMessage().c_str());
      return 0;
    }
  }
  // old level format
  else {
    pHud_Debug->Set_Text(_("Unsupported Level format : ") +
                         (const std::string)filename.c_str());
    return 0;
  }
...
void cLevel ::Save(void) {
  ...
  // use user level dir
  if (m_level_filename.find(pResource_Manager->user_data_dir + USER_LEVEL_DIR +
                            "/") == std::string::npos) {
    // erase old directory
    m_level_filename = Trim_Filename(m_level_filename, 0, 1);
    // set user directory
    m_level_filename.insert(0, pResource_Manager->user_data_dir +
                                   USER_LEVEL_DIR + "/");
  }
...
// fixme : Check if there is a more portable way f.e. with imbue()
  ...
  if (!file) {
    printf(
        "Error : Couldn't open level file for saving. Is the file read-only ?");
    pHud_Debug->Set_Text(_("Couldn't save level ") + m_level_filename,
                         speedfactor_fps * 5.0f);
    return;
  }
  ...
  // game version
  Write_Property(stream, "game_version",
                 int_to_string(SMC_VERSION_MAJOR) + "." +
                     int_to_string(SMC_VERSION_MINOR) + "." +
                     int_to_string(SMC_VERSION_PATCH));
  // engine version
  ...
  // land type
  Write_Property(stream, "lvl_land_type",
                 Get_Level_Land_Type_Name(m_land_type));
  // camera limits
  ...
  // backgrounds
  for (vector<cBackground *>::iterator itr =
           m_background_manager->objects.begin();
       itr != m_background_manager->objects.end(); ++itr) {
  ...
  // position
  Write_Property(stream, "posx",
                 static_cast<int>(pLevel_Player->m_start_pos_x));
  Write_Property(stream, "posy",
                 static_cast<int>(pLevel_Player->m_start_pos_y));
  // direction
  Write_Property(stream, "direction",
                 Get_Direction_Name(pLevel_Player->m_start_direction));
  // end player
  stream.closeTag();
  ...
  file.close();
  pHud_Debug->Set_Text(_("Level ") + Trim_Filename(m_level_filename, 0, 0) +
                       _(" saved"));
}
...
void cLevel ::Draw_Layer_2(LevelDrawType type /* = LVL_DRAW */) {
  ...
  if (pLevel_Player->m_maryo_type == MARYO_GHOST) {
    ...
    if (pLevel_Player->m_ghost_time > 220) {
      color.alpha = static_cast<Uint8>(
          color.alpha * ((-pLevel_Player->m_ghost_time + 320) * 0.01f));
    }
    // fade alpha out
    else if (pLevel_Player->m_ghost_time < 100) {
      color.alpha = static_cast<Uint8>(color.alpha *
                                       (pLevel_Player->m_ghost_time * 0.01f));
    }

    pVideo->Draw_Rect(0, 0, static_cast<float>(game_res_w),
                      static_cast<float>(game_res_h), 0.12f, &color, request);
...
bool cLevel ::Key_Down(const SDL_Keycode key) {
  ...
  // special key F3
    ...
    anim->Set_Emitter_Rect(
        pLevel_Player->m_pos_x +
            static_cast<float>(pLevel_Player->m_col_rect.m_w / 2),
        pLevel_Player->m_pos_y - 100, 10, 10);
  ...
  // God Mode
  else if (pKeyboard->m_keys[SDLK_g] && pKeyboard->m_keys[SDLK_o] &&
           pKeyboard->m_keys[SDLK_d] && !editor_enabled) {
  ...
  // Set Small state
  else if (pKeyboard->m_keys[SDLK_k] && pKeyboard->m_keys[SDLK_i] &&
           pKeyboard->m_keys[SDLK_d] && !editor_enabled) {
...
std::string cLevel ::Get_Music_Filename(int with_dir /* = 2 */,
                                        bool with_end /* = 1 */) const {
  ...
  else if (with_dir == 1 && filename.find(DATA_DIR "/" GAME_MUSIC_DIR "/") !=
                                std::string::npos) {
...
void cLevel ::Set_Author(const std::string &name) { m_author = name; }
...
void cLevel ::elementStart(const CEGUI::String &element,
                           const CEGUI::XMLAttributes &attributes) {
  if (element == "property" || element == "Property") {
    m_xml_attributes.add(attributes.getValueAsString("name"),
                         attributes.getValueAsString("value"));
  }
}
...
void cLevel ::elementEnd(const CEGUI::String &element) {
  ...
  if (element == "information") {
    ...
    float engine_version_float =
        m_xml_attributes.getValueAsFloat("engine_version");
    ...
    m_last_saved =
        string_to_int64(m_xml_attributes.getValueAsString("save_time").c_str());
  } else if (element == "settings") {
    // if V.1.9 and lower : move y coordinate bottom to 0
    if (m_engine_version < 35) {
      if (m_xml_attributes.exists("cam_limit_h")) {
        m_xml_attributes.add(
            "cam_limit_h",
            CEGUI::PropertyHelper<float>::toString(
                m_xml_attributes.getValueAsFloat("cam_limit_h") - 600.0f));
      }
    }

    Set_Author(xml_string_to_string(
        m_xml_attributes.getValueAsString("lvl_author").c_str()));
    Set_Version(xml_string_to_string(
        m_xml_attributes.getValueAsString("lvl_version").c_str()));
    Set_Music(xml_string_to_string(
        m_xml_attributes.getValueAsString("lvl_music").c_str()));
    Set_Description(xml_string_to_string(
        m_xml_attributes.getValueAsString("lvl_description").c_str()));
    Set_Difficulty(m_xml_attributes.getValueAsInteger("lvl_difficulty"));
    Set_Land_Type(Get_Level_Land_Type_Id(
        m_xml_attributes.getValueAsString("lvl_land_type", "undefined")
            .c_str()));
    m_camera_limits = GL_rect(
        static_cast<float>(m_xml_attributes.getValueAsInteger("cam_limit_x")),
        static_cast<float>(m_xml_attributes.getValueAsInteger("cam_limit_y")),
        static_cast<float>(m_xml_attributes.getValueAsInteger("cam_limit_w")),
        static_cast<float>(m_xml_attributes.getValueAsInteger("cam_limit_h")));
    m_fixed_camera_hor_vel =
        m_xml_attributes.getValueAsFloat("cam_fixed_hor_vel");
  } else if (element == "background") {
    BackgroundType bg_type =
        static_cast<BackgroundType>(m_xml_attributes.getValueAsInteger("type"));
    ...
    // default background
      ...
      m_background_manager->Add(
          new cBackground(m_xml_attributes, m_sprite_manager));
    }
  } else if (element == "player") {
    // position
    m_player_start_pos_x =
        static_cast<float>(m_xml_attributes.getValueAsInteger(
            "posx", static_cast<int>(cLevel_Player::m_default_pos_x)));
    m_player_start_pos_y =
        static_cast<float>(m_xml_attributes.getValueAsInteger(
            "posy", static_cast<int>(cLevel_Player::m_default_pos_y)));
    ...
    // direction
    m_player_start_direction = Get_Direction_Id(
        m_xml_attributes.getValueAsString("direction").c_str());

    // if invalid set default
    if (m_player_start_direction != DIR_LEFT &&
        m_player_start_direction != DIR_RIGHT) {
      m_player_start_direction = DIR_RIGHT;
    }
  } else if (Is_Level_Object_Element(element)) {
    // create sprite
    cSprite *object = Create_Level_Object_From_XML(
        element, m_xml_attributes, m_engine_version, m_sprite_manager);
...
cSprite *Create_Level_Object_From_XML(const CEGUI::String &xml_element,
                                      CEGUI::XMLAttributes &attributes,
                                      int engine_version,
  ...
  if (element == "sprite") {
    ...
    if (engine_version < 25) {
      ...
      Relocate_Image(attributes, "game/box/stone8.png",
                     "blocks/metal/stone_2_violet.png");
      // move jungle_1 trees into a directory
      Relocate_Image(attributes, "ground/jungle_1/tree_type_1.png",
                     "ground/jungle_1/tree/1.png");
      Relocate_Image(attributes, "ground/jungle_1/tree_type_1_front.png",
                     "ground/jungle_1/tree/1_front.png");
      Relocate_Image(attributes, "ground/jungle_1/tree_type_2.png",
                     "ground/jungle_1/tree/2.png");
      // move yoshi_1 extra to jungle_2 hedge
      Relocate_Image(attributes, "ground/yoshi_1/extra_1_blue.png",
                     "ground/jungle_2/hedge/1_blue.png");
      Relocate_Image(attributes, "ground/yoshi_1/extra_1_green.png",
                     "ground/jungle_2/hedge/1_green.png");
      Relocate_Image(attributes, "ground/yoshi_1/extra_1_red.png",
                     "ground/jungle_2/hedge/1_red.png");
      Relocate_Image(attributes, "ground/yoshi_1/extra_1_yellow.png",
                     "ground/jungle_2/hedge/1_yellow.png");
      // move yoshi_1 rope to jungle_2
      Relocate_Image(attributes, "ground/yoshi_1/rope_1_leftright.png",
                     "ground/jungle_2/rope_1_hor.png");
    }
    // if V.1.5 and lower : change pipe connection image paths
    if (engine_version < 28) {
      Relocate_Image(attributes, "blocks/pipe/connection_left_down.png",
                     "blocks/pipe/connection/plastic_1/orange/right_up.png");
      Relocate_Image(attributes, "blocks/pipe/connection_left_up.png",
                     "blocks/pipe/connection/plastic_1/orange/right_down.png");
      Relocate_Image(attributes, "blocks/pipe/connection_right_down.png",
                     "blocks/pipe/connection/plastic_1/orange/left_up.png");
      Relocate_Image(attributes, "blocks/pipe/connection_right_up.png",
                     "blocks/pipe/connection/plastic_1/orange/left_down.png");
      Relocate_Image(attributes, "blocks/pipe/metal_connector.png",
                     "blocks/pipe/connection/metal_1/grey/middle.png");
    }
    // if V.1.7 and lower : change yoshi_1 hill_up to jungle_1 slider image
    // paths
    if (engine_version < 31) {
      Relocate_Image(attributes, "ground/yoshi_1/hill_up_1.png",
                     "ground/jungle_1/slider/2_green_left.png");
      Relocate_Image(attributes, "ground/yoshi_1/hill_up_2.png",
                     "ground/jungle_1/slider/2_blue_left.png");
      Relocate_Image(attributes, "ground/yoshi_1/hill_up_3.png",
                     "ground/jungle_1/slider/2_brown_left.png");
    }
    // if V.1.7 and lower : change slider grey_1 to green_1 brown slider image
    // paths
    if (engine_version < 32) {
      Relocate_Image(attributes, "slider/grey_1/slider_left.png",
                     "ground/green_1/slider/1/brown/left.png");
      Relocate_Image(attributes, "slider/grey_1/slider_middle.png",
                     "ground/green_1/slider/1/brown/middle.png");
      Relocate_Image(attributes, "slider/grey_1/slider_right.png",
                     "ground/green_1/slider/1/brown/right.png");
    }
    // if V.1.7 and lower : change green_1 ground to green_3 ground image paths
    if (engine_version < 34) {
      // normal
      Relocate_Image(attributes, "ground/green_1/ground/left_up.png",
                     "ground/green_3/ground/top/left.png");
      Relocate_Image(attributes, "ground/green_1/ground/left_down.png",
                     "ground/green_3/ground/bottom/left.png");
      Relocate_Image(attributes, "ground/green_1/ground/right_up.png",
                     "ground/green_3/ground/top/right.png");
      Relocate_Image(attributes, "ground/green_1/ground/right_down.png",
                     "ground/green_3/ground/bottom/right.png");
      Relocate_Image(attributes, "ground/green_1/ground/up.png",
                     "ground/green_3/ground/top/1.png");
      Relocate_Image(attributes, "ground/green_1/ground/down.png",
                     "ground/green_3/ground/bottom/1.png");
      Relocate_Image(attributes, "ground/green_1/ground/right.png",
                     "ground/green_3/ground/middle/right.png");
      Relocate_Image(attributes, "ground/green_1/ground/left.png",
                     "ground/green_3/ground/middle/left.png");
      Relocate_Image(attributes, "ground/green_1/ground/middle.png",
                     "ground/green_3/ground/middle/1.png");

      // hill (not available)
      // Relocate_Image( attributes, "ground/green_1/ground/hill_left_up.png",
      // "ground/green_3/ground/" ); Relocate_Image( attributes,
      // "ground/green_1/ground/hill_right_up.png", "ground/green_3/ground/" );
      // Relocate_Image( attributes, "ground/green_1/ground/hill_right.png",
      // "ground/green_3/ground/" ); Relocate_Image( attributes,
      // "ground/green_1/ground/hill_left.png", "ground/green_3/ground/" );
    }
    // if V.1.9 and lower : move y coordinate bottom to 0
    if (engine_version < 35) {
      if (attributes.exists("posy")) {
        attributes.add("posy",
                       CEGUI::PropertyHelper<float>::toString(
                           attributes.getValueAsFloat("posy") - 600.0f));
      }
    }
    // if V.1.9 and lower : change fire_1 animation to particles
    if (engine_version < 37) {
      Relocate_Image(attributes, "animation/fire_1/1.png",
                     "animation/particles/fire_1.png");
      Relocate_Image(attributes, "animation/fire_1/2.png",
                     "animation/particles/fire_2.png");
      Relocate_Image(attributes, "animation/fire_1/3.png",
                     "animation/particles/fire_3.png");
      Relocate_Image(attributes, "animation/fire_1/4.png",
                     "animation/particles/fire_4.png");
    }
    // always : fix sprite with undefined massive-type
    if (attributes.exists("type") &&
        attributes.getValueAsString("type") == "undefined") {
      // change to passive
    ...
    if (sprite->m_image) {
      ...
      if (engine_version < 22) {
        if (sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/green/up.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/green/ver.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/green/down.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/blue/up.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/blue/ver.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/blue/down.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/yellow/up.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/yellow/ver.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/yellow/down.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/grey/up.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/grey/ver.png") == 0 ||
            sprite->m_image->m_filename.compare(DATA_DIR
                                                "/" GAME_PIXMAPS_DIR "/"
                                                "pipes/grey/down.png") == 0) {
          sprite->Move(-6, 0, 1);
          sprite->m_start_pos_x = sprite->m_pos_x;
        } else if (sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/green/right.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/green/hor.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/green/left.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/blue/right.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/blue/hor.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/blue/left.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/yellow/right.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/yellow/hor.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/yellow/left.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/grey/right.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/grey/hor.png") == 0 ||
                   sprite->m_image->m_filename.compare(
                       DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                                "pipes/grey/left.png") == 0) {
          sprite->Move(0, -6, 1);
      ...
      if (engine_version < 23) {
        if (sprite->m_image->m_filename.compare(
                DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                         "hills/green_1/head.png") == 0 ||
            sprite->m_image->m_filename.compare(
                DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                         "hills/light_blue_1/head.png") == 0) {
          sprite->Move(0, -6, 1);
          sprite->m_start_pos_y = sprite->m_pos_y;
        }
      }
      ...
      // paths
      if (engine_version < 31) {
        // image filename is already changed but we need to add the middle and
        // right tiles
        if (sprite_manager &&
            (sprite->m_image->m_filename.compare(
                 DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                          "ground/jungle_1/slider/2_green_left.png") == 0 ||
             sprite->m_image->m_filename.compare(
                 DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                          "ground/jungle_1/slider/2_blue_left.png") == 0 ||
             sprite->m_image->m_filename.compare(
                 DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                          "ground/jungle_1/slider/2_brown_left.png") == 0)) {
          std::string color;
          ...
          for (unsigned int i = 0; i < 4; i++) {
            copy = copy->Copy();
            copy->Set_Image(pVideo->Get_Surface("ground/jungle_1/slider/2_" +
                                                color + "_middle.png"),
                            1);
      ...
      // paths
      if (engine_version < 32) {
        // image filename is already changed but we need to add an additional
        // middle tile for left and right
        if (sprite_manager &&
            (sprite->m_image->m_filename.compare(
                 DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                          "ground/green_1/slider/1/brown/left.png") == 0 ||
             sprite->m_image->m_filename.compare(
                 DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                          "ground/green_1/slider/1/brown/right.png") == 0)) {
          // add middle tile
          cSprite *copy = sprite->Copy();
          copy->Set_Image(
              pVideo->Get_Surface("ground/green_1/slider/1/brown/middle.png"),
              1);
    ...
    if (engine_version < 36) {
      if (attributes.exists("motion")) {
        attributes.add("camera_motion",
                       CEGUI::PropertyHelper<int>::toString(
                           attributes.getValueAsInteger("motion") + 1));
  ...
  // powerup is pre V.0.99.5
    ...
    cMoving_Platform *moving_platform =
        new cMoving_Platform(attributes, sprite_manager);

    // if V.1.7 and lower : change new slider middle count because start and end
    // image is now half the width
    if (engine_version < 32) {
      if (moving_platform->m_images[0].m_image->m_filename.compare(
              DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                       "ground/green_1/slider/1/brown/left.png") == 0) {
  ...
  // falling platform is pre V.1.5
    ...
    if (attributes.exists("time_fall")) {
      attributes.add("touch_time", CEGUI::PropertyHelper<float>::toString(
                                       attributes.getValueAsFloat("time_fall") *
                                       speedfactor_fps));
    ...
    if (engine_version < 26) {
      ...
      if (str_type == "gumba") {
        ...
        if (attributes.exists("color") &&
            attributes.getValueAsString("color") == "red") {
    ...
    if (engine_version < 29) {
      ...
      if (str_type == "jpiranha") {
        ...
        if (attributes.exists("image_dir")) {
          std::string img_dir =
              attributes.getValueAsString("image_dir").c_str();
    ...
    if (str_type == "eato") {
      ...
      if (engine_version < 27) {
        if (attributes.exists("max_downgrade_time")) {
          attributes.add("shell_time",
                         attributes.getValueAsString("max_downgrade_time"));
    ...
    if (engine_version < 35) {
      if (attributes.exists("pos_y")) {
        attributes.add("pos_y",
                       CEGUI::PropertyHelper<float>::toString(
                           attributes.getValueAsFloat("pos_y") - 600.0f));
      }
    }
    ...
    if (engine_version < 37) {
      Relocate_Image(attributes, "animation/fire_1/1.png",
                     "animation/particles/fire_1.png", "file");
      Relocate_Image(attributes, "animation/fire_1/2.png",
                     "animation/particles/fire_2.png", "file");
      Relocate_Image(attributes, "animation/fire_1/3.png",
                     "animation/particles/fire_3.png", "file");
      Relocate_Image(attributes, "animation/fire_1/4.png",
                     "animation/particles/fire_4.png", "file");
    }
    ...
    cParticle_Emitter *particle_emitter =
        new cParticle_Emitter(attributes, sprite_manager);
    // set to not spawned
  ...
  // if V.1.9 and lower : convert global effect to an advanced particle emitter
    ...
    if (engine_version < 21) {
      if (!attributes.exists("time_to_live")) {
        attributes.add(
            "time_to_live",
            CEGUI::PropertyHelper<float>::toString(
                attributes.getValueAsFloat("lifetime_mod", 20) * 0.3f));
        attributes.remove("lifetime_mod");
      }
    }

    // if V.0.99.7 and lower : change creation speed to emitter iteration
    // interval
    if (engine_version < 22) {
      if (!attributes.exists("emitter_iteration_interval")) {
        attributes.add(
            "emitter_iteration_interval",
            CEGUI::PropertyHelper<float>::toString(
                (1.0f / attributes.getValueAsFloat("creation_speed", 0.3f)) *
                0.032f));
    ...
    if (engine_version < 37) {
      Relocate_Image(attributes, "animation/fire_1/1.png",
                     "animation/particles/fire_1.png", "image");
      Relocate_Image(attributes, "animation/fire_1/2.png",
                     "animation/particles/fire_2.png", "image");
      Relocate_Image(attributes, "animation/fire_1/3.png",
                     "animation/particles/fire_3.png", "image");
      Relocate_Image(attributes, "animation/fire_1/4.png",
                     "animation/particles/fire_4.png", "image");
    }
    ...
    // rename attributes
    attributes.add("pos_x", CEGUI::PropertyHelper<int>::toString(
                                attributes.getValueAsInteger("rect_x", 0)));
    attributes.add("pos_y",
                   CEGUI::PropertyHelper<int>::toString(
                       attributes.getValueAsInteger("rect_y", 0) - 600));
    attributes.add("size_x",
                   CEGUI::PropertyHelper<int>::toString(
                       attributes.getValueAsInteger("rect_w", game_res_w)));
    attributes.add("size_y", CEGUI::PropertyHelper<int>::toString(
                                 attributes.getValueAsInteger("rect_h", 0)));
    attributes.add("emitter_time_to_live",
                   CEGUI::PropertyHelper<float>::toString(-1.0f));
    attributes.add("pos_z", CEGUI::PropertyHelper<float>::toString(
                                attributes.getValueAsFloat("z", 0.12f)));
    attributes.add("pos_z_rand",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("z_rand", 0.0f)));
    if (!attributes.exists("time_to_live")) {
      attributes.add("time_to_live",
                     CEGUI::PropertyHelper<float>::toString(7.0f));
    }
    attributes.add(
        "emitter_interval",
        CEGUI::PropertyHelper<float>::toString(
            attributes.getValueAsFloat("emitter_iteration_interval", 0.3f)));
    attributes.add("size_scale",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("scale", 0.2f)));
    attributes.add("size_scale_rand",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("scale_rand", 0.2f)));
    attributes.add("vel", CEGUI::PropertyHelper<float>::toString(
                              attributes.getValueAsFloat("speed", 2.0f)));
    attributes.add("vel_rand",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("speed_rand", 8.0f)));
    attributes.add("angle_start",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("dir_range_start", 0.0f)));
    attributes.add("angle_range",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("dir_range_size", 90.0f)));
    attributes.add("const_rot_z",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("const_rotz", -5.0f)));
    attributes.add("const_rot_z_rand",
                   CEGUI::PropertyHelper<float>::toString(
                       attributes.getValueAsFloat("const_rotz_rand", 10.0f)));

    cParticle_Emitter *emitter =
        new cParticle_Emitter(attributes, sprite_manager);
    emitter->Set_Spawned(0);

    // clip to the camera
    emitter->Set_Clip_Rect(
        GL_rect(0.0f, 0.0f, static_cast<float>(game_res_w),
                static_cast<float>(game_res_h) +
                    (attributes.getValueAsInteger("rect_y", 0) * -1)));

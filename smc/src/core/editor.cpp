...
namespace SMC
{
  ...
  cEditor_CEGUI_Texture ::cEditor_CEGUI_Texture(CEGUI::OpenGLRenderer &owner,
                                                GLuint tex,
                                                const CEGUI::Sizef &size)
      : CEGUI::OpenGLTexture(dynamic_cast<CEGUI::OpenGLRendererBase &>(owner),
                             CEGUI::String("default"), tex, size) {}

  cEditor_CEGUI_Texture ::cEditor_CEGUI_Texture(CEGUI::OpenGLRenderer &owner, const CEGUI::String &name,
                                                const CEGUI::String &filename, const CEGUI::String &resourceGroup)
      : CEGUI::OpenGLTexture(dynamic_cast<CEGUI::OpenGLRendererBase &>(owner),
                             CEGUI::String("default"), filename, resourceGroup) {}
  ...
  cEditor_Item_Object ::~cEditor_Item_Object(void)
  {
    if (m_image)
    {
      ...
      CEGUI::ImageManager::getSingleton()
          .destroy(m_image->getName());
      m_image = nullptr;
    }
  ...
  void cEditor_Item_Object ::Init(cSprite *sprite)
  {
    ...
    const auto imageset_name =
        "editor_item " + m_text + " " +
        CEGUI::PropertyHelper<unsigned int>::toString(m_parent->getItemCount());

    // create CEGUI link
    cEditor_CEGUI_Texture *texture = new cEditor_CEGUI_Texture(
        *pGuiRenderer, sprite_obj->m_start_image->m_image,
        CEGUI::Sizef(sprite_obj->m_start_image->m_tex_w,
                     sprite_obj->m_start_image->m_tex_h));

    m_image = dynamic_cast<CEGUI::BasicImage *>(
        &CEGUI::ImageManager::getSingleton().create("BasicImage", imageset_name));
  ...
  CEGUI::Sizef cEditor_Item_Object ::getPixelSize(void) const
  {
    ...
    {
      tmp.d_height +=
          (pPreferences->m_editor_item_image_size + 10) * global_upscaley;
    }

    return tmp;
  }

  void cEditor_Item_Object ::draw(CEGUI::GeometryBuffer &buffer,
                                  const CEGUI::Rectf &targetRect, float alpha,
                                  const CEGUI::Rectf *clipper) const
  {
    // image
    if (m_image && pPreferences->m_editor_show_item_images)
    {
      const auto rec = CEGUI::Rectf(targetRect.left() + 15, targetRect.top() + 22,
                                    targetRect.left() + 15 +
                                        (sprite_obj->m_start_image->m_start_w *
                                         preview_scale * global_upscalex),
                                    targetRect.top() + 22 +
                                        (sprite_obj->m_start_image->m_start_h *
                                         preview_scale * global_upscaley));
      m_image->render(buffer, rec, clipper,
                      CEGUI::ColourRect(CEGUI::Colour(1.0f, 1.0f, 1.0f, alpha)));
    }
    CEGUI::FontManager::getSingleton().getIterator().getCurrentValue()->drawText(
        buffer, m_text, CEGUI::Vector2f(targetRect.left(), targetRect.top()),
        clipper,
        CEGUI::ColourRect(Get_Massive_Type_Color(sprite_obj->m_massive_type)
                              .Get_cegui_Color()));
  }
  ...
  void cEditor::Init(void)
  {
    ...
    m_editor_window =
        CEGUI::WindowManager::getSingleton().loadLayoutFromFile("editor.layout");
    CEGUI::System::getSingleton()
        .getDefaultGUIContext()
        .getRootWindow()
        ->addChild(m_editor_window);

    // Get TabControl
    m_tabcontrol_menu = static_cast<CEGUI::TabControl *>(
        CEGUI::System::getSingleton()
            .getDefaultGUIContext()
            .getRootWindow()
            ->getChildRecursive("tabcontrol_editor"));
    // fixme : CEGUI does not detect the mouse enter event if in the listbox or
    // any other window in it TabControl Menu Tab Events
    m_tabcontrol_menu->getTabContents("editor_tab_menu")
        ->subscribeEvent(
            CEGUI::Window::EventMouseEntersArea,
            CEGUI::Event::Subscriber(&cEditor::Editor_Mouse_Enter, this));
    // TabControl Items Tab Events
    m_tabcontrol_menu->getTabContents("editor_tab_items")
        ->subscribeEvent(
            CEGUI::Window::EventMouseEntersArea,
            CEGUI::Event::Subscriber(&cEditor::Editor_Mouse_Enter, this));
    ...
    m_listbox_menu =
        static_cast<CEGUI::Listbox *>(CEGUI::System::getSingleton()
                                          .getDefaultGUIContext()
                                          .getRootWindow()
                                          ->getChildRecursive("editor_menu"));
    // Menu Listbox events
    m_listbox_menu->subscribeEvent(
        CEGUI::Window::EventMouseEntersArea,
        CEGUI::Event::Subscriber(&cEditor::Editor_Mouse_Enter, this));
    m_listbox_menu->subscribeEvent(
        CEGUI::Listbox::EventSelectionChanged,
        CEGUI::Event::Subscriber(&cEditor::Menu_Select, this));
    // Get Items Listbox
    m_listbox_items =
        static_cast<CEGUI::Listbox *>(CEGUI::System::getSingleton()
                                          .getDefaultGUIContext()
                                          .getRootWindow()
                                          ->getChildRecursive("editor_items"));
    // Items Listbox events
    m_listbox_items->subscribeEvent(
        CEGUI::Window::EventMouseEntersArea,
        CEGUI::Event::Subscriber(&cEditor::Editor_Mouse_Enter, this));
    m_listbox_items->subscribeEvent(
        CEGUI::Listbox::EventSelectionChanged,
        CEGUI::Event::Subscriber(&cEditor::Item_Select, this));
    ...
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, m_items_filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/Editor_Items.xsd", "");
    ...
    // Parse Menu
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, m_menu_filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/Editor_Menu.xsd", "");
  }
  ...
  void cEditor ::Unload(void)
  {
    ...
    if (CEGUI::System::getSingleton()
            .getDefaultGUIContext()
            .getRootWindow()
            ->getChildRecursive("editor_help_window"))
    {
    ...
    // Tagged Items
    for (TaggedItemObjectsList::iterator itr = m_tagged_item_objects.begin();
         itr != m_tagged_item_objects.end(); ++itr)
    {
      delete *itr;
    }
    ...
    for (TaggedItemImageSettingsList::iterator itr = m_tagged_item_images.begin();
         itr != m_tagged_item_images.end(); ++itr)
    {
      delete *itr;
    }
  ...
  void cEditor ::Update(void)
  {
    ...
    if (m_listbox_menu->isVisible())
    {
      ...
      if (m_menu_timer >= speedfactor_fps * 2)
      {
        // fade out
        float new_alpha =
            m_editor_window->getAlpha() - (pFramerate->m_speed_factor * 0.05f);
      ...
      else
      {
        ...
        {
          float new_alpha =
              m_editor_window->getAlpha() + pFramerate->m_speed_factor * 0.1f;
        ...
        // inactive counter
        else if (Is_Float_Equal(m_editor_window->getXPosition().d_offset / 1.0f +
                                    m_editor_window->getXPosition().d_scale,
                                0.0f))
        {
          // if mouse is over the window
          if (m_tabcontrol_menu->isHit(CEGUI::System::getSingleton()
                                           .getDefaultGUIContext()
                                           .getMouseCursor()
                                           .getPosition()))
          {
  ...
  void cEditor ::Draw(void)
  {
    ...
    if (camera_bottom > pActive_Camera->m_limit_rect.m_y &&
        camera_top < pActive_Camera->m_limit_rect.m_y)
    {
      float start_x = 0.0f;
      ...
      pVideo->Draw_Line(start_x, -pActive_Camera->m_y,
                        static_cast<float>(game_res_w), -pActive_Camera->m_y,
                        0.124f, &color);
    }
    // Camera limit top line
    if (camera_bottom >
            pActive_Camera->m_limit_rect.m_y + pActive_Camera->m_limit_rect.m_h &&
        camera_top <
            pActive_Camera->m_limit_rect.m_y + pActive_Camera->m_limit_rect.m_h)
    {
      float start_x = 0.0f;
    ...
    if (camera_left < pActive_Camera->m_limit_rect.m_x &&
        camera_right > pActive_Camera->m_limit_rect.m_x)
    {
      ...
      pVideo->Draw_Line(pActive_Camera->m_limit_rect.m_x - pActive_Camera->m_x,
                        start_y, -pActive_Camera->m_x, 0, 0.124f, &color);
    }
    // Camera limit right line
    if (camera_left <
            pActive_Camera->m_limit_rect.m_x + pActive_Camera->m_limit_rect.m_w &&
        camera_right >
            pActive_Camera->m_limit_rect.m_x + pActive_Camera->m_limit_rect.m_w)
    {
      float start_y = static_cast<float>(game_res_h);
  ...
  void cEditor ::Process_Input(void)
  {
    ...
    if (pKeyboard->m_keys[SDLK_RIGHT] || pJoystick->m_right)
    {
      ...
      {
        pActive_Camera->Move(m_camera_speed * pFramerate->m_speed_factor * 3 *
                                 pPreferences->m_scroll_speed,
                             0.0f);
      }
      else
      {
        pActive_Camera->Move(m_camera_speed * pFramerate->m_speed_factor *
                                 pPreferences->m_scroll_speed,
                             0.0f);
      }
    }
    ...
    if (pKeyboard->m_keys[SDLK_UP] || pJoystick->m_up)
    {
      if (pKeyboard->Is_Shift_Down())
      {
        pActive_Camera->Move(0.0f, -(m_camera_speed * pFramerate->m_speed_factor *
                                     3 * pPreferences->m_scroll_speed));
      }
      else
      {
        pActive_Camera->Move(0.0f, -(m_camera_speed * pFramerate->m_speed_factor *
  ...
  bool cEditor ::Key_Down(SDL_Keycode key)
  {
    ...
    else if (key == SDLK_F1)
    {
      ...
      {
        CEGUI::FrameWindow *window_help = static_cast<CEGUI::FrameWindow *>(
            CEGUI::WindowManager::getSingleton().createWindow(
                "TaharezLook/FrameWindow", "editor_help_window"));
        window_help->setPosition(CEGUI::UVector2(
            CEGUI::UDim(0, (game_res_w * 0.1f) * global_upscalex),
            CEGUI::UDim(0, (game_res_h * 0.1f) * global_upscalex)));
        window_help->setSize(
            CEGUI::USize(CEGUI::UDim(0, (game_res_w * 0.8f) * global_upscalex),
                         CEGUI::UDim(0, (game_res_h * 0.8f) * global_upscalex)));
        window_help->getCloseButton()->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&cEditor::Window_Help_Exit_Clicked, this));
        window_help->setText("Editor Help");

        CEGUI::Window *text_help =
            CEGUI::WindowManager::getSingleton().createWindow(
                "TaharezLook/StaticText", "editor_help_text");
        text_help->setPosition(
            CEGUI::UVector2(CEGUI::UDim(0.00f, 0.0f), CEGUI::UDim(0.00f, 0.0f)));
        text_help->setSize(
            CEGUI::USize(CEGUI::UDim(1, 0.0f), CEGUI::UDim(1, 0.0f)));
        ...
        text_help->setText(
            ...
            "   "
            "[colour='FFFF2F0F']Massive[colour='FFFFFFFF']->[colour='FFFFAF2F']"
            "Halfmassive[colour='FFFFFFFF']->[colour='FFDF00FF']Climbable[colour="
            "'FFFFFFFF']->[colour='FF1FFF1F']Passive[colour='FFFFFFFF']->[colour="
            "'FF1FFF1F']Front Passive[colour='FFFFFFFF']\n"
            ...
            "Ctrl + R - Replace the selected basic sprite(s) image with another "
            "one\n"
    ...
    {
      int pos_x = string_to_int(Box_Text_Input(
          int_to_string(static_cast<int>(pActive_Camera->m_x)), "Position X", 1));
      int pos_y = string_to_int(Box_Text_Input(
          int_to_string(static_cast<int>(pActive_Camera->m_y)), "Position Y", 1));

      pActive_Camera->Set_Pos(static_cast<float>(pos_x),
                              static_cast<float>(pos_y));
    }
    // push selected objects into the front
    else if (key == SDLK_KP_PLUS)
    {
      for (SelectedObjectList::iterator itr =
               pMouseCursor->m_selected_objects.begin();
           itr != pMouseCursor->m_selected_objects.end(); ++itr)
      {
    ...
    // copy into direction
    else if ((key == pPreferences->m_key_editor_fast_copy_up ||
              key == pPreferences->m_key_editor_fast_copy_down ||
              key == pPreferences->m_key_editor_fast_copy_left ||
              key == pPreferences->m_key_editor_fast_copy_right) &&
             pMouseCursor->m_hovering_object->m_obj &&
             pMouseCursor->m_fastcopy_mode)
    {
      ...
      // add new objects
      for (cSprite_List::iterator itr = new_objects.begin();
           itr != new_objects.end(); ++itr)
      {
        cSprite *obj = (*itr);
      ...
      for (cSprite_List::const_iterator itr = objects.begin();
           itr != objects.end(); ++itr)
      {
    ...
    else if ((key == pPreferences->m_key_editor_pixel_move_up ||
              key == pPreferences->m_key_editor_pixel_move_down ||
              key == pPreferences->m_key_editor_pixel_move_left ||
              key == pPreferences->m_key_editor_pixel_move_right) &&
             pMouseCursor->m_hovering_object->m_obj)
    {
    ...
    {
      pMouseCursor->Paste_Copy_Objects(
          static_cast<float>(static_cast<int>(pMouseCursor->m_pos_x)),
          static_cast<float>(static_cast<int>(pMouseCursor->m_pos_y)));
    }
    ...
    else if (pKeyboard->Is_Shift_Down() &&
             pMouseCursor->m_hovering_object->m_obj)
    {
             ...
             itr != pMouseCursor->m_selected_objects.end(); ++itr)
        {
          ...
          if (object->m_obj->m_type == TYPE_PARTICLE_EMITTER)
          {
            cParticle_Emitter *emitter =
                static_cast<cParticle_Emitter *>(object->m_obj);
            emitter->Pre_Update();
          }
        }
      }
  ...
  void cEditor ::Add_Menu_Object(
      const std::string &name, std::string tags,
      CEGUI::Colour normal_color /* = CEGUI::Colour( 1, 1, 1 ) */)
  {
    ...
    if (tags.find("header") == 0)
    {
      ...
      // header color rect
      new_menu->setTextColours(normal_color, normal_color,
                               CEGUI::Colour(0.5f, 0.5f, 0.5f),
                               CEGUI::Colour(0.5f, 0.5f, 0.5f));
      // not selectable
  ...
  bool cEditor ::Load_Item_Menu(std::string item_tags)
  {
    ...
    for (TaggedItemImageSettingsList::const_iterator itr =
             m_tagged_item_images.begin();
         itr != m_tagged_item_images.end(); ++itr)
    {
    ...
    for (TaggedItemObjectsList::const_iterator itr =
             m_tagged_item_objects.begin();
         itr != m_tagged_item_objects.end(); ++itr)
    {
      cSprite *object = (*itr);
  ...
  void cEditor ::Add_Item_Object(cSprite *sprite, std::string new_name /* = "" */,
                                 cGL_Surface *image /* = NULL */)
  {
    ...
    {
      // special object
      if (sprite->m_type == TYPE_ENEMY_STOPPER ||
          sprite->m_type == TYPE_LEVEL_EXIT ||
          sprite->m_type == TYPE_LEVEL_ENTRY || sprite->m_type == TYPE_SOUND ||
          sprite->m_type == TYPE_ANIMATION || sprite->m_type == TYPE_PATH)
      {
    ...
    cEditor_Item_Object *new_item =
        new cEditor_Item_Object(obj_name, m_listbox_items);
    // Initialize
  ...
  void cEditor ::Load_Image_Items(std::string dir)
  {
    ...
    for (vector<std::string>::const_iterator itr = image_files.begin();
         itr != image_files.end(); ++itr)
    {
      // get filename
      ...
      if (settings)
      {
        ...
        if (settings->m_editor_tags.find(m_editor_item_tag) !=
            std::string::npos)
        {
  ...
  void cEditor ::Activate_Item(cEditor_Item_Object *entry)
  {
    ...
    pMouseCursor->m_hovering_object->m_mouse_offset_y =
        static_cast<int>(new_sprite->m_col_rect.m_h / 2);
    pMouseCursor->m_hovering_object->m_mouse_offset_x =
        static_cast<int>(new_sprite->m_col_rect.m_w / 2);
    pMouseCursor->Set_Hovered_Object(new_sprite);
  }

  cSprite *cEditor ::Get_Object(const CEGUI::String &element,
  ...
  cSprite_List cEditor ::Copy_Direction(const cSprite_List &objects,
                                        const ObjectDirection dir) const
  {
    ...
    if (dir == DIR_LEFT || dir == DIR_RIGHT)
    {
      ...
      for (unsigned int i = 1; i < objects.size(); i++)
      {
        if (objects[i]->m_start_pos_x + objects[i]->m_start_rect.m_w >
            last->m_start_pos_x + last->m_start_rect.m_w)
        {
          last = objects[i];
        }
      }
      ...
      // Set X offset
      offset = static_cast<int>(last->m_start_pos_x - first->m_start_pos_x +
                                last->m_start_rect.m_w);
    }
    else if (dir == DIR_UP || dir == DIR_DOWN)
    {
      ...
      for (unsigned int i = 1; i < objects.size(); i++)
      {
        if (objects[i]->m_start_pos_y + objects[i]->m_start_rect.m_h >
            last->m_start_pos_y + last->m_start_rect.m_h)
        {
          last = objects[i];
        }
      }
      ...
      // Set Y offset
      offset = static_cast<int>(last->m_start_pos_y - first->m_start_pos_y +
                                last->m_start_rect.m_h);
    }

    // new copied objects
    cSprite_List new_objects;

    for (cSprite_List::const_iterator itr = objects.begin(); itr != objects.end();
         ++itr)
    {
      const cSprite *obj = (*itr);
  ...
  cSprite *cEditor ::Copy_Direction(const cSprite *obj, const ObjectDirection dir,
                                    int offset /* = 0 */) const
  {
    float w = 0.0f;
    float h = 0.0f;
    ...
    return pMouseCursor->Copy(obj, obj->m_start_pos_x + w,
                              obj->m_start_pos_y + h);
  }
  ...
  void cEditor ::Replace_Sprites(void)
  {
    ...
    std::string image_filename = Box_Text_Input(
        pMouseCursor->m_selected_objects[0]->m_obj->m_start_image->m_filename,
        _("Change selected Sprite(s) image to"), 0);
  ...
  bool cEditor ::Is_Tag_Available(const std::string &str, const std::string &tag,
  ...
  {
    CEGUI::Window *window_help = CEGUI::System::getSingleton()
                                     .getDefaultGUIContext()
                                     .getRootWindow()
                                     ->getChildRecursive("editor_help_window");
    CEGUI::System::getSingleton()
        .getDefaultGUIContext()
        .getRootWindow()
        ->removeChild(window_help);
    CEGUI::WindowManager::getSingleton().destroyWindow(window_help);

    return 1;
  }

  void cEditor ::elementStart(const CEGUI::String &element,
  ...
  void cEditor ::elementEnd(const CEGUI::String &element)
  {
    ...
    else if (element == "items" || element == "menu" || element == "Items" ||
             element == "Menu")
    {
      // ignore
    }
  ...
  void cEditor ::Handle_Menu(const CEGUI::XMLAttributes &attributes)
  {
    ...
    Add_Menu_Object(name, tags,
                    CEGUI::PropertyHelper<CEGUI::Colour>::fromString(
                        m_xml_attributes.getValueAsString("color", "FFFFFFFF")));
  }

...
void cLevel_Exit ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // type
  Set_Type(static_cast<Level_Exit_type>(
      attributes.getValueAsInteger("type", m_exit_type)));
  // motion
  Set_Camera_Motion(static_cast<Camera_movement>(
      attributes.getValueAsInteger("camera_motion", m_exit_motion)));
  // destination level
  Set_Level(attributes.getValueAsString("level_name").c_str());
  // destination entry
  Set_Entry(attributes.getValueAsString("entry").c_str());
  // path identifier
  if (m_exit_motion == CAMERA_MOVE_ALONG_PATH ||
      m_exit_motion == CAMERA_MOVE_ALONG_PATH_BACKWARDS) {
...
void cLevel_Exit ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (m_editor_entry_name) {
    ...
    // blit
    m_editor_entry_name->Blit(
        m_col_rect.m_x + m_col_rect.m_w + 5 - pActive_Camera->m_x,
        m_col_rect.m_y - pActive_Camera->m_y, m_editor_pos_z, surface_request);
...
void cLevel_Exit ::Activate(void) {
  ...
  if (m_exit_type == LEVEL_EXIT_WARP) {
    ...
    pLevel_Player->Set_Image_Num(pLevel_Player->Get_Image() +
                                 pLevel_Player->m_direction);
    pLevel_Player->Stop_Ducking();
    pLevel_Player->Reset_On_Ground();

    // set position and image
    if (m_direction == DIR_UP || m_direction == DIR_DOWN) {
      pLevel_Player->Set_Pos_X(m_col_rect.m_x - pLevel_Player->m_col_pos.m_x +
                               (m_col_rect.m_w * 0.5f) -
                               (pLevel_Player->m_col_rect.m_w * 0.5f));
    } else if (m_direction == DIR_LEFT || m_direction == DIR_RIGHT) {
      pLevel_Player->Set_Pos_Y(m_col_rect.m_y - pLevel_Player->m_col_pos.m_y +
                               (m_col_rect.m_h * 0.5f) -
                               (pLevel_Player->m_col_rect.m_h * 0.5f));
  ...
  // enter entry
    ...
    pLevel_Manager->Goto_Sub_Level(m_dest_level, m_dest_entry, m_exit_motion,
                                   m_path_identifier);
  }
}
...
std::string cLevel_Exit ::Get_Level(bool with_dir /* = 1 */,
...
void cLevel_Exit ::Set_Entry(const std::string &entry_name) {
  ...
  m_editor_entry_name =
      pFont->Render_Text(pFont->m_font_small, m_dest_entry, white);
}
...
void cLevel_Exit ::Editor_Activate(void) {
  ...
  if (m_exit_type == LEVEL_EXIT_WARP) {
    ...
    CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
        wmgr.createWindow("TaharezLook/Combobox", "level_exit_direction"));
    Editor_Add(UTF8_("Direction"), UTF8_("Direction to move in"), combobox, 100,
               105);
    ...
    combobox->subscribeEvent(
        ...
        CEGUI::Event::Subscriber(&cLevel_Exit::Editor_Direction_Select, this));
  }

  // motion
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "CAMERA_MOVEotion"));
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cLevel_Exit::Editor_Motion_Select, this));

  // destination level
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "level_exit_destination_level"));
  Editor_Add(UTF8_("Destination Level"),
             UTF8_("Name of the level that should be entered. If empty uses "
                   "the current level."),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cLevel_Exit::Editor_Destination_Level_Text_Changed, this));

  // destination entry
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "level_exit_destination_entry"));
  Editor_Add(UTF8_("Destination Entry"),
             UTF8_("Name of the Entry in the destination level. If empty the "
                   "entry point is the player start position."),
             editbox, 150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cLevel_Exit::Editor_Destination_Entry_Text_Changed, this));

  // path identifier
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "level_exit_path_identifier"));
  Editor_Add(UTF8_("Path Identifier"),
             UTF8_("Name of the Path to use for the camera movement."), editbox,
             150);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cLevel_Exit::Editor_Path_Identifier_Text_Changed, this));
...
void cLevel_Exit ::Editor_State_Update(void) {
  ...
  CEGUI::Editbox *editbox_path_identifier = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          ...
          .getRootWindow()
          ->getChildRecursive("level_exit_path_identifier"));
  // destination level
  CEGUI::Editbox *editbox_destination_level = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("level_exit_destination_level"));
  // direction
  // CEGUI::Combobox *combobox_direction = static_cast<CEGUI::Combobox
  // *>(CEGUI::System::getSingleton().getDefaultGUIContext().getRootWindow()->getChildRecursive(
...
bool cLevel_Exit ::Editor_Destination_Level_Text_Changed(
...
bool cLevel_Exit ::Editor_Destination_Entry_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cLevel_Exit ::Editor_Path_Identifier_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =

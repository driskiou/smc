...
cLevel_Player ::~cLevel_Player(void) { Ball_Clear(); }
...
void cLevel_Player ::Set_Direction(const ObjectDirection dir,
                                   bool new_start_direction /* = 0 */) {
  // set start image
  if (new_start_direction) {
    cMovingSprite::Set_Image(pVideo->Get_Surface("maryo/small/stand_" +
...
bool cLevel_Player ::Set_On_Ground(cSprite *obj, bool set_on_top /* = 1 */) {
  ...
  if (valid) {
    ...
    if (m_ground_object->m_massive_type == MASS_MASSIVE &&
        pKeyboard->m_keys[pPreferences->m_key_down]) {
      Start_Ducking();
    }
  }
...
void cLevel_Player ::DownGrade_Player(bool delayed /* = 1 */,
                                      bool force /* = 0 */) {
  ...
  // game over
    ...
    pAudio->Play_Sound(DATA_DIR "/" GAME_MUSIC_DIR "/game/lost_1.ogg",
                       RID_MARYO_DEATH);
  }
    ...
    while (SDL_PollEvent(&input_event)) {
      if (input_event.type == SDL_KEYDOWN) {
        if (input_event.key.keysym.sym == SDLK_ESCAPE) {
          goto animation_end;
        } else if (input_event.key.keysym.sym ==
...
animation_end:
  ...
  if (m_lives < 0) {
    ...
    sprite->Set_Pos((game_res_w * 0.5f) - (game_over->m_w * 0.5f),
                    (game_res_h * 0.5f) - (game_over->m_h * 0.5f));
    sprite->m_pos_z = 0.8f;
    ...
    anim->Set_Emitter_Rect(sprite->m_rect.m_x + pActive_Camera->m_x,
                           sprite->m_rect.m_y + pActive_Camera->m_y,
                           sprite->m_rect.m_w, sprite->m_rect.m_h);
    ...
    anim->Set_Color(Color(static_cast<Uint8>(250), 140, 90),
                    Color(static_cast<Uint8>(5), 100, 0, 0));
    ...
    for (i = 10.0f; i > 0.0f; i -= 0.011f * pFramerate->m_speed_factor) {
      ...
      // Escape stops
      if (keys[SDLK_ESCAPE] || keys[SDLK_RETURN] || keys[SDLK_SPACE] ||
          keys[pPreferences->m_key_action]) {
        break;
      }

      // if joystick enabled and exit pressed
      if (pPreferences->m_joy_enabled &&
          SDL_JoystickGetButton(pJoystick->m_joystick,
  ...
  // custom level
    ...
    Game_Action_Data_Middle.add(
        "menu_start_current_level",
        Trim_Filename(pActive_Level->m_level_filename, 0, 0));
    // reset saved data
    Game_Action_Data_Middle.add("reset_save", "1");
  }
  // back to overworld
...
void cLevel_Player ::Move_Player(float velocity, float vel_wrongway) {
  ...
  // get collision list
  cObjectCollisionType *col_list = Collision_Check_Relative(
      velocity, 0.0f, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);
  // if collision with a blocking object
  bool is_col = 0;

  // check collisions
  for (cObjectCollision_List::iterator itr = col_list->objects.begin();
       itr != col_list->objects.end(); ++itr) {
...
void cLevel_Player ::Generate_Feet_Clouds(
    cParticle_Emitter *anim /* = NULL */) {
  // check if valid
  if (!m_ground_object || !m_ground_object->m_image ||
      m_ground_object->m_image->m_ground_type == GROUND_NORMAL) {
    return;
  }
  ...
  anim->Set_Emitter_Rect(m_col_rect.m_x, m_col_rect.m_y + m_col_rect.m_h - 2.0f,
                         m_col_rect.m_w);
...
void cLevel_Player ::Update_Walking(void) {
  if (m_ducked_counter || !m_ground_object ||
      (m_state != STA_STAY && m_state != STA_WALK && m_state != STA_RUN)) {
    return;
  }

  // validate ground object
  if ((m_ground_object->m_type == TYPE_EATO ||
       m_ground_object->m_type == TYPE_SPIKA ||
       m_ground_object->m_type == TYPE_ROKKO ||
       m_ground_object->m_type == TYPE_STATIC_ENEMY) &&
      m_invincible <= 0 && !m_god_mode) {
    Reset_On_Ground();
  }

  // only if left or right is pressed
  if (pKeyboard->m_keys[pPreferences->m_key_left] ||
      pKeyboard->m_keys[pPreferences->m_key_right] || pJoystick->m_left ||
      pJoystick->m_right) {
    ...
    Move_Player((0.6f * ground_mod * Get_Vel_Modifier()),
                (1.2f * ground_mod * Get_Vel_Modifier()));
  }
...
void cLevel_Player ::Update_Running(void) {
  ...
  while (m_running_particle_counter > 1.0f) {
    ...
    if (vel > 17.0f) {
      // light particles
      cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
      anim->Set_Emitter_Rect(m_col_rect.m_x + m_col_rect.m_w * 0.25f,
                             m_col_rect.m_y + m_col_rect.m_h * 0.1f,
                             m_col_rect.m_w * 0.5f, m_col_rect.m_h * 0.8f);
      anim->Set_Image(pVideo->Get_Surface("animation/particles/light.png"));
      anim->Set_Pos_Z(m_pos_z + 0.000001f);
...
void cLevel_Player ::Update_Staying(void) {
  ...
  if (!m_ground_object || m_ducked_counter || m_state == STA_JUMP ||
      m_state == STA_CLIMB) {
    return;
  }
  ...
  if (!pKeyboard->m_keys[pPreferences->m_key_left] &&
    ...
    // walking
    if (m_velx) {
      if (m_ground_object->m_image &&
          m_ground_object->m_image->m_ground_type == GROUND_ICE) {
    ...
    if (m_ground_object->m_type == TYPE_SPIKA) {
      cMovingSprite *moving_ground_object =
          static_cast<cMovingSprite *>(m_ground_object);

      if (moving_ground_object->m_velx < 0.0f) {
        m_walk_count -=
            moving_ground_object->m_velx * pFramerate->m_speed_factor * 0.1f;
...
void cLevel_Player ::Start_Falling(void) {
  ...
  cObjectCollisionType *col_list =
      Collision_Check_Relative(0.0f, move_y, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);
...
void cLevel_Player ::Start_Ducking(void) {
  ...
  if ((m_state != STA_STAY && m_state != STA_WALK && m_state != STA_RUN) ||
      m_state == STA_CLIMB || !m_ground_object ||
      m_ground_object->m_massive_type == MASS_HALFMASSIVE || m_ducked_counter) {
    return;
  }
  ...
  cSprite::Move(
      0.0f, m_image->m_col_h - m_images[MARYO_IMG_DUCK].m_image->m_col_h, 1);
...
void cLevel_Player ::Stop_Ducking(void) {
  ...
  const float move_y =
      -(m_images[MARYO_IMG_STAND].m_image->m_col_h - m_image->m_col_h);
  ...
  if (m_ducked_counter > power_jump_delta) {
    ...
    if (m_ducked_animation_counter > 1.0f) {
      ...
      anim->Set_Emitter_Rect(m_col_rect.m_x,
                             m_col_rect.m_y + (m_col_rect.m_h * 0.8f),
                             m_col_rect.m_w * 0.9f, m_col_rect.m_h * 0.1f);
      ...
      m_ducked_animation_counter -=
          static_cast<int>(m_ducked_animation_counter);
    }
  }
...
bool cLevel_Player ::Is_On_Climbable(float move_y /* = 0.0f */) {
  ...
  // collision testing
  cObjectCollisionType *col_list =
      Collision_Check(&climb_rect, COLLIDE_ONLY_INTERNAL);
...
void cLevel_Player ::Start_Jump_Keytime(void) {
  if (m_god_mode || m_state == STA_STAY || m_state == STA_WALK ||
      m_state == STA_RUN || m_state == STA_FALL || m_state == STA_FLY ||
      m_state == STA_JUMP ||
      (m_state == STA_CLIMB && !pKeyboard->m_keys[pPreferences->m_key_up])) {
...
void cLevel_Player ::Update_Jump_Keytime(void) {
  ...
  if (m_force_jump || (m_up_key_time && (m_ground_object || m_god_mode ||
                                         m_state == STA_CLIMB))) {
    Start_Jump();
  }
}
...
void cLevel_Player ::Start_Jump(float deaccel /* = 0.08f */) {
  ...
  // if jump key pressed
  if (pKeyboard->m_keys[pPreferences->m_key_jump] ||
      (pPreferences->m_joy_analog_jump && pJoystick->m_up) ||
      pJoystick->Button(pPreferences->m_joy_button_jump)) {
    jump_key = 1;
  }
  ...
  // fly
  if (m_maryo_type == MARYO_CAPE && !m_force_jump && m_state == STA_RUN &&
      jump_key &&
      ((m_direction == DIR_RIGHT && m_velx > 14) ||
       (m_direction == DIR_LEFT && m_velx < -14))) {
  ...
  // jump with velx if ducking but only into the opposite start duck direction
...
void cLevel_Player ::Update_Jump(void) {
  ...
  if (pKeyboard->m_keys[pPreferences->m_key_jump] ||
      (pPreferences->m_joy_analog_jump && pJoystick->m_up) ||
      pJoystick->Button(pPreferences->m_joy_button_jump)) {
    Add_Velocity_Y(-(m_jump_accel_up +
                     (m_vely * m_jump_vel_deaccel) / Get_Vel_Modifier()));
  ...
  if ((pKeyboard->m_keys[pPreferences->m_key_left] || pJoystick->m_left) &&
    ...
    if (m_velx > max_vel) {
      Add_Velocity_X_Min((-1.1f * Get_Vel_Modifier()) - (m_velx / 100),
                         max_vel);
    }
...
void cLevel_Player ::Update_Item(void) {
  ...
  if (m_active_object) {
    ...
    if (m_active_object->m_type == TYPE_TURTLE &&
        static_cast<cTurtle *>(m_active_object)->m_dead) {
      Release_Item(0);
      return;
    }
  ...
  if (pKeyboard->m_keys[pPreferences->m_key_action] ||
      pJoystick->Button(pPreferences->m_joy_button_action)) {
    // next position velocity with extra size
    float check_x = (m_velx > 0.0f) ? (m_velx + 5.0f) : (m_velx - 5.0f);

    // if wrong direction return
    if ((m_direction == DIR_LEFT && check_x >= 0.0f) ||
        (m_direction == DIR_RIGHT && check_x <= 0.0f)) {
      return;
    }

    // check the next player position for objects
    cObjectCollisionType *col_list = Collision_Check_Relative(
        (m_direction == DIR_LEFT) ? (check_x) : (m_col_rect.m_w), 0,
        (m_direction == DIR_LEFT) ? (-check_x) : (check_x));
...
void cLevel_Player ::Release_Item(bool set_position /* = 1 */,
                                  bool no_action /* = 0 */) {
  if (!m_active_object) {
    return;
  }
  ...
  if (set_position) {
    ...
    float start_posx = m_pos_x + m_col_pos.m_x +
                       (m_active_object->m_col_rect.m_w / 2) -
                       (m_col_rect.m_w / 2);
    ...
    if (kick_direction == DIR_LEFT) {
      ...
      // put on top
      start_posy +=
          -(m_active_object->m_col_pos.m_y + m_active_object->m_col_rect.m_h) +
          (m_active_object->m_vely * pFramerate->m_speed_factor);
    }
    ...
    while (step_size < 50.0f) {
      ...
      // check left side
      col_list = m_active_object->Collision_Check_Relative(
          step_size, 0.0f, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);

      // collides with a blocking object on the left side
      if (!col_list->empty() && (col_list->Is_Included(ARRAY_MASSIVE) ||
                                 col_list->Is_Included(ARRAY_ACTIVE) ||
                                 col_list->Is_Included(ARRAY_ENEMY))) {
        delete col_list;
        // check right side
        col_list = m_active_object->Collision_Check_Relative(
            -step_size, 0.0f, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);
    ...
    if (!col_list->empty() && (col_list->Is_Included(ARRAY_MASSIVE) ||
      ...
      if (m_active_object->m_type == TYPE_TURTLE) {
        ...
        if (m_active_object->m_state == STA_RUN) {
          ...
          if (col_list->Is_Included(ARRAY_MASSIVE) ||
              col_list->Is_Included(ARRAY_ACTIVE)) {
...
void cLevel_Player ::Set_Type(SpriteType item_type, bool animation /* = 1 */,
                              bool sound /* = 1 */, bool temp_power /* = 0 */) {
...
void cLevel_Player ::Set_Type(Maryo_type new_type, bool animation /* = 1 */,
                              bool sound /* = 1 */, bool temp_power /* = 0 */) {
  // already set
...
void cLevel_Player ::Set_Moving_State(Moving_state new_state) {
  ...
  // was flying
    ...
    Change_Size(0, -(m_images[MARYO_IMG_FALL].m_image->m_h -
                     m_images[MARYO_IMG_FLY].m_image->m_h));
...
bool cLevel_Player ::Change_Size(float x, float y, bool only_check /* = 0 */) {
  ...
  while (!valid_hor) {
    cObjectCollisionType *col_list = Collision_Check_Relative(
        check_pos, 0.0f, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);
  ...
  while (!valid_ver) {
    cObjectCollisionType *col_list = Collision_Check_Relative(
        0.0f, check_pos, 0.0f, 0.0f, COLLIDE_ONLY_BLOCKING);
...
void cLevel_Player ::Update(void) {
  ...
  if (m_god_mode || m_invincible_star > 0.0f) {
    if (editor_level_enabled) {
      ...
      if (m_invincible_star_counter > 1.0f) {
        ...
        // create particle
        cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
        anim->Set_Emitter_Rect(m_col_rect.m_x + m_col_rect.m_w * 0.1f,
                               m_col_rect.m_y + m_col_rect.m_h * 0.1f,
                               m_col_rect.m_w * 0.8f, m_col_rect.m_h * 0.8f);
        ...
        m_invincible_star_counter -=
            static_cast<int>(m_invincible_star_counter);
      }
    }
  }
  ...
  // image counter
  if (m_state == STA_WALK ||
      (m_maryo_type != MARYO_CAPE && m_state == STA_RUN)) {
    // 4 frames
    if (m_maryo_type == MARYO_SMALL) {
  ...
  if (m_state == STA_WALK || m_state == STA_STAY ||
      (m_maryo_type != MARYO_CAPE && m_state == STA_RUN)) {
    // 4 frames
    if (m_maryo_type == MARYO_SMALL) {
...
void cLevel_Player ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (m_invincible > 0.0f) {
    // star invincible
    if (m_invincible_star > 0.0f) {
      Set_Color_Combine(m_invincible_mod / 130, m_invincible_mod / 130,
                        m_invincible_mod / 180, GL_ADD);
    }
    // default invincible
    else {
  ...
  if (!editor_enabled) {
    ...
    if (game_debug) {
      ...
      if (m_ground_object) {
        ...
        // draw
        pVideo->Draw_Rect(&m_ground_object->m_col_rect, m_pos_z + 0.000009f,
                          &grey, rect_request);
...
void cLevel_Player ::Draw_Animation(Maryo_type new_mtype) {
  ...
  if (new_mtype == m_maryo_type || m_maryo_type == MARYO_DEAD ||
      new_mtype == MARYO_DEAD) {
    return;
  }
  ...
  if (maryo_type_old == MARYO_SMALL &&
      (new_mtype == MARYO_BIG || new_mtype == MARYO_FIRE ||
       new_mtype == MARYO_ICE || new_mtype == MARYO_CAPE ||
       new_mtype == MARYO_GHOST)) {
    Change_Size(-5.0f, -12.0f);
  }
  // correct position for small maryo
  else if ((maryo_type_old == MARYO_BIG || maryo_type_old == MARYO_FIRE ||
            maryo_type_old == MARYO_ICE || new_mtype == MARYO_CAPE ||
            maryo_type_old == MARYO_GHOST) &&
           new_mtype == MARYO_SMALL) {
    Change_Size(5.0f, 12.0f);
  }
...
unsigned int cLevel_Player ::Get_Image(void) const {
  // throwing
  if (m_throwing_counter &&
      (m_maryo_type == MARYO_FIRE || m_maryo_type == MARYO_ICE) &&
      !m_ducked_counter &&
      (m_state == STA_FALL || m_state == STA_STAY || m_state == STA_WALK ||
       m_state == STA_RUN || m_state == STA_JUMP)) {
    int imgnum = 0;
  ...
  if (m_ducked_counter &&
      (m_state == STA_STAY || m_state == STA_WALK || m_state == STA_RUN ||
       m_state == STA_JUMP || m_state == STA_FALL)) {
  ...
  if (m_state == STA_STAY || m_state == STA_WALK ||
      (m_maryo_type != MARYO_CAPE && m_state == STA_RUN)) {
...
void cLevel_Player ::Load_Images(void) {
  ...
  if (m_maryo_type == MARYO_SMALL) {
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/small/stand_left" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/stand_right" + special_state +
                                  ".png"));
    // walking
    Add_Image(pVideo->Get_Surface("maryo/small/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/walk_right_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/walk_left_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/walk_right_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/small/walk_right_1" + special_state +
    ...
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/small/fall_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/small/fall_right" + special_state + ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/small/jump_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/small/jump_right" + special_state + ".png"));
    // dead
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/big/stand_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/stand_right" + special_state + ".png"));
    // walking
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_left_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_right_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_left_2" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_right_2" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_left_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/walk_right_1" + special_state + ".png"));
    ...
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/big/fall_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/fall_right" + special_state + ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/big/jump_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/big/jump_right" + special_state + ".png"));
    // dead
    Add_Image(pVideo->Get_Surface("maryo/small/dead_left.png"));
    Add_Image(pVideo->Get_Surface("maryo/small/dead_right.png"));
    // ducked
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/fire/stand_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/fire/stand_right" + special_state + ".png"));
    // walking
    Add_Image(
        pVideo->Get_Surface("maryo/fire/walk_left_1" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/fire/walk_right_1" + special_state +
                                  ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/fire/walk_left_2" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/fire/walk_right_2" + special_state +
                                  ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/fire/walk_left_1" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/fire/walk_right_1" + special_state +
    ...
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/fire/fall_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/fire/fall_right" + special_state + ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/fire/jump_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/fire/fall_right" + special_state + ".png"));
    // dead
    Add_Image(pVideo->Get_Surface("maryo/small/dead_left.png"));
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/ice/stand_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/stand_right" + special_state + ".png"));
    // walking
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_left_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_right_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_left_2" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_right_2" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_left_1" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/walk_right_1" + special_state + ".png"));
    ...
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/ice/fall_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/fall_right" + special_state + ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/ice/jump_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ice/fall_right" + special_state + ".png"));
    // dead
    Add_Image(pVideo->Get_Surface("maryo/small/dead_left.png"));
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/flying/left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/flying/right" + special_state + ".png"));
    // walking
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_right_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_left_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_right_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/walk_right_1" + special_state +
                                  ".png"));
    // running
    Add_Image(pVideo->Get_Surface("maryo/flying/run_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/run_right_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/run_left_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/run_right_2" + special_state +
                                  ".png"));
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/flying/fall_left" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/fall_right" + special_state +
                                  ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/flying/jump_left" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/flying/fall_right" + special_state +
                                  ".png"));
    // dead
    Add_Image(pVideo->Get_Surface("maryo/small/dead_left.png"));
    ...
    // standing
    Add_Image(
        pVideo->Get_Surface("maryo/ghost/stand_left" + special_state + ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/stand_right" + special_state +
                                  ".png"));
    // walking
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_right_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_left_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_right_2" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_left_1" + special_state +
                                  ".png"));
    Add_Image(pVideo->Get_Surface("maryo/ghost/walk_right_1" + special_state +
    ...
    // falling
    Add_Image(
        pVideo->Get_Surface("maryo/ghost/fall_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ghost/fall_right" + special_state + ".png"));
    // jumping
    Add_Image(
        pVideo->Get_Surface("maryo/ghost/jump_left" + special_state + ".png"));
    Add_Image(
        pVideo->Get_Surface("maryo/ghost/fall_right" + special_state + ".png"));
    // dead
    Add_Image(pVideo->Get_Surface("maryo/small/dead_left.png"));
...
void cLevel_Player ::Get_Item(SpriteType item_type, bool force /* = 0 */,
                              cMovingSprite *base /* = NULL */) {
  ...
  if (item_type == TYPE_MUSHROOM_DEFAULT) {
    if ((current_maryo_type == MARYO_SMALL || force) &&
        Change_Size(-5, -12, 1)) {
      // change to big
  ...
  // Fireplant
    ...
    if (((current_maryo_type == MARYO_SMALL || force) &&
         Change_Size(-5, -12, 1)) ||
        (current_maryo_type == MARYO_BIG || current_maryo_type == MARYO_ICE ||
         force)) {
      // move item to itembox
    ...
    // fire explosion
             ...
             pHud_Itembox->m_item_id == TYPE_FIREPLANT) {
      ...
      pHud_Points->Add_Points(1000, m_pos_x + (m_col_rect.m_w / 2),
                              m_pos_y + 2);
    }
    // move item to itembox
    else {
  ...
  // Blue Mushroom
    ...
    if (((current_maryo_type == MARYO_SMALL || force) &&
         Change_Size(-5, -20, 1)) ||
        ((current_maryo_type == MARYO_BIG || current_maryo_type == MARYO_FIRE ||
          force))) {
      // move item to itembox
    ...
    // ice explosion
    else if (current_maryo_type == MARYO_ICE &&
             pHud_Itembox->m_item_id == TYPE_MUSHROOM_BLUE) {
      unsigned int ball_amount = 10;
...
float cLevel_Player ::Get_Vel_Modifier(void) const {
  ...
  if (pPreferences->m_always_run ||
      pKeyboard->m_keys[pPreferences->m_key_action] ||
      pJoystick->Button(pPreferences->m_joy_button_action)) {
    vel_mod = 1.5f;
  }
...
void cLevel_Player ::Action_Interact(input_identifier key_type) {
  ...
  if (key_type == INP_UP) {
    ...
    for (cSprite_List::iterator itr = m_sprite_manager->objects.begin();
         itr != m_sprite_manager->objects.end(); ++itr) {
      cSprite *obj = (*itr);
      ...
      if (obj->m_type == TYPE_LEVEL_EXIT) {
        ...
        // if leaving level
        if (level_exit->m_dest_level.empty() &&
            level_exit->m_dest_entry.empty()) {
...
bool cLevel_Player ::Ball_Add(ball_effect effect_type /* = FIREBALL_DEFAULT */,
                              float ball_start_angle /* = -1 */,
                              unsigned int amount /* = 0 */) const {
  if ((m_maryo_type != MARYO_FIRE && m_maryo_type_temp_power != MARYO_FIRE &&
       m_maryo_type != MARYO_ICE && m_maryo_type_temp_power != MARYO_ICE) ||
      m_ducked_counter) {
    return 0;
  }
  ...
  // half circle fire/ice-ball explosion
  else if (effect_type == FIREBALL_EXPLOSION ||
           effect_type == ICEBALL_EXPLOSION) {
    // start angle
    ...
    for (unsigned int i = 0; i < amount; i++) {
      ...
      // add ball
      cBall *ball = new cBall(m_sprite_manager);
      ball->Set_Pos(m_pos_x + m_col_rect.m_w / 2, m_pos_y + m_col_rect.m_h / 2,
                    1);
      ball->Set_Origin(m_sprite_array, m_type);
      ball->Set_Ball_Type(effect_type);
    ...
    if (effect_type == FIREBALL_EXPLOSION) {
      cAnimation_Fireball *anim = new cAnimation_Fireball(
          m_sprite_manager, m_pos_x + (m_col_rect.m_w / 2),
          m_pos_y + (m_col_rect.m_h / 3), 10);
      ...
      // create animation
      cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
      anim->Set_Pos(m_pos_x + (m_col_rect.m_w * 0.5f),
                    m_pos_y + (m_col_rect.m_h * 0.5f));
      anim->Set_Image(pVideo->Get_Surface("animation/particles/light.png"));
      anim->Set_Quota(10);
...
void cLevel_Player ::Handle_Collision_Enemy(cObjectCollision *collision) {
  // if invalid
  if (collision->m_direction == DIR_UNDEFINED || m_maryo_type == MARYO_DEAD ||
      !m_active) {
    return;
  }

  cEnemy *enemy =
      static_cast<cEnemy *>(m_sprite_manager->Get_Pointer(collision->m_number));
  ...
  if (m_invincible_star > 0.0f) {
    ...
    if ((enemy->m_type == TYPE_SPIKA || enemy->m_type == TYPE_EATO ||
         enemy->m_type == TYPE_THROMP) &&
        collision->m_direction == DIR_DOWN) {
      hit_enemy = 0;
    }
    // rokko
    ...
    if (hit_enemy) {
      ...
      pHud_Points->Add_Points(
          static_cast<unsigned int>(enemy->m_kill_points * 1.2f),
          enemy->m_pos_x, enemy->m_pos_y - 5.0f, "", yellow, 1);
  ...
  if (enemy->m_freeze_counter) {
    ...
    anim->Set_Color(
        Color(static_cast<Uint8>(160), 160, 240),
        Color(static_cast<Uint8>(rand() % 80), rand() % 80, rand() % 10, 0));
    anim->Set_Fading_Alpha(1);
    anim->Set_Fading_Size(1);
    ...
    // get points
    pHud_Points->Add_Points(enemy->m_kill_points, enemy->m_pos_x,
                            enemy->m_pos_y - 10.0f, "", static_cast<Uint8>(255),
                            1);
...
void cLevel_Player ::Handle_Collision_Massive(cObjectCollision *collision) {
  ...
  /* fixme: the collision direction is sometimes wrong as left/right if landing
   * on a moving platform which moves upwards this seems to cause maryo to hang
   * or to get stuck in it easily reproducible with a low framerate and fast
   * moving platform
   */
  // printf( "direction is %s\n", Get_Direction_Name( collision->m_direction
  // ).c_str() );

  cSprite *col_obj = m_sprite_manager->Get_Pointer(collision->m_number);

  // climbable
  if (col_obj->m_massive_type == MASS_CLIMBABLE && m_state != STA_CLIMB &&
      m_state != STA_FLY) {
    // if not climbing and player wants to climb
    if (pKeyboard->m_keys[pPreferences->m_key_up] || pJoystick->m_up ||
        ((pKeyboard->m_keys[pPreferences->m_key_down] || pJoystick->m_down) &&
         !m_ground_object)) {
      // start climbing
      Start_Climbing();
    }
  ...
  // send back if item
  if (col_obj->m_type == TYPE_FIREPLANT || col_obj->m_type == TYPE_GOLDPIECE ||
      col_obj->m_type == TYPE_MOON) {
    Send_Collision(collision);
    return;
  }
  ...
  if (collision->m_direction == DIR_UP) {
    ...
    if (m_state != STA_FLY) {
      if (m_state != STA_CLIMB) {
        ...
        if (collision->m_array == ARRAY_MASSIVE) {
          ...
          // create animation
          cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
          anim->Set_Emitter_Rect(m_col_rect.m_x, m_col_rect.m_y + 6,
                                 m_col_rect.m_w);
          anim->Set_Image(pVideo->Get_Surface("animation/particles/light.png"));
          anim->Set_Quota(4);
          anim->Set_Pos_Z(m_pos_z - 0.000001f);
          anim->Set_Time_to_Live(0.3f);
          anim->Set_Color(Color(static_cast<Uint8>(150), 150, 150, 200),
                          Color(static_cast<Uint8>(rand() % 55), rand() % 55,
                                rand() % 55, 0));
    ...
    // not flying
      ...
      // jumping
      if (m_state == STA_JUMP) {
        /* hack : only pick up after some time of jumping because moving objects
         * collide move before the player also see cMovingSprite ::
         * Validate_Collision_Object_On_Top
         */
    ...
    // flying
      ...
      // box collision
      if (col_obj->m_type == TYPE_BONUS_BOX ||
          col_obj->m_type == TYPE_SPIN_BOX) {
...
void cLevel_Player ::Handle_out_of_Level(ObjectDirection dir) {
  if (dir == DIR_LEFT) {
    ...
    Set_Pos_X(pLevel_Manager->m_camera->m_limit_rect.m_x +
              pLevel_Manager->m_camera->m_limit_rect.m_w - m_col_pos.m_x -
              m_col_rect.m_w - 0.01f);
    m_velx = 0.0f;
    Stop_Flying();
  } else if (dir == DIR_TOP) {
    /*if( pActive_Level->limit_top_blocks )
    {
            Set_Pos_Y( pLevel_Manager->m_camera->m_limit_rect.m_y +
    pLevel_Manager->m_camera->m_limit_rect.m_h + 0.01f ); m_vely = 0.0f;
    }*/
    ...
    if (m_god_mode) {
      ...
      {
              Set_Pos_Y( pLevel_Manager->m_camera->m_limit_rect.m_y + game_res_h
      - m_col_pos.m_x - m_col_rect.m_w + 0.01f ); m_vely = 0.0f;
      }
...
void cLevel_Player ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_player_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cLevel_Player::Editor_Direction_Select, this));

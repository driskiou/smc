...
public:
  cAnimation_Goldpiece(cSprite_Manager *sprite_manager, float posx, float posy,
                       float height = 40.0f, float width = 20.0f);
...
public:
  cAnimation_Fireball(cSprite_Manager *sprite_manager, float posx, float posy,
...
public:
  ...
  // create from stream
  cParticle_Emitter(CEGUI::XMLAttributes &attributes,
  ...
  void Keep_Particles_In_Rect(const GL_rect &clip_rect,
                              ParticleClipMode mode = PCM_MOVE);
  ...
  virtual void Set_Color(const Color &col,
                         const Color &col_rand = Color(static_cast<Uint8>(0), 0,
                                                       0, 0));
  // Set fading type
  ...
  bool
  Editor_Emitter_Based_On_Camera_Pos_Changed(const CEGUI::EventArgs &event);
  bool Editor_Particle_Based_On_Emitter_Pos_Text_Changed(
  ...
  bool
  Editor_Horizontal_Gravity_Base_Text_Changed(const CEGUI::EventArgs &event);
  bool
  Editor_Horizontal_Gravity_Rand_Text_Changed(const CEGUI::EventArgs &event);

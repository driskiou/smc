...
const SDL_Keycode cPreferences::m_key_editor_fast_copy_right_default =
    SDLK_KP_6;
const SDL_Keycode cPreferences::m_key_editor_pixel_move_up_default = SDLK_KP_8;
const SDL_Keycode cPreferences::m_key_editor_pixel_move_down_default =
    SDLK_KP_2;
const SDL_Keycode cPreferences::m_key_editor_pixel_move_left_default =
    SDLK_KP_4;
const SDL_Keycode cPreferences::m_key_editor_pixel_move_right_default =
    SDLK_KP_6;
...
cPreferences ::cPreferences(void) { Reset_All(); }
...
// again if CEGUI 0.8 works with std::string utf8
    ...
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, (const CEGUI::utf8 *)m_config_filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/Config.xsd", "");
#else
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, m_config_filename, DATA_DIR "/" GAME_SCHEMA_DIR "/Config.xsd",
        "default");
#endif
  }
  // catch CEGUI Exceptions
    ...
    pHud_Debug->Set_Text(_("Preferences Loading failed : ") +
...
// fixme : Check if there is a more portable way f.e. with imbue()
  ...
  if (!file.is_open()) {
    printf(
        "Error : couldn't open config %s for saving. Is the file read-only ?\n",
        m_config_filename.c_str());
    return;
  }
  ...
  Write_Property(stream, "video_screen_bpp",
                 static_cast<int>(m_video_screen_bpp));
  ...
  Write_Property(stream, "audio_sound_volume",
                 static_cast<int>(pAudio->m_sound_volume));
  Write_Property(stream, "audio_music_volume",
                 static_cast<int>(pAudio->m_music_volume));
  ...
  Write_Property(stream, "keyboard_key_editor_fast_copy_up",
                 m_key_editor_fast_copy_up);
  Write_Property(stream, "keyboard_key_editor_fast_copy_down",
                 m_key_editor_fast_copy_down);
  Write_Property(stream, "keyboard_key_editor_fast_copy_left",
                 m_key_editor_fast_copy_left);
  Write_Property(stream, "keyboard_key_editor_fast_copy_right",
                 m_key_editor_fast_copy_right);
  Write_Property(stream, "keyboard_key_editor_pixel_move_up",
                 m_key_editor_pixel_move_up);
  Write_Property(stream, "keyboard_key_editor_pixel_move_down",
                 m_key_editor_pixel_move_down);
  Write_Property(stream, "keyboard_key_editor_pixel_move_left",
                 m_key_editor_pixel_move_left);
  Write_Property(stream, "keyboard_key_editor_pixel_move_right",
                 m_key_editor_pixel_move_right);
  // Joystick/Gamepad
  ...
  Write_Property(stream, "joy_button_jump",
                 static_cast<int>(m_joy_button_jump));
  Write_Property(stream, "joy_button_item",
                 static_cast<int>(m_joy_button_item));
  Write_Property(stream, "joy_button_shoot",
                 static_cast<int>(m_joy_button_shoot));
  Write_Property(stream, "joy_button_action",
                 static_cast<int>(m_joy_button_action));
  Write_Property(stream, "joy_button_exit",
                 static_cast<int>(m_joy_button_exit));
  // Special
...
void cPreferences ::Apply_Video(Uint16 screen_w, Uint16 screen_h,
                                Uint8 screen_bpp, bool fullscreen, bool vsync,
                                float geometry_detail, float texture_detail) {
  /* if resolution, bpp, vsync or texture detail changed
   * a texture reload is necessary
   */
  if (m_video_screen_w != screen_w || m_video_screen_h != screen_h ||
      m_video_screen_bpp != screen_bpp || m_video_vsync != vsync ||
      !Is_Float_Equal(pVideo->m_texture_quality, texture_detail)) {
    // new settings
...
void cPreferences ::elementStart(const CEGUI::String &element,
...
void cPreferences ::handle_item(CEGUI::XMLAttributes attributes) {
  ...
  if (name.compare("game_version") == 0) {
    m_game_version =
        string_to_version_number(attributes.getValueAsString("value").c_str());
  } else if (name.compare("game_language") == 0) {
    m_language = attributes.getValueAsString("value").c_str();
  } else if (name.compare("game_always_run") == 0 ||
             name.compare("always_run") == 0) {
  ...
  } else if (name.compare("game_user_data_dir") == 0 ||
             name.compare("user_data_dir") == 0) {
  ...
  } else if (name.compare("game_camera_hor_speed") == 0 ||
             name.compare("camera_hor_speed") == 0) {
    m_camera_hor_speed = attributes.getValueAsFloat("value");
  } else if (name.compare("game_camera_ver_speed") == 0 ||
             name.compare("camera_ver_speed") == 0) {
  ...
  } else if (name.compare("video_geometry_detail") == 0 ||
             name.compare("video_geometry_quality") == 0) {
    pVideo->m_geometry_quality = attributes.getValueAsFloat("value");
  } else if (name.compare("video_texture_detail") == 0 ||
             name.compare("video_texture_quality") == 0) {

...
cSpika ::cSpika(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cSpika::Init();
}
...
void cSpika ::Update(void) {
  ...
  // update rotation
  if (m_velx != 0) {
    Add_Rotation_Z((m_velx / (m_image->m_w * 0.01f)) *
  ...
  if (pLevel_Player->m_maryo_type != MARYO_GHOST &&
      player_rect.Intersects(rect_left)) {
    if (m_velx > -m_speed) {
  ...
  // if player is right
           ...
           player_rect.Intersects(rect_right)) {
    if (m_velx < m_speed) {
      m_velx += m_speed * 0.1f * pFramerate->m_speed_factor;
  ...
  if (obj->m_massive_type == MASS_MASSIVE) {
    ...
    if (obj->m_sprite_array == ARRAY_ENEMY) {
      ...
      if (!Is_Float_Equal(m_velx, 0.0f)) {
        ...
        if (obj->m_type == TYPE_SPIKA &&
            m_speed < static_cast<cSpika *>(obj)->m_speed) {
...
void cSpika ::Handle_Collision_Player(cObjectCollision *collision) {
  ...
  if (collision->m_direction == DIR_LEFT ||
      collision->m_direction == DIR_RIGHT) {
    m_velx = 0.0f;
  }
}

...
cLevel_Manager ::cLevel_Manager(void) : cObject_Manager<cLevel>() {
...
void cLevel_Manager ::Unload(void) {
  ...
  if (size() > 1) {
    for (vector<cLevel *>::iterator itr = objects.begin();
         itr != objects.end() - 1;) {
      cLevel *obj = (*itr);
...
cLevel *cLevel_Manager ::Get(const std::string &str) {
  for (vector<cLevel *>::iterator itr = objects.begin(); itr != objects.end();
       ++itr) {
    cLevel *obj = (*itr);
...
bool cLevel_Manager ::Get_Path(std::string &filename,
                               bool check_only_user_dir /* = 0 */) const {
  ...
  if (!check_only_user_dir) {
    ...
    filename.erase(0, pResource_Manager->user_data_dir.length() +
                          strlen(USER_LEVEL_DIR "/"));
...
void cLevel_Manager ::Finish_Level(bool win_music /* = 0 */) {
  ...
  Game_Action_Data_Start.add(
      "screen_fadeout",
      CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_RANDOM));
  if (win_music) {
...
void cLevel_Manager ::Goto_Sub_Level(
    std::string str_level, const std::string &str_entry,
    Camera_movement move_camera /* = CAMERA_MOVE_FLY */,
    const std::string &path_identifier /* = "" */) {
  ...
  // same level
  if (str_level.compare(Trim_Filename(pActive_Level->m_level_filename, 0, 0)) ==
      0) {
    ...
    if (move_camera != CAMERA_MOVE_NONE) {
      ...
      if (move_camera == CAMERA_MOVE_FLY ||
          move_camera == CAMERA_MOVE_ALONG_PATH ||
          move_camera == CAMERA_MOVE_ALONG_PATH_BACKWARDS) {
        // move along path
        if (move_camera == CAMERA_MOVE_ALONG_PATH ||
          ...
          if (path_state.m_path) {
            ...
            // start path position
            float start_path_pos_x =
                path_state.m_path->m_col_rect.m_x - (game_res_w * 0.5f);
            float start_path_pos_y =
                path_state.m_path->m_col_rect.m_y - (game_res_h * 0.5f);
            // move gradually to start point
            pActive_Camera->Move_to_Position_Gradually(
                start_path_pos_x + path_state.m_pos_x,
                start_path_pos_y + path_state.m_pos_y, 20);
            // reset offset
            ...
            while (1) {
              ...
              pAudio->Update();
              // Move camera
              pActive_Camera->Set_Pos(start_path_pos_x + path_state.m_pos_x,
                                      start_path_pos_y + path_state.m_pos_y);

              // keep particles on screen
              for (cSprite_List::iterator itr =
                       pActive_Level->m_sprite_manager->objects.begin();
                   itr != pActive_Level->m_sprite_manager->objects.end();
                   ++itr) {
                cSprite *obj = (*itr);

                if (obj->m_type == TYPE_PARTICLE_EMITTER) {
                  cParticle_Emitter *emitter =
                      static_cast<cParticle_Emitter *>(obj);
         ...
         */
        pActive_Camera->Move_to_Position_Gradually(
            pActive_Camera->Get_Center_Pos_X(),
            pActive_Camera->Get_Center_Pos_Y());
      }

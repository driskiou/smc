...
cSize_Int
cImage_Settings_Data ::Get_Surface_Size(const SDL_Surface *sdl_surface) const {
  if (!sdl_surface) {
    return cSize_Int();
  }
  ...
  if (m_width > 0 && m_height > 0) {
     ...
     */
    while (new_w * 0.5f > m_width * global_upscalex &&
           new_h * 0.5f > m_height * global_upscaley) {
      new_w *= 0.5f;
      new_h *= 0.5f;
    }
    ...
    if (pVideo->m_texture_quality < 0.25f) {
      ...
      if (new_w * 0.8f > m_width * global_upscalex &&
          new_h * 0.8f > m_height * global_upscaley) {
        new_w *= 0.5f;
        new_h *= 0.5f;
      }
    }
...
void cImage_Settings_Data ::Apply(cGL_Surface *image) const {
  ...
  if (m_rotation_x != 0) {
    ...
    if (image->m_base_rot_x == 180.0f) {
      image->m_col_pos.m_y =
          image->m_h - (image->m_col_h + image->m_col_pos.m_y);
    }
  }
  ...
  if (m_rotation_y != 0) {
    ...
    if (image->m_base_rot_y == 180.0f) {
      image->m_col_pos.m_x =
          image->m_w - (image->m_col_w + image->m_col_pos.m_x);
    }
  }
  ...
  if (m_rotation_z != 0) {
    ...
    // image->m_col_pos = image->m_col_pos.rotate3d( image->m_base_rot_z, 0, 0,
    // 1 );

    if (image->m_base_rot_z == 90.0f) {
      // rotate position
      GL_point pos(image->m_int_x, image->m_int_y);
      pos = pos.rotate(GL_point(image->m_w * 0.5f, image->m_h * 0.5f),
                       image->m_base_rot_z);
...
void cImage_Settings_Data ::Apply_Base(
    const cImage_Settings_Data *base_settings_data) {
...
cImage_Settings_Parser ::cImage_Settings_Parser(void) : cFile_parser() {
...
cImage_Settings_Data *
cImage_Settings_Parser ::Get(const std::string &filename,
                             bool load_base_settings /* = 1 */) {
...
bool cImage_Settings_Parser ::HandleMessage(const std::string *parts,
                                            unsigned int count,
                                            unsigned int line) {
  if (parts[0].compare("base") == 0) {
    if (count < 2 || count > 3) {
      printf("%s : line %d Error :\n", Trim_Filename(data_file, 0, 0).c_str(),
             line);
    ...
    if (!Is_Valid_Number(parts[2])) {
      printf("%s : line %d Error : ", Trim_Filename(data_file, 0, 0).c_str(),
             line);
      printf("%s is not a valid integer value\n", parts[1].c_str());
      return 0;
    }

    m_settings_temp->m_base =
        data_file.substr(0, data_file.rfind("/") + 1) + parts[1];
    ...
    if (count == 3 && string_to_int(parts[2])) {
      ...
      if (m_load_base) {
        ...
        while (!settings_file.empty()) {
          ...
          // handle
          if (base_settings) {
            // todo : apply settings in reverse order ( deepest settings should
            // override first )
            m_settings_temp->Apply_Base(base_settings);

            // if also based on settings
            if (!base_settings->m_base.empty() &&
                base_settings->m_base_settings) {
    ...
    m_settings_temp->m_col_rect =
        GL_rect(static_cast<float>(string_to_int(parts[1])),
                static_cast<float>(string_to_int(parts[2])),
                static_cast<float>(string_to_int(parts[3])),
                static_cast<float>(string_to_int(parts[4])));

...
cRokko ::cRokko(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cRokko::Init();
}
...
void cRokko ::Update_Distance_rect(void) {
  if (m_start_direction == DIR_LEFT) {
    ...
    m_distance_rect.m_y = ((m_col_pos.m_y + m_col_rect.m_h) * 0.5f) -
                          (m_max_distance_sides * 0.5f);
    ...
    m_distance_rect.m_x = ((m_col_pos.m_x + m_col_rect.m_w) * 0.5f) -
                          (m_max_distance_sides * 0.5f);
...
void cRokko ::Generate_Smoke(unsigned int amount /* = 10 */) const {
  ...
  if (!m_dead) {
    if (m_direction == DIR_LEFT) {
      anim->Set_Emitter_Rect(m_pos_x + m_col_rect.m_w - 16, m_pos_y + 20, 6,
                             m_rect.m_h - 40);
      ...
      anim->Set_Emitter_Rect(m_pos_x + 20, m_pos_y + m_col_rect.m_h - 16,
                             m_rect.m_w - 40, 6);
  ...
  // dead
    ...
    anim->Set_Emitter_Rect(m_pos_x + (m_col_rect.m_w * 0.2f),
                           m_pos_y + (m_col_rect.m_h * 0.2f),
                           m_col_rect.m_w * 0.3f, m_col_rect.m_h * 0.3f);
...
void cRokko ::Generate_Sparks(unsigned int amount /* = 5 */) const {
  ...
  cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
  anim->Set_Emitter_Rect(m_pos_x + m_col_rect.m_w * 0.2f,
                         m_pos_y + m_rect.m_h * 0.2f, m_col_rect.m_w * 0.6f,
                         m_rect.m_h * 0.6f);
  anim->Set_Pos_Z(m_pos_z + 0.00001f);
  anim->Set_Quota(amount);
  ...
  anim->Set_Color(Color(static_cast<Uint8>(250), 250, 200),
                  Color(static_cast<Uint8>(5), 5, 0, 0));
...
void cRokko ::Handle_Collision_Player(cObjectCollision *collision) {
  ...
  if (m_direction == DIR_LEFT || m_direction == DIR_RIGHT) {
    ...
    if (collision->m_direction == DIR_TOP &&
      ...
      pHud_Points->Add_Points(m_kill_points, m_pos_x + m_rect.m_w / 3,
                              m_pos_y - 10.0f, "", static_cast<Uint8>(255), 1);
...
void cRokko ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_rokko_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cRokko::Editor_Direction_Select, this));

  // speed
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_rokko_speed"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cRokko::Editor_Speed_Text_Changed, this));

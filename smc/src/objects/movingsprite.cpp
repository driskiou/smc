...
cMovingSprite ::cMovingSprite(cSprite_Manager *sprite_manager,
...
void cMovingSprite ::Set_Image(cGL_Surface *new_image,
                               bool new_start_image /* = 0 */,
                               bool del_img /* = 0 */) {
...
void cMovingSprite ::Set_Direction(const ObjectDirection dir,
                                   bool new_start_direction /* = 0 */) {
  m_direction = dir;

  // turn velocity if wrong
  if ((dir == DIR_LEFT && m_velx > 0.0f) ||
      (dir == DIR_RIGHT && m_velx < 0.0f)) {
    m_velx *= -1;
  }
...
cMovingSprite ::Col_Move_in_Steps(float move_x, float move_y, float step_size_x,
                                  float step_size_y, float final_pos_x,
                                  float final_pos_y, cSprite_List sprite_list,
                                  bool stop_on_internal /* = 0 */) {
  ...
  while (move_x_valid || move_y_valid) {
    if (move_x_valid) {
      ...
      // collision check
      cObjectCollisionType *col_list_temp = Collision_Check_Relative(
          step_size_x, 0.0f, 0.0f, 0.0f, COLLIDE_COMPLETE, &sprite_list);
      ...
      // stop only on blocking
        ...
        // remove internal collision from further checks
          ...
          for (cObjectCollision_List::iterator itr =
                   col_list_temp->objects.begin();
               itr != col_list_temp->objects.end(); ++itr) {
            ...
            // find in sprite list
            cSprite_List::iterator sprite_itr =
                std::find(sprite_list.begin(), sprite_list.end(), col->m_obj);
      ...
      if (col_list_temp->size()) {
        col_list->objects.insert(col_list->objects.end(),
                                 col_list_temp->objects.begin(),
                                 col_list_temp->objects.end());
      ...
      if (!collision_found) {
        ...
        if ((step_size_x > 0.0f && final_pos_x <= m_pos_x) ||
            (step_size_x < 0.0f && final_pos_x >= m_pos_x)) {
          m_pos_x = final_pos_x;
          move_x_valid = 0;
          step_size_x = 0.0f;
        }
    ...
    if (move_y_valid) {
      ...
      // collision check
      cObjectCollisionType *col_list_temp = Collision_Check_Relative(
          0.0f, step_size_y, 0.0f, 0.0f, COLLIDE_COMPLETE, &sprite_list);
      ...
      if (!collision_found) {
        ...
        if ((step_size_y > 0.0f && final_pos_y <= m_pos_y) ||
            (step_size_y < 0.0f && final_pos_y >= m_pos_y)) {
          m_pos_y = final_pos_y;
          move_y_valid = 0;
          step_size_y = 0.0f;
        }
...
void cMovingSprite ::Col_Move(float move_x, float move_y, bool real /* = 0 */,
                              bool force /* = 0 */,
  ...
  if (Is_Float_Equal(m_col_rect.m_w, 0.0f) ||
      Is_Float_Equal(m_col_rect.m_h, 0.0f)) {
    return;
  }
  ...
  if (!force) {
    ...
    m_sprite_manager->Get_Colliding_Objects(sprite_list, complete_rect, 1,
                                            this);
    ...
    cObjectCollisionType *col_list =
        Col_Move_in_Steps(move_x, move_y, step_size_x, step_size_y, final_pos_x,
                          final_pos_y, sprite_list, 1);
    ...
    if (col_list && col_list->size()) {
      ...
      delete col_list;
      col_list = Col_Move_in_Steps(move_x, move_y, step_size_x, step_size_y,
                                   final_pos_x, final_pos_y, sprite_list);
...
void cMovingSprite ::Add_Velocity(const float x, const float y,
                                  const bool real /* = 0 */) {
  if (real) {
    m_velx += x;
    m_vely += y;
...
void cMovingSprite ::Turn_Around(
    ObjectDirection col_dir /* = DIR_UNDEFINED */) {
  // check if the collision direction is not in front
  if (col_dir != DIR_UNDEFINED && m_direction != DIR_UNDEFINED &&
      m_direction != col_dir) {
    return;
  }
...
cObjectCollisionType *cMovingSprite ::Collision_Check_Absolute(
    const float x, const float y, const float w /* = 0 */,
    const float h /* = 0 */,
    const ColCheckType check_type /* = COLLIDE_COMPLETE */,
    cSprite_List *objects /* = NULL */) {
  // save original rect
  GL_rect new_rect;
...
cObjectCollisionType *cMovingSprite ::Collision_Check(
    const GL_rect &new_rect,
    const ColCheckType check_type /* = COLLIDE_COMPLETE */,
    cSprite_List *objects /* = NULL */) {
  ...
  if (Is_Float_Equal(new_rect.m_w, 0.0f) ||
      Is_Float_Equal(new_rect.m_h, 0.0f)) {
    return col_list;
  }
  ...
  if (!objects) {
    ...
    if (m_type != TYPE_PLAYER &&
        new_rect.Intersects(pActive_Player->m_col_rect)) {
      // validate
  ...
  for (cSprite_List::iterator itr = objects->begin(); itr != objects->end();
    ...
    if (level_object->m_sprite_array == ARRAY_UNDEFINED ||
        level_object->m_sprite_array == ARRAY_HUD ||
        level_object->m_sprite_array == ARRAY_ANIM) {
      continue;
    }

    // if enemy is dead
    if (level_object->m_sprite_array == ARRAY_ENEMY &&
        static_cast<cEnemy *>(level_object)->m_dead) {
      continue;
    }
...
void cMovingSprite ::Check_And_Handle_Out_Of_Level(const float move_x,
                                                   const float move_y) {
...
bool cMovingSprite ::Is_Out_Of_Level_Left(const float move_x) const {
  if (m_col_rect.m_x < pActive_Camera->m_limit_rect.m_x &&
      m_col_rect.m_x - (move_x - 0.00001f) >=
          pActive_Camera->m_limit_rect.m_x) {
    return 1;
  }
...
bool cMovingSprite ::Is_Out_Of_Level_Right(const float move_x) const {
  if (m_col_rect.m_x + m_col_rect.m_w >
          pActive_Camera->m_limit_rect.m_x + pActive_Camera->m_limit_rect.m_w &&
      m_col_rect.m_x + m_col_rect.m_w - (move_x + 0.00001f) <=
          pActive_Camera->m_limit_rect.m_x + pActive_Camera->m_limit_rect.m_w) {
    return 1;
  }
...
bool cMovingSprite ::Is_Out_Of_Level_Top(const float move_y) const {
  if (m_col_rect.m_y <
      ...
      m_col_rect.m_y - (move_y - 0.00001f) >=
          pActive_Camera->m_limit_rect.m_h + pActive_Camera->m_limit_rect.m_y) {
    return 1;
  }
...
bool cMovingSprite ::Is_Out_Of_Level_Bottom(const float move_y) const {
  if (m_col_rect.m_y + m_col_rect.m_h > pActive_Camera->m_limit_rect.m_y &&
      m_col_rect.m_y + m_col_rect.m_h - (move_y + 0.00001f) <=
          pActive_Camera->m_limit_rect.m_y) {
    return 1;
  }
...
void cMovingSprite ::Check_on_Ground(void) {
  ...
  if (m_type != TYPE_PLAYER && m_sprite_array != ARRAY_ENEMY &&
  ...
  // if ground object
  if (m_ground_object) {
    GL_rect rect2(m_col_rect.m_x, m_col_rect.m_y + m_col_rect.m_h,
                  m_col_rect.m_w, 1.0f);

    // if on ground object
    if (m_ground_object->m_col_rect.Intersects(rect2) &&
        m_ground_object->m_can_be_ground) {
      return;
    }
  }
  ...
  cObjectCollisionType *col_list = Collision_Check_Relative(
      0.0f, m_col_rect.m_h, 0.0f, 1.0f, COLLIDE_ONLY_BLOCKING);
...
void cMovingSprite ::Update_Anti_Stuck(void) {
  // collision count
  cObjectCollisionType *col_list =
      Collision_Check(&m_col_rect, COLLIDE_ONLY_BLOCKING);
  ...
  for (cObjectCollision_List::iterator itr = col_list->objects.begin();
    ...
    if (collision->m_array == ARRAY_ENEMY ||
        (collision->m_array == ARRAY_ACTIVE &&
         (col_obj->m_massive_type == MASS_HALFMASSIVE ||
          col_obj->m_massive_type == MASS_CLIMBABLE))) {
      continue;
    }

    debug_print("Anti Stuck detected object %s on %s side\n",
                col_obj->m_name.c_str(),
...
void cMovingSprite ::Move_With_Ground(void) {
  if (!m_ground_object ||
      (m_ground_object->m_sprite_array != ARRAY_ACTIVE &&
       m_ground_object->m_sprite_array !=
           ARRAY_ENEMY)) // || m_ground_object->sprite_array == ARRAY_MASSIVE
  {
    return;
  }

  cMovingSprite *moving_ground_object =
      dynamic_cast<cMovingSprite *>(m_ground_object);
  ...
  // does not move
  if (Is_Float_Equal(moving_ground_object->m_velx, 0.0f) &&
      Is_Float_Equal(moving_ground_object->m_vely, 0.0f)) {
    return;
  }
  ...
  /* stop object from getting stopped of the moving object which did not yet
   * move itself for example the player always moves as last
   */
  bool is_massive = 0;
  ...
  if (moving_ground_object->m_vely < -0.01f) {
    // test if we could not move upwards because something did block us in
    // Col_Move()
...
void cMovingSprite ::Handle_Move_Object_Collision(
    const cObjectCollision *collision) {
  // if not massive
  ...
  if (collision->m_array == ARRAY_ENEMY) {
    obj = static_cast<cMovingSprite *>(
        m_sprite_manager->Get_Pointer(collision->m_number));

    // ignore these enemies
    if (obj->m_type == TYPE_THROMP || obj->m_type == TYPE_EATO ||
        obj->m_type == TYPE_FLYON || obj->m_type == TYPE_STATIC_ENEMY) {
      return;
    }
  ...
  if (collision->m_direction == DIR_BOTTOM) {
    if (obj->m_ground_object &&
        obj->m_ground_object->m_massive_type == MASS_MASSIVE) {
      // got crunched
      obj->DownGrade(1);
    } else {
      // move
  ...
  } else if ((collision->m_direction == DIR_LEFT && m_velx < 0.0f) ||
             (collision->m_direction == DIR_RIGHT && m_velx > 0.0f)) {
    // save posx for possible can not move test
...
Col_Valid_Type
cMovingSprite ::Validate_Collision_Object_On_Top(cMovingSprite *moving_sprite) {
  ...
  if (moving_sprite->Is_On_Top(this) && m_direction == DIR_UP &&
      moving_sprite->m_vely > m_vely && moving_sprite->m_can_be_on_ground) {
    // halfmassive
...
void cMovingSprite ::Send_Collision(const cObjectCollision *collision,
                                    bool handle_now /* = 0 */) {
  // empty collision
  if (!collision) {
    return;
  ...
  // get sprite array number
    ...
    if (my_number < 0) {
      debug_print("Warning : Object %s did send Collision but doesn't exists "
                  "in Manager\n",
                  m_name.c_str());
      return;
    }

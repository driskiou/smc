...
void cEnemy ::Load_From_Savegame(cSave_Level_Object *save_object) {
  ...
  if (save_object->exists("state")) {
    m_state = static_cast<Moving_state>(
        string_to_int(save_object->Get_Value("state")));
  }
...
cSave_Level_Object *cEnemy ::Save_To_Savegame(void) {
  ...
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("state", int_to_string(m_state)));
  ...
  if (m_dead) {
    save_object->m_properties.push_back(
        cSave_Level_Object_Property("dead", int_to_string(m_dead)));
  }
...
void cEnemy ::Update(void) {
  ...
  if (m_freeze_counter) {
    // update gravity
    if (m_type == TYPE_FURBALL || m_type == TYPE_TURTLE ||
        m_type == TYPE_KRUSH || m_type == TYPE_SPIKA ||
...
void cEnemy ::Generate_Hit_Animation(
    cParticle_Emitter *anim /* = NULL */) const {
  bool create_anim = 0;
  ...
  anim->Set_Emitter_Rect(m_col_rect.m_x, m_pos_y + (m_col_rect.m_h / 3),
                         m_col_rect.m_w);
  ...
  Color col_rand =
      Color(static_cast<Uint8>(rand() % 5), rand() % 5, rand() % 100, 0);
  // not bright enough
...
void cEnemy ::Handle_out_of_Level(ObjectDirection dir) {
  if (dir == DIR_LEFT) {
    ...
    Set_Pos_X(pActive_Camera->m_limit_rect.m_x +

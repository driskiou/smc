...
void cStaticEnemy ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // rotation speed
  Set_Rotation_Speed(
      static_cast<float>(attributes.getValueAsFloat("rotation_speed", -7.5f)));
  // image
  Set_Static_Image(
      attributes.getValueAsString("image", "enemy/static/saw/default.png")
          .c_str());
  // path
  ...
  // fire resistant
  m_fire_resistant =
      attributes.getValueAsBool("fire_resistant", m_fire_resistant);
  // ice resistance
  m_ice_resistance = static_cast<float>(
      attributes.getValueAsFloat("ice_resistance", m_ice_resistance));
}
...
void cStaticEnemy ::Set_Static_Image(const std::string &filename) {
  if (filename.empty() ||
      (!File_Exists(filename) &&
       !File_Exists(DATA_DIR "/" GAME_PIXMAPS_DIR "/" + filename))) {
    return;
  }
...
void cStaticEnemy ::Set_Speed(float speed) { m_speed = speed; }
...
void cStaticEnemy ::Update(void) {
  ...
  if (m_path_state.m_path) {
    ...
    // get difference
    float diff_x =
        (m_path_state.m_path->m_start_pos_x + m_path_state.m_pos_x) - m_pos_x;
    float diff_y =
        (m_path_state.m_path->m_start_pos_y + m_path_state.m_pos_y) - m_pos_y;
...
void cStaticEnemy ::Handle_Collision_Enemy(cObjectCollision *collision) {
  ...
  pHud_Points->Add_Points(enemy->m_kill_points, m_pos_x, m_pos_y - 5.0f, "",
                          static_cast<Uint8>(255), 1);
  enemy->DownGrade(1);
}
...
void cStaticEnemy ::Editor_Activate(void) {
  ...
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_static_enemy_image"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cStaticEnemy::Editor_Image_Text_Changed, this));

  // rotation speed
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_static_enemy_rotation_speed"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cStaticEnemy::Editor_Rotation_Speed_Text_Changed, this));

  // path
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_static_enemy_path_identifier"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cStaticEnemy::Editor_Path_Identifier_Text_Changed, this));

  // speed
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_static_enemy_speed"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cStaticEnemy::Editor_Speed_Text_Changed, this));

  // fire resistant
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_static_enemy_fire_resistant"));
  Editor_Add(UTF8_("Fire Resistant"), UTF8_("If it is resistant against fire"),
             combobox, 120, 80);
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cStaticEnemy::Editor_Fire_Resistant_Select,
                               this));

  // ice resistance
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_static_enemy_ice_resistance"));
  Editor_Add(UTF8_("Ice Resistance"), UTF8_("Resistance against Ice (0.0-1.0)"),
             editbox, 120);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cStaticEnemy::Editor_Ice_Resistance_Text_Changed, this));
...
bool cStaticEnemy ::Editor_Rotation_Speed_Text_Changed(
...
bool cStaticEnemy ::Editor_Path_Identifier_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cStaticEnemy ::Editor_Fire_Resistant_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
...
bool cStaticEnemy ::Editor_Ice_Resistance_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =

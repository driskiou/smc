...
cEato ::cEato(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cEato::Init();
}
...
void cEato ::Set_Image_Dir(std::string dir) {
  ...
  if (!File_Exists(DATA_DIR "/" GAME_PIXMAPS_DIR "/" + dir + "1.png") &&
      !File_Exists(DATA_DIR "/" GAME_PIXMAPS_DIR "/" + dir + "1.settings")) {
...
void cEato ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_eato_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cEato::Editor_Direction_Select, this));

  // image dir
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_eato_image_dir"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cEato::Editor_Image_Dir_Text_Changed, this));
  // init
  Editor_Init();
}

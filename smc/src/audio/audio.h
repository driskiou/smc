...
public:
   ...
   * loops : if set to -1 loops indefinitely or if greater than zero, loop the
   * sound that many times.
   */
...
public:
  ...
  /* Check if the sound was already loaded and returns a pointer to it else it
   * will be loaded. The returned sound should not be deleted or modified.
   */
  cSound *Get_Sound_File(std::string filename) const;

  // Play the given sound
  bool Play_Sound(std::string filename, int res_id = -1, int volume = -1,
                  int loops = 0);
  // If no forcing it will be played after the current music
  bool Play_Music(std::string filename, int loops = 0, bool force = 1,
   ...
   */
  void Fadeout_Sounds(unsigned int ms = 200, int channel = -1,
   ...
   * overwrite_fading : overwrite an already existing fade out
   */
  void Fadeout_Sounds(unsigned int ms, std::string filename,

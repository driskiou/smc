...
class GL_rect {
public:
  GL_rect(void) : m_x(0), m_y(0), m_w(0), m_h(0) {}
  ...
  // unary - operator
  inline GL_rect operator-() const { return GL_rect(-m_x, -m_y, -m_w, -m_h); }
  ...
  inline bool operator==(const GL_rect &r) const {
    return (Is_Float_Equal(m_x, r.m_x) && Is_Float_Equal(m_y, r.m_y) &&
            Is_Float_Equal(m_w, r.m_w) && Is_Float_Equal(m_h, r.m_h));
  }

  inline bool operator!=(const GL_rect &r) const { return !(operator==(r)); }

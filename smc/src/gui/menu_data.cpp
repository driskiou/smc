...
cMenu_Base ::~cMenu_Base(void) {
  if (m_gui_window) {
    CEGUI::System::getSingleton()
        ...
        .getRootWindow()
        ->removeChild(m_gui_window);
    CEGUI::WindowManager::getSingleton().destroyWindow(m_gui_window);
  }

  for (HudSpriteList::iterator itr = m_draw_list.begin();
...
void cMenu_Base ::Init(void) { m_layout_file = ""; }
...
void cMenu_Base ::Init_GUI(void) {
  ...
  m_gui_window =
      CEGUI::WindowManager::getSingleton().loadLayoutFromFile(m_layout_file);
  CEGUI::System::getSingleton()
      .getDefaultGUIContext()
      .getRootWindow()
      ->addChild(m_gui_window);
}
...
void cMenu_Base ::Update(void) {
  if (m_exit_to_gamemode != MODE_LEVEL &&
      m_exit_to_gamemode != MODE_OVERWORLD) {
    // animation
...
cMenu_Main ::cMenu_Main(void) : cMenu_Base() {}
...
void cMenu_Main ::Init(void) {
  ...
  temp_item->m_image_menu->Set_Pos(
      temp_item->m_pos_x + (temp_item->m_image_default->m_col_rect.m_w + 16),
      temp_item->m_pos_y);
  ...
  temp_item->m_image_menu->Set_Pos(
      temp_item->m_pos_x - temp_item->m_image_menu->m_col_rect.m_w - 16,
      temp_item->m_pos_y);
  pMenuCore->m_handler->Add_Menu_Item(temp_item);
  // Load
  m_menu_pos_y += 60;
  ...
  temp_item->m_image_menu->Set_Pos(
      temp_item->m_pos_x + temp_item->m_col_rect.m_w + 16, temp_item->m_pos_y);
  pMenuCore->m_handler->Add_Menu_Item(temp_item);

  if (m_exit_to_gamemode == MODE_NOTHING) {
    // Credits
    cGL_Surface *credits =
        pFont->Render_Text(pFont->m_font_normal, _("Credits"), yellow);
    temp_item = new cMenu_Item(pMenuCore->m_handler->m_level->m_sprite_manager);
    temp_item->m_image_default->Set_Image(credits);
    temp_item->Set_Pos(static_cast<float>(game_res_w) * 0.45f,
                       static_cast<float>(game_res_h) - 30.0f);
    pMenuCore->m_handler->Add_Menu_Item(temp_item, 1.5f, grey);

    cHudSprite *hud_sprite =
        new cHudSprite(pMenuCore->m_handler->m_level->m_sprite_manager);
    ...
    hud_sprite->Set_Pos(static_cast<float>(game_res_w) * 0.04f,
                        static_cast<float>(game_res_h) * 0.935f);
    m_draw_list.push_back(hud_sprite);
  }
...
void cMenu_Main ::Init_GUI(void) {
  ...
  CEGUI::Window *text_version = CEGUI::System::getSingleton()
                                    .getDefaultGUIContext()
                                    .getRootWindow()
                                    ->getChildRecursive("text_version");
  text_version->setProperty(
      "Text",
      UTF8_("Version ") +
          CEGUI::PropertyHelper<int>::toString(SMC_VERSION_MAJOR) + "." +
          CEGUI::PropertyHelper<int>::toString(SMC_VERSION_MINOR) + "." +
          CEGUI::PropertyHelper<int>::toString(SMC_VERSION_PATCH));

  // if in a level/world
  if (m_exit_to_gamemode != MODE_NOTHING) {
    CEGUI::Window *text_website = CEGUI::System::getSingleton()
                                      .getDefaultGUIContext()
                                      .getRootWindow()
                                      ->getChildRecursive("text_website");
    text_website->hide();
  }
}
...
void cMenu_Main ::Update(void) {
  ...
  if (m_exit_to_gamemode != MODE_NOTHING) {
    Game_Action_Data_Middle.add("menu_exit_back_to",
                                int_to_string(m_exit_to_gamemode));
  }
}
...
cMenu_Start ::cMenu_Start(void) : cMenu_Base() {}
...
void cMenu_Start ::Init_GUI(void) {
  ...
  CEGUI::TabControl *tabcontrol = static_cast<CEGUI::TabControl *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("tabcontrol_main"));
  tabcontrol->activate();

  // events
  tabcontrol->subscribeEvent(
      CEGUI::TabControl::EventSelectionChanged,
      CEGUI::Event::Subscriber(&cMenu_Start::TabControl_Selection_Changed,
                               this));
  tabcontrol->subscribeEvent(
      CEGUI::Window::EventKeyDown,
      CEGUI::Event::Subscriber(&cMenu_Start::TabControl_Keydown, this));

  // ### Campaign ###
  CEGUI::Listbox *listbox_campaigns = static_cast<CEGUI::Listbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("listbox_campaigns"));

  // campaign names
  for (vector<cCampaign *>::const_iterator itr =
           pCampaign_Manager->objects.begin();
       itr != pCampaign_Manager->objects.end(); ++itr) {
    const cCampaign *campaign = (*itr);

    CEGUI::ListboxTextItem *item = new CEGUI::ListboxTextItem(
        reinterpret_cast<const CEGUI::utf8 *>(campaign->m_name.c_str()));
    // is in game dir
    ...
    // is in both
      ...
      // mix colors
      item->setTextColours(
          CEGUI::Colour(0.8f, 1, 0.6f), CEGUI::Colour(0.8f, 1, 0.6f),
          CEGUI::Colour(1, 0.8f, 0.6f), CEGUI::Colour(1, 0.8f, 0.6f));
    }
  ...
  // events
  listbox_campaigns->subscribeEvent(
      CEGUI::Window::EventKeyDown,
      CEGUI::Event::Subscriber(&cMenu_Start::Listbox_Keydown, this));
  listbox_campaigns->subscribeEvent(
      CEGUI::Window::EventCharacterKey,
      CEGUI::Event::Subscriber(&cMenu_Start::Listbox_Character_Key, this));
  listbox_campaigns->subscribeEvent(
      CEGUI::Listbox::EventSelectionChanged,
      CEGUI::Event::Subscriber(&cMenu_Start::Campaign_Select, this));
  listbox_campaigns->subscribeEvent(
      CEGUI::Listbox::EventMouseDoubleClick,
      CEGUI::Event::Subscriber(&cMenu_Start::Campaign_Select_final_list, this));
  ...
  // ### World ###
  CEGUI::Listbox *listbox_worlds =
      static_cast<CEGUI::Listbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("listbox_worlds"));

  // overworld names
  for (vector<cOverworld *>::const_iterator itr =
           pOverworld_Manager->objects.begin();
       itr != pOverworld_Manager->objects.end(); ++itr) {
...
// show all worlds in debug builds
    ...
    CEGUI::ListboxTextItem *item = new CEGUI::ListboxTextItem(
        reinterpret_cast<const CEGUI::utf8 *>(world->m_name.c_str()));
    // is in game dir
    if (world->m_user == 0) {
      item->setTextColours(CEGUI::Colour(1, 0.8f, 0.6f));
    }
  ...
  listbox_worlds->subscribeEvent(
      CEGUI::Listbox::EventSelectionChanged,
      CEGUI::Event::Subscriber(&cMenu_Start::World_Select, this));
  listbox_worlds->subscribeEvent(
      CEGUI::Listbox::EventMouseDoubleClick,
      CEGUI::Event::Subscriber(&cMenu_Start::World_Select_final_list, this));
  ...
  // ### Level ###
  CEGUI::Listbox *listbox_levels =
      static_cast<CEGUI::Listbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("listbox_levels"));
  listbox_levels->setSortingEnabled(1);

  // get game level
  Get_Levels(DATA_DIR "/" GAME_LEVEL_DIR, CEGUI::Colour(1, 0.8f, 0.6f));
  // get user level
  Get_Levels(pResource_Manager->user_data_dir + USER_LEVEL_DIR,
             CEGUI::Colour(0.8f, 1, 0.6f));

  // events
  listbox_levels->subscribeEvent(
      CEGUI::Listbox::EventSelectionChanged,
      CEGUI::Event::Subscriber(&cMenu_Start::Level_Select, this));
  listbox_levels->subscribeEvent(
      CEGUI::Listbox::EventMouseDoubleClick,
      CEGUI::Event::Subscriber(&cMenu_Start::Level_Select_Final_List, this));
  listbox_levels->subscribeEvent(
      CEGUI::Window::EventKeyDown,
      CEGUI::Event::Subscriber(&cMenu_Start::Listbox_Keydown, this));
  listbox_levels->subscribeEvent(
  ...
  // Level Buttons
  CEGUI::PushButton *button_new = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("button_level_new"));
  button_new->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Start::Button_Level_New_Clicked, this));
  CEGUI::PushButton *button_edit = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("button_level_edit"));
  button_edit->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Start::Button_Level_Edit_Clicked, this));
  CEGUI::PushButton *button_delete = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("button_level_delete"));
  button_delete->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Start::Button_Level_Delete_Clicked,
                               this));

  // Button Enter
  CEGUI::PushButton *button_enter =
      static_cast<CEGUI::PushButton *>(CEGUI::System::getSingleton()
                                           .getDefaultGUIContext()
                                           .getRootWindow()
                                           ->getChildRecursive("button_enter"));
  // Button back
  CEGUI::PushButton *button_back =
      static_cast<CEGUI::PushButton *>(CEGUI::System::getSingleton()
                                           .getDefaultGUIContext()
                                           .getRootWindow()
                                           ->getChildRecursive("button_back"));

  // events
  button_enter->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Start::Button_Enter_Clicked, this));
  button_back->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Start::Button_Back_Clicked, this));
...
void cMenu_Start ::Get_Levels(std::string dir, CEGUI::Colour color) {
  ...
  for (vector<std::string>::iterator itr = lvl_files.begin();
       itr != lvl_files.end(); ++itr) {
    // get filename
    ...
    // create listbox item
    CEGUI::ListboxTextItem *item = new CEGUI::ListboxTextItem(
        reinterpret_cast<const CEGUI::utf8 *>(lvl_name.c_str()));
    item->setTextColours(color);

    // check if item with the same name already exists
    CEGUI::ListboxTextItem *item_old = static_cast<CEGUI::ListboxTextItem *>(
        listbox_levels->findItemWithText(lvl_name, NULL));

    if (item_old) {
      // mix colors
      item->setTextColours(item->getTextColours().d_top_left,
                           item->getTextColours().d_top_right,
                           item_old->getTextColours().d_bottom_left,
                           item_old->getTextColours().d_bottom_right);
      // remove old item
...
void cMenu_Start ::Load_Selected(void) {
  ...
  // World
    ...
    CEGUI::ListboxItem *item = (static_cast<CEGUI::Listbox *>(
                                    CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("listbox_worlds")))
...
void cMenu_Start ::Load_Campaign(std::string name) {
  if (pLevel_Player->m_points > 0 &&
      !Box_Question(_("This will reset your current progress.\nContinue ?"))) {
    return;
  }
  ...
  // if not available
  if (!new_campaign) {
    pHud_Debug->Set_Text(_("Couldn't load campaign ") + name,
                         static_cast<float>(speedfactor_fps));
  } else {
    // enter level
    ...
    // enter world
      ...
      Game_Action = GA_ENTER_WORLD;
      Game_Action_Data_Middle.add("enter_world",
                                  new_campaign->m_target.c_str());
    }

    Game_Action_Data_Start.add("music_fadeout", "1000");
    Game_Action_Data_Start.add(
        "screen_fadeout",
        CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_BLACK));
    Game_Action_Data_Start.add("screen_fadeout_speed", "3");
    Game_Action_Data_Middle.add("unload_menu", "1");
    Game_Action_Data_Middle.add("reset_save", "1");
    Game_Action_Data_End.add(
        "screen_fadein",
        CEGUI::PropertyHelper<int>::toString(EFFECT_IN_RANDOM));
...
void cMenu_Start ::Load_World(std::string name) {
  ...
  // if not available
  if (!new_world) {
    pHud_Debug->Set_Text(_("Couldn't load overworld ") + name,
                         static_cast<float>(speedfactor_fps));
  } else {
    // enter world
    Game_Action = GA_ENTER_WORLD;
...
bool cMenu_Start ::Load_Level(std::string level_name) {
  ...
  if (!pLevel_Manager->Get_Path(level_name)) {
    ...
    pHud_Debug->Set_Text(_("Couldn't load level ") + level_name,
                         static_cast<float>(speedfactor_fps));
    return 0;
  }
  ...
  Game_Action_Data_Start.add("music_fadeout", "1000");
  Game_Action_Data_Start.add(
      "screen_fadeout", CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_BLACK));
  Game_Action_Data_Start.add("screen_fadeout_speed", "3");
  Game_Action_Data_Middle.add("load_level", level_name.c_str());
  Game_Action_Data_Middle.add("unload_menu", "1");
  Game_Action_Data_Middle.add("reset_save", "1");
  Game_Action_Data_End.add(
      "screen_fadein", CEGUI::PropertyHelper<int>::toString(EFFECT_IN_RANDOM));
  Game_Action_Data_End.add("screen_fadein_speed", "3");
...
bool cMenu_Start ::TabControl_Selection_Changed(const CEGUI::EventArgs &e) {
  ...
  CEGUI::TabControl *tabcontrol =
      static_cast<CEGUI::TabControl *>(windowEventArgs.window);
...
bool cMenu_Start ::TabControl_Keydown(const CEGUI::EventArgs &e) {
  ...
  // Return
  if (ke.scancode == CEGUI::Key::Return ||
      ke.scancode == CEGUI::Key::NumpadEnter) {
    Load_Selected();
    return 1;
  }
  // Left (todo: only for joystick when CEGUI supports these events)
  else if (ke.scancode ==
           pKeyboard->SDL_Keycode_to_CEGUIKey(pPreferences->m_key_left)) {
    // Get Tab Control
    CEGUI::TabControl *tabcontrol = static_cast<CEGUI::TabControl *>(
        CEGUI::System::getSingleton()
            .getDefaultGUIContext()
  ...
  // Right (todo: only for joystick when CEGUI supports these events)
           ...
           pKeyboard->SDL_Keycode_to_CEGUIKey(pPreferences->m_key_right)) {
    // Get Tab Control
    CEGUI::TabControl *tabcontrol = static_cast<CEGUI::TabControl *>(
        CEGUI::System::getSingleton()
            .getDefaultGUIContext()
...
bool cMenu_Start ::Listbox_Keydown(const CEGUI::EventArgs &e) {
  ...
  // Down/Up (todo: detect event for joystick properly when CEGUI supports these
  // events)
  if (ke.scancode == CEGUI::Key::ArrowDown ||
      ke.scancode == CEGUI::Key::ArrowUp ||
      ke.scancode == CEGUI::Key::PageDown ||
      ke.scancode == CEGUI::Key::PageUp || ke.scancode == CEGUI::Key::Home ||
      ke.scancode == CEGUI::Key::End ||
      ke.scancode ==
          pKeyboard->SDL_Keycode_to_CEGUIKey(pPreferences->m_key_up) ||
      ke.scancode ==
          pKeyboard->SDL_Keycode_to_CEGUIKey(pPreferences->m_key_down)) {
    int new_selected = 0;
    int last_selected = 0;
    ...
    // down (todo: detect event for joystick properly when CEGUI supports these
    // events)
    if (ke.scancode == CEGUI::Key::ArrowDown ||
        ke.scancode ==
            pKeyboard->SDL_Keycode_to_CEGUIKey(pPreferences->m_key_down)) {
    ...
    // up (todo: detect event for joystick properly when CEGUI supports these
    // events)
...
bool cMenu_Start ::Listbox_Character_Key(const CEGUI::EventArgs &e) {
  ...
  if (listbox->getFont()->isCodepointAvailable(ke.codepoint)) {
    ...
    m_listbox_search_buffer.insert(m_listbox_search_buffer.end(), 1,
                                   ke.codepoint);
    ...
    while (index < listbox->getItemCount()) {
      ...
      // found
      if (item->getText()
              .substr(0, m_listbox_search_buffer.length())
              .compare(m_listbox_search_buffer) == 0) {
        new_selected = item;
        break;
      }
      // no match
...
bool cMenu_Start ::Campaign_Select(const CEGUI::EventArgs &event) {
  ...
  CEGUI::ListboxItem *item =
      static_cast<CEGUI::Listbox *>(windowEventArgs.window)
          ->getFirstSelectedItem();

  // description
  CEGUI::Editbox *editbox_campaign_description = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editbox_campaign_description"));

  // set description
  if (item) {
    // todo : should be from the filename not name (more unique)
    editbox_campaign_description->setText(reinterpret_cast<const CEGUI::utf8 *>(
        pCampaign_Manager->Get_from_Name(item->getText().c_str())
            ->m_description.c_str()));
  }
  // clear
  else {
...
bool cMenu_Start ::World_Select(const CEGUI::EventArgs &event) {
  ...
  // description
  CEGUI::Editbox *editbox_world_description = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editbox_world_description"));

  // set description
  if (item) {
    // todo : should be from the path not name (more unique)
    editbox_world_description->setText(reinterpret_cast<const CEGUI::utf8 *>(
        pOverworld_Manager->Get_from_Name(item->getText().c_str())
            ->m_description->m_comment.c_str()));
  }
  // clear
  else {
...
cMenu_Options ::cMenu_Options(void) : cMenu_Base() {}
...
void cMenu_Options ::Init_GUI(void) {
  ...
  button_back->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Button_Back_Click, this));

  // Tab Control
  m_tabcontrol = static_cast<CEGUI::TabControl *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("tabcontrol_main"));
...
void cMenu_Options ::Init_GUI_Game(void) {
  ...
  // always run
  CEGUI::Window *text_always_run = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_text_always_run"));
  text_always_run->setText(UTF8_("Always Run"));

  m_game_combo_always_run = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_combo_always_run"));
  ...
  m_game_combo_always_run->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Always_Run_Select, this));

  // Camera Horizontal Speed
  CEGUI::Window *text_camera_hor_speed = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_text_camera_hor_speed"));
  text_camera_hor_speed->setText(UTF8_("Camera Hor Speed"));

  m_game_spinner_camera_hor_speed = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_spinner_camera_hor_speed"));
  m_game_spinner_camera_hor_speed->setCurrentValue(
      pLevel_Manager->m_camera->m_hor_offset_speed);

  m_game_spinner_camera_hor_speed->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Camera_Hor_Select, this));

  // Camera Vertical Speed
  CEGUI::Window *text_camera_ver_speed = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_text_camera_ver_speed"));
  text_camera_ver_speed->setText(UTF8_("Camera Ver Speed"));

  m_game_spinner_camera_ver_speed = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_spinner_camera_ver_speed"));
  m_game_spinner_camera_ver_speed->setCurrentValue(
      pLevel_Manager->m_camera->m_ver_offset_speed);

  m_game_spinner_camera_ver_speed->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Camera_Ver_Select, this));

  // language
  CEGUI::Window *text_language = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_text_language"));
  text_language->setText(UTF8_("Language"));

  m_game_combo_language = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_combo_language"));
  ...
  vector<std::string> language_files =
      Get_Directory_Files(DATA_DIR "/" GAME_TRANSLATION_DIR, ".none", 1, 0);
  // add english as it is the base language and not in the translation directory
  language_files.push_back(DATA_DIR "/" GAME_TRANSLATION_DIR "/"
                                    "en");

  for (vector<std::string>::iterator itr = language_files.begin();
       itr != language_files.end(); ++itr) {
    // get filename
  ...
  m_game_combo_language->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Language_Select, this));

  // menu level
  CEGUI::Window *text_menu_level = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_text_menu_level"));
  text_menu_level->setText(UTF8_("Menu Level"));

  m_game_combo_menu_level = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_combo_menu_level"));
  ...
  m_game_combo_menu_level->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Menu_Level_Select, this));
  m_game_combo_menu_level->getEditbox()->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Menu_Level_Text_Changed,
                               this));

  // Reset Game
  CEGUI::PushButton *button_reset_game = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("game_button_reset"));
  button_reset_game->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Button_Reset_Game_Clicked,
                               this));
...
void cMenu_Options ::Init_GUI_Video(void) {
  ...
  // Resolution
  CEGUI::Window *text_resolution = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_text_resolution"));
  text_resolution->setText(UTF8_("Resolution"));

  m_video_combo_resolution = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_combo_resolution"));

  vector<cSize_Int> valid_resolutions = pVideo->Get_Supported_Resolutions();
  CEGUI::ListboxTextItem *item;

  // add to listbox
  for (vector<cSize_Int>::iterator itr = valid_resolutions.begin();
       itr != valid_resolutions.end(); ++itr) {
    // get resolution
    cSize_Int res = (*itr);
    ...
    float ar =
        static_cast<float>(res.m_width) / static_cast<float>(res.m_height);

    item = new CEGUI::ListboxTextItem(int_to_string(res.m_width) + "x" +
                                      int_to_string(res.m_height));
  ...
  std::string temp = int_to_string(pPreferences->m_video_screen_w) + "x" +
                     int_to_string(pPreferences->m_video_screen_h);
  m_video_combo_resolution->setText(temp.c_str());

  m_video_combo_resolution->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Resolution_Select, this));

  // Bpp
  CEGUI::Window *text_bpp =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("video_text_bpp"));
  text_bpp->setText(UTF8_("Bpp"));

  m_video_combo_bpp = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_combo_bpp"));
  ...
  m_video_combo_bpp->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Bpp_Select, this));

  // Fullscreen
  CEGUI::Window *text_fullscreen = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_text_fullscreen"));
  text_fullscreen->setText(UTF8_("Fullscreen"));

  m_video_combo_fullscreen = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_combo_fullscreen"));
  ...
  m_video_combo_fullscreen->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Fullscreen_Select, this));

  // VSync
  CEGUI::Window *text_vsync =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("video_text_vsync"));
  text_vsync->setText(UTF8_("VSync"));

  m_video_combo_vsync = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_combo_vsync"));
  ...
  m_video_combo_vsync->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Vsync_Select, this));

  // FPS Limit
  CEGUI::Window *text_fps_limit = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_text_fps_limit"));
  text_fps_limit->setText(UTF8_("FPS Limit"));

  m_video_spinner_fps_limit = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_spinner_fps_limit"));
  m_video_spinner_fps_limit->setCurrentValue(pPreferences->m_video_fps_limit);

  m_video_spinner_fps_limit->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_FPS_Limit_Select, this));

  // Geometry quality
  CEGUI::Window *text_geometry_quality = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_text_geometry_quality"));
  text_geometry_quality->setText(UTF8_("Geometry Quality"));

  m_video_slider_geometry_quality = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_slider_geometry_quality"));
  m_video_slider_geometry_quality->setCurrentValue(pVideo->m_geometry_quality);
  m_video_slider_geometry_quality->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Video_Slider_Geometry_Quality_Changed, this));

  // Texture quality
  CEGUI::Window *text_texture_quality = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_text_texture_quality"));
  text_texture_quality->setText(UTF8_("Texture Quality"));

  m_video_slider_texture_quality = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_slider_texture_quality"));
  m_video_slider_texture_quality->setCurrentValue(pVideo->m_texture_quality);
  m_video_slider_texture_quality->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Video_Slider_Texture_Quality_Changed, this));

  // Reset
  CEGUI::PushButton *button_reset = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_button_reset"));
  button_reset->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Button_Reset_Clicked,
                               this));
  button_reset->setText(UTF8_("Reset"));

  // Apply
  CEGUI::PushButton *button_apply = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_button_apply"));
  button_apply->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Video_Button_Apply_Clicked,
                               this));
  button_apply->setText(UTF8_("Apply"));

  // Recreate Cache
  CEGUI::PushButton *button_recreate_cache = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("video_button_recreate_cache"));
  button_recreate_cache->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Video_Button_Recreate_Cache_Clicked, this));
...
void cMenu_Options ::Init_GUI_Audio(void) {
  ...
  // Audio Hz
  CEGUI::Window *text_hz =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("audio_text_hz"));
  text_hz->setText(UTF8_("Hertz (Hz)"));
  text_hz->setTooltipText(
      UTF8_("You should only change the value if the audio is scratchy."));

  m_audio_combo_hz =
      static_cast<CEGUI::Combobox *>(CEGUI::System::getSingleton()
                                         .getDefaultGUIContext()
                                         .getRootWindow()
                                         ->getChildRecursive("audio_combo_hz"));
  ...
  m_audio_combo_hz->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Hz_Select, this));

  // Music
  CEGUI::Window *text_music =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("audio_text_music"));
  text_music->setText(UTF8_("Music"));
  text_music->setTooltipText(UTF8_(
      "Enable to play music. You need to have the Music Addon installed."));

  m_audio_combo_music = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("audio_combo_music"));
  ...
  m_audio_combo_music->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Music_Select, this));

  // music volume slider
  m_audio_slider_music = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("audio_slider_music_volume"));
  m_audio_slider_music->setTooltipText(UTF8_("Set the Music Volume."));

  m_audio_slider_music->setCurrentValue(
      static_cast<float>(pAudio->m_music_volume));
  m_audio_slider_music->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Music_Volume_Changed,
                               this));

  // Sounds
  CEGUI::Window *text_sound =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("audio_text_sound"));
  text_sound->setText(UTF8_("Sound"));
  text_sound->setTooltipText(UTF8_("Enable to play Sounds."));

  m_audio_combo_sounds = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("audio_combo_sounds"));
  ...
  m_audio_combo_sounds->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Sound_Select, this));

  // sound volume slider
  m_audio_slider_sound = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("audio_slider_sound_volume"));
  m_audio_slider_sound->setTooltipText(UTF8_("Set the Sound Volume."));

  m_audio_slider_sound->setCurrentValue(
      static_cast<float>(pAudio->m_sound_volume));
  m_audio_slider_sound->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Sound_Volume_Changed,
                               this));
  ...
  CEGUI::PushButton *button_reset = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("audio_button_reset"));
  button_reset->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Audio_Button_Reset_Clicked,
                               this));
  button_reset->setText(UTF8_("Reset"));
}
...
void cMenu_Options ::Init_GUI_Keyboard(void) {
  ...
  // Keyboard listbox
  CEGUI::Window *text_keyboard_shortcuts =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("keyboard_text_shortcuts");
  text_keyboard_shortcuts->setText(UTF8_("Shortcuts"));

  CEGUI::MultiColumnList *listbox_keyboard =
      static_cast<CEGUI::MultiColumnList *>(
          CEGUI::System::getSingleton()
              .getDefaultGUIContext()
              .getRootWindow()
              ->getChildRecursive("keyboard_listbox"));

  listbox_keyboard->addColumn(UTF8_("Name"), 0, CEGUI::UDim(0.47f, 0));
  listbox_keyboard->addColumn(UTF8_("Key"), 1, CEGUI::UDim(0.47f, 0));
  Build_Shortcut_List();

  listbox_keyboard->subscribeEvent(
      CEGUI::MultiColumnList::EventMouseDoubleClick,
      CEGUI::Event::Subscriber(&cMenu_Options::Keyboard_List_Double_Click,
                               this));

  // Keyboard scroll speed
  CEGUI::Window *text_keyboard_scroll_speed =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("keyboard_text_scroll_speed");
  text_keyboard_scroll_speed->setText(UTF8_("Scroll Speed"));

  CEGUI::Slider *slider_scoll_speed = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("keyboard_slider_scroll_speed"));
  slider_scoll_speed->setCurrentValue(pPreferences->m_scroll_speed);
  slider_scoll_speed->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Keyboard_Slider_Scroll_Speed_Changed, this));

  // Reset Keyboard
  CEGUI::PushButton *button_reset_keyboard = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("keyboard_button_reset"));
  button_reset_keyboard->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Keyboard_Button_Reset_Clicked,
                               this));
...
void cMenu_Options ::Init_GUI_Joystick(void) {
  ...
  CEGUI::Window *text_joystick_sensitivity =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_sensitivity");
  text_joystick_sensitivity->setText(UTF8_("Sensitivity"));

  // Joystick analog jump text
  CEGUI::Window *text_joystick_analog_jump =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_analog_jump");
  text_joystick_analog_jump->setText(UTF8_("Analog Jump"));

  // Joystick name
  CEGUI::Window *text_joystick_name =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_name");
  text_joystick_name->setText(UTF8_("Joystick"));

  text_joystick_name->subscribeEvent(
      CEGUI::Window::EventMouseClick,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_Name_Click, this));

  CEGUI::Combobox *combo_joy =
      static_cast<CEGUI::Combobox *>(CEGUI::System::getSingleton()
                                         .getDefaultGUIContext()
                                         .getRootWindow()
                                         ->getChildRecursive("joystick_combo"));
  ...
  if (pPreferences->m_joy_enabled) {
    selected_item = static_cast<CEGUI::ListboxTextItem *>(
        combo_joy->findItemWithText(pJoystick->Get_Name(), NULL));
  }
  // disabled
  else {
    selected_item = static_cast<CEGUI::ListboxTextItem *>(
        combo_joy->getListboxItemFromIndex(0));
  }
  // set Item
  combo_joy->setText(selected_item->getText());

  combo_joy->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_Name_Select, this));

  // Joystick Sensitivity
  CEGUI::Slider *slider_joy_sensitivity = static_cast<CEGUI::Slider *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_slider_sensitivity"));
  slider_joy_sensitivity->setCurrentValue(pPreferences->m_joy_axis_threshold);
  slider_joy_sensitivity->subscribeEvent(
      CEGUI::Slider::EventValueChanged,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_Sensitivity_Changed,
                               this));

  // Joystick analog jump
  CEGUI::Combobox *combo_joy_analog_jump = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_combo_analog_jump"));
  ...
  combo_joy_analog_jump->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_Analog_Jump_Select,
                               this));

  // Joystick axis horizontal
  CEGUI::Window *text_joystick_axis_hor =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_axis_hor");
  text_joystick_axis_hor->setText(UTF8_("Axis Hor"));

  CEGUI::Spinner *spinner_joystick_axis_hor = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_spinner_axis_hor"));
  spinner_joystick_axis_hor->setCurrentValue(
      static_cast<float>(pPreferences->m_joy_axis_hor));
  spinner_joystick_axis_hor->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Joystick_Spinner_Axis_Hor_Changed, this));

  // Joystick axis vertical
  CEGUI::Window *text_joystick_axis_ver =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_axis_ver");
  text_joystick_axis_ver->setText(UTF8_("Ver"));

  CEGUI::Spinner *spinner_joystick_axis_ver = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_spinner_axis_ver"));
  spinner_joystick_axis_ver->setCurrentValue(
      static_cast<float>(pPreferences->m_joy_axis_ver));
  spinner_joystick_axis_ver->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Joystick_Spinner_Axis_Ver_Changed, this));

  // Joystick shortcut listbox
  CEGUI::Window *text_joystick_shortcuts =
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_text_shortcuts");
  text_joystick_shortcuts->setText(UTF8_("Shortcuts"));

  CEGUI::MultiColumnList *listbox_joystick =
      static_cast<CEGUI::MultiColumnList *>(
          CEGUI::System::getSingleton()
              .getDefaultGUIContext()
              .getRootWindow()
              ->getChildRecursive("joystick_listbox"));
  ...
  listbox_joystick->subscribeEvent(
      CEGUI::MultiColumnList::EventMouseDoubleClick,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_List_Double_Click,
                               this));

  // Reset Joystick
  CEGUI::PushButton *button_reset_joystick = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("joystick_button_reset"));
  button_reset_joystick->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Joystick_Button_Reset_Clicked,
                               this));
...
void cMenu_Options ::Init_GUI_Editor(void) {
  ...
  // show item images
  CEGUI::Window *text_editor_show_item_images = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_text_show_item_images"));
  text_editor_show_item_images->setText(UTF8_("Show images"));

  m_game_combo_editor_show_item_images = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_combo_show_item_images"));
  ...
  m_game_combo_editor_show_item_images->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Game_Editor_Show_Item_Images_Select, this));

  // item image size
  CEGUI::Window *text_editor_item_image_size = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_text_item_image_size"));
  text_editor_item_image_size->setText(UTF8_("Item image size"));

  m_game_spinner_editor_item_image_size = static_cast<CEGUI::Spinner *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_spinner_item_image_size"));
  m_game_spinner_editor_item_image_size->setCurrentValue(
      static_cast<float>(pPreferences->m_editor_item_image_size));

  m_game_spinner_editor_item_image_size->subscribeEvent(
      CEGUI::Spinner::EventValueChanged,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Game_Editor_Item_Image_Size_Select, this));

  // editor mouse auto hide
  CEGUI::Window *text_editor_mouse_auto_hide = static_cast<CEGUI::Window *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_text_mouse_auto_hide"));
  text_editor_mouse_auto_hide->setText(UTF8_("Auto-Hide Mouse"));

  m_game_combo_editor_mouse_auto_hide = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_combo_mouse_auto_hide"));
  ...
  m_game_combo_editor_mouse_auto_hide->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(
          &cMenu_Options::Game_Editor_Auto_Hide_Mouse_Select, this));

  // Reset Editor
  CEGUI::PushButton *button_reset_editor = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_button_reset"));
  button_reset_editor->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cMenu_Options::Game_Button_Reset_Editor_Clicked,
                               this));
...
void cMenu_Options ::Change_Game_Setting(int setting) {
  ...
  // language
    ...
    unsigned int selected = m_game_combo_language->getItemIndex(
        m_game_combo_language->findItemWithText(
            m_game_combo_language->getText(), NULL));
    ...
    // select next item
      ...
      new_selected =
          m_game_combo_language->getListboxItemFromIndex(selected + 1);
    }
  ...
  // menu level
    ...
    unsigned int selected = m_game_combo_menu_level->getItemIndex(
        m_game_combo_menu_level->findItemWithText(
            m_game_combo_menu_level->getText(), NULL));
    ...
    // select next item
      ...
      new_selected =
          m_game_combo_menu_level->getListboxItemFromIndex(selected + 1);
    }
...
void cMenu_Options ::Change_Video_Setting(int setting) {
  ...
  if (pMenuCore->m_handler->m_active == 5) {
    // Resolution
    unsigned int selected = m_video_combo_resolution->getItemIndex(
        m_video_combo_resolution->findItemWithText(
            m_video_combo_resolution->getText(), NULL));
    ...
    // select next item
      ...
      new_selected =
          m_video_combo_resolution->getListboxItemFromIndex(selected + 1);
    }
...
void cMenu_Options ::Change_Audio_Setting(int setting) {
  // Hz
  if (pMenuCore->m_handler->m_active == 5) {
    unsigned int selected = m_audio_combo_hz->getItemIndex(
        m_audio_combo_hz->findItemWithText(m_audio_combo_hz->getText(), NULL));
...
void cMenu_Options ::Change_Editor_Setting(int setting) {
  ...
  if (pMenuCore->m_handler->m_active == 10) {
    pPreferences->m_editor_show_item_images =
        !pPreferences->m_editor_show_item_images;
  ...
  // editor auto mouse hide
    ...
    pPreferences->m_editor_mouse_auto_hide =
        !pPreferences->m_editor_mouse_auto_hide;
...
void cMenu_Options ::Build_Shortcut_List(bool joystick /* = 0 */) {
  ...
  // Keyboard
  if (!joystick) {
    listbox = static_cast<CEGUI::MultiColumnList *>(
        CEGUI::System::getSingleton()
            .getDefaultGUIContext()
            .getRootWindow()
            ->getChildRecursive("keyboard_listbox"));
  ...
  // Keyboard
  if (!joystick) {
    shortcuts.push_back(cShortcut_item(UTF8_("Up"), &pPreferences->m_key_up,
                                       &pPreferences->m_key_up_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Down"), &pPreferences->m_key_down,
                                       &pPreferences->m_key_down_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Left"), &pPreferences->m_key_left,
                                       &pPreferences->m_key_left_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Right"),
                                       &pPreferences->m_key_right,
                                       &pPreferences->m_key_right_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Jump"), &pPreferences->m_key_jump,
                                       &pPreferences->m_key_jump_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Shoot"),
                                       &pPreferences->m_key_shoot,
                                       &pPreferences->m_key_shoot_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Item"), &pPreferences->m_key_item,
                                       &pPreferences->m_key_item_default));
    shortcuts.push_back(cShortcut_item(UTF8_("Action"),
                                       &pPreferences->m_key_action,
                                       &pPreferences->m_key_action_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Screenshot"), &pPreferences->m_key_screenshot,
                       &pPreferences->m_key_screenshot_default));
    shortcuts.push_back(cShortcut_item(
        UTF8_("Editor fast copy up"), &pPreferences->m_key_editor_fast_copy_up,
        &pPreferences->m_key_editor_fast_copy_up_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor fast copy down"),
                       &pPreferences->m_key_editor_fast_copy_down,
                       &pPreferences->m_key_editor_fast_copy_down_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor fast copy left"),
                       &pPreferences->m_key_editor_fast_copy_left,
                       &pPreferences->m_key_editor_fast_copy_left_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor fast copy right"),
                       &pPreferences->m_key_editor_fast_copy_right,
                       &pPreferences->m_key_editor_fast_copy_right_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor pixel move up"),
                       &pPreferences->m_key_editor_pixel_move_up,
                       &pPreferences->m_key_editor_pixel_move_up_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor pixel move down"),
                       &pPreferences->m_key_editor_pixel_move_down,
                       &pPreferences->m_key_editor_pixel_move_down_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor pixel move left"),
                       &pPreferences->m_key_editor_pixel_move_left,
                       &pPreferences->m_key_editor_pixel_move_left_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Editor pixel move right"),
                       &pPreferences->m_key_editor_pixel_move_right,
                       &pPreferences->m_key_editor_pixel_move_right_default));
  }
  // Joystick
  else {
    shortcuts.push_back(
        cShortcut_item(UTF8_("Jump"), &pPreferences->m_joy_button_jump,
                       &pPreferences->m_joy_button_jump_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Shoot"), &pPreferences->m_joy_button_shoot,
                       &pPreferences->m_joy_button_shoot_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Action"), &pPreferences->m_joy_button_action,
                       &pPreferences->m_joy_button_action_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Item"), &pPreferences->m_joy_button_item,
                       &pPreferences->m_joy_button_item_default));
    shortcuts.push_back(
        cShortcut_item(UTF8_("Exit"), &pPreferences->m_joy_button_exit,
                       &pPreferences->m_joy_button_exit_default));
  }

  // add all available shortcuts
  for (vector<cShortcut_item>::iterator itr = shortcuts.begin();
       itr != shortcuts.end(); ++itr) {
    cShortcut_item shortcut_item = (*itr);

    CEGUI::ListboxTextItem *item = new CEGUI::ListboxTextItem(
        shortcut_item.m_name, 0, shortcut_item.m_key);
    item->setSelectionColours(CEGUI::Colour(0.33f, 0.33f, 0.33f));
    item->setSelectionBrushImage("TaharezLook/ListboxSelectionBrush");
    ...
    if (!joystick) {
      ...
      const SDL_Keycode *key_default =
          static_cast<const SDL_Keycode *>(shortcut_item.m_key_default);
    ...
    // Joystick
      ...
      const Uint8 *button_default =
          static_cast<const Uint8 *>(shortcut_item.m_key_default);
...
void cMenu_Options ::Set_Shortcut(std::string name, void *data,
                                  bool joystick /* = 0 */) {
  ...
  Draw_Static_Text(info_text + _(" for ") + name + _(". Press ESC to cancel."),
                   &orange, NULL, 0);
  ...
  while (!sub_done) {
    ...
    if (input_event.key.keysym.sym == SDLK_ESCAPE ||
        input_event.key.keysym.sym == SDLK_BACKSPACE) {
      sub_done = 1;
      break;
    }
...
bool cMenu_Options ::Game_Camera_Hor_Select(const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::Spinner *spinner_camera_hor =
      static_cast<CEGUI::Spinner *>(windowEventArgs.window);

  pLevel_Manager->m_camera->m_hor_offset_speed =
      spinner_camera_hor->getCurrentValue();
...
bool cMenu_Options ::Game_Camera_Ver_Select(const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::Spinner *spinner_camera_ver =
      static_cast<CEGUI::Spinner *>(windowEventArgs.window);

  pLevel_Manager->m_camera->m_ver_offset_speed =
      spinner_camera_ver->getCurrentValue();
...
bool cMenu_Options ::Game_Menu_Level_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  pPreferences->m_menu_level =
      static_cast<CEGUI::Editbox *>(windowEventArgs.window)->getText().c_str();

  return 1;
}

bool cMenu_Options ::Game_Button_Reset_Game_Clicked(
    const CEGUI::EventArgs &event) {
...
bool cMenu_Options ::Video_FPS_Limit_Select(const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::Spinner *spinner_fps_limit =
      static_cast<CEGUI::Spinner *>(windowEventArgs.window);
...
bool cMenu_Options ::Video_Slider_Geometry_Quality_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  m_vid_geometry_detail =
      static_cast<CEGUI::Slider *>(windowEventArgs.window)->getCurrentValue();

  return 1;
}

bool cMenu_Options ::Video_Slider_Texture_Quality_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  m_vid_texture_detail =
      static_cast<CEGUI::Slider *>(windowEventArgs.window)->getCurrentValue();
...
bool cMenu_Options ::Video_Button_Reset_Clicked(const CEGUI::EventArgs &event) {
  CEGUI::ListboxItem *list_item = m_video_combo_resolution->findItemWithText(
      int_to_string(cPreferences::m_video_screen_w_default) + "x" +
          int_to_string(cPreferences::m_video_screen_h_default),
      NULL);
  if (list_item) {
  ...
  list_item = m_video_combo_bpp->findItemWithText(
      int_to_string(cPreferences::m_video_screen_bpp_default), NULL);
  if (list_item) {
  ...
  m_video_spinner_fps_limit->setCurrentValue(
      cPreferences::m_video_fps_limit_default);
  pPreferences->m_video_fps_limit = cPreferences::m_video_fps_limit_default;

  m_video_slider_geometry_quality->setCurrentValue(
      cPreferences::m_geometry_quality_default);
  m_vid_geometry_detail = cPreferences::m_geometry_quality_default;

  m_video_slider_texture_quality->setCurrentValue(
      cPreferences::m_texture_quality_default);
...
bool cMenu_Options ::Video_Button_Apply_Clicked(const CEGUI::EventArgs &event) {
  ...
  // apply new settings
  pPreferences->Apply_Video(m_vid_w, m_vid_h, m_vid_bpp, m_vid_fullscreen,
                            m_vid_vsync, m_vid_geometry_detail,
                            m_vid_texture_detail);
...
bool cMenu_Options ::Video_Button_Recreate_Cache_Clicked(
    const CEGUI::EventArgs &event) {
  Loading_Screen_Init();
...
bool cMenu_Options ::Audio_Music_Select(const CEGUI::EventArgs &event) {
  ...
  if (pAudio->m_music_enabled != music_enabled) {
    ...
    if (pAudio->m_music_enabled &&
        !File_Exists(
            std::string(DATA_DIR "/" GAME_MUSIC_DIR "/game/menu.ogg")) &&
        !File_Exists(
            std::string(DATA_DIR "/" GAME_MUSIC_DIR "/land/land_1.ogg"))) {
      Draw_Static_Text(
          _("Music addon not detected.\nYou can download it from the Website."),
          &orange);
    }
  }
...
bool cMenu_Options ::Keyboard_List_Double_Click(const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =
      static_cast<CEGUI::MultiColumnList *>(windowEventArgs.window)
          ->getFirstSelectedItem();
...
bool cMenu_Options ::Keyboard_Slider_Scroll_Speed_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  pPreferences->m_scroll_speed =
      static_cast<CEGUI::Slider *>(windowEventArgs.window)->getCurrentValue();

  return 1;
}

bool cMenu_Options ::Keyboard_Button_Reset_Clicked(
    const CEGUI::EventArgs &event) {
...
bool cMenu_Options ::Joystick_Name_Click(const CEGUI::EventArgs &event) {
  ...
  CEGUI::Combobox *combo_joy =
      static_cast<CEGUI::Combobox *>(CEGUI::System::getSingleton()
                                         .getDefaultGUIContext()
                                         .getRootWindow()
                                         ->getChildRecursive("combo_joy"));

  // selected item id
  int selected_item = 0;
  // current list item
  CEGUI::ListboxTextItem *list_item = static_cast<CEGUI::ListboxTextItem *>(
      combo_joy->findItemWithText(combo_joy->getText(), NULL));
  ...
  combo_joy->setText(
      combo_joy->getListboxItemFromIndex(selected_item)->getText());
...
bool cMenu_Options ::Joystick_Name_Select(const CEGUI::EventArgs &event) {
  ...
  CEGUI::Combobox *combo =
      static_cast<CEGUI::Combobox *>(windowEventArgs.window);
...
bool cMenu_Options ::Joystick_Sensitivity_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  pPreferences->m_joy_axis_threshold = static_cast<Sint16>(
      static_cast<CEGUI::Slider *>(windowEventArgs.window)->getCurrentValue());

  return 1;
}

bool cMenu_Options ::Joystick_Analog_Jump_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =
...
bool cMenu_Options ::Joystick_Spinner_Axis_Hor_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  pPreferences->m_joy_axis_hor = static_cast<int>(
      static_cast<CEGUI::Spinner *>(windowEventArgs.window)->getCurrentValue());

  return 1;
}

bool cMenu_Options ::Joystick_Spinner_Axis_Ver_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  // set new value
  pPreferences->m_joy_axis_ver = static_cast<int>(
      static_cast<CEGUI::Spinner *>(windowEventArgs.window)->getCurrentValue());
...
bool cMenu_Options ::Joystick_Button_Reset_Clicked(
    const CEGUI::EventArgs &event) {
...
bool cMenu_Options ::Game_Editor_Show_Item_Images_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =
...
bool cMenu_Options ::Game_Editor_Item_Image_Size_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::Spinner *spinner_item_image_size =
      static_cast<CEGUI::Spinner *>(windowEventArgs.window);

  pPreferences->m_editor_item_image_size =
      static_cast<unsigned int>(spinner_item_image_size->getCurrentValue());

  return 1;
}

bool cMenu_Options ::Game_Editor_Auto_Hide_Mouse_Select(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  CEGUI::ListboxItem *item =
...
bool cMenu_Options ::Game_Button_Reset_Editor_Clicked(
    const CEGUI::EventArgs &event) {
...
cMenu_Savegames ::cMenu_Savegames(bool type) : cMenu_Base() {
  m_type_save = type;

  for (unsigned int i = 0; i < 9; i++) {
    m_savegame_temp.push_back(
        new cHudSprite(pMenuCore->m_handler->m_level->m_sprite_manager));
  }
}

cMenu_Savegames ::~cMenu_Savegames(void) {
  for (HudSpriteList::iterator itr = m_savegame_temp.begin();
       itr != m_savegame_temp.end(); ++itr) {
    delete *itr;
  }
...
void cMenu_Savegames ::Init(void) {
  ...
  // back
  cGL_Surface *back1 =
      pFont->Render_Text(pFont->m_font_normal, _("Back"), m_text_color);
  temp_item = new cMenu_Item(pMenuCore->m_handler->m_level->m_sprite_manager);
...
void cMenu_Savegames ::Init_GUI(void) { cMenu_Base::Init_GUI(); }
...
void cMenu_Savegames ::Update_Load(void) {
  ...
  if (!level_name.empty()) {
    ...
    if (pActive_Level->Get_Music_Filename(1).compare(
            level->Get_Music_Filename(1)) != 0) {
      Game_Action_Data_Start.add("music_fadeout", "1000");
    }
  } else {
    Game_Action = GA_ENTER_WORLD;
  ...
  Game_Action_Data_End.add(
      "screen_fadein", CEGUI::PropertyHelper<int>::toString(EFFECT_IN_BLACK));
  Game_Action_Data_End.add("screen_fadein_speed", "3");
}
...
void cMenu_Savegames ::Update_Save(void) {
  ...
  if (pOverworld_Player->m_current_waypoint < 0 &&
      !pActive_Level->Is_Loaded()) {
    return;
  }

  std::string descripion =
      Set_Save_Description(pMenuCore->m_handler->m_active + 1);
...
// save always in debug builds
  ...
  return Box_Text_Input(save_description, _("Enter Description"),
                        auto_erase_description);
}
...
void cMenu_Savegames ::Update_Saved_Games_Text(void) {
  ...
  for (HudSpriteList::iterator itr = m_savegame_temp.begin();
       itr != m_savegame_temp.end(); ++itr) {
    save_slot++;
    (*itr)->Set_Image(pFont->Render_Text(pFont->m_font_normal,
                                         pSavegame->Get_Description(save_slot),
                                         m_text_color_value),
                      1, 1);
  }
}
...
cMenu_Credits ::cMenu_Credits(void) : cMenu_Base() {}
...
void cMenu_Credits ::Init(void) {
  ...
  Add_Credits_Line("Anthony Smith (mrvertigo27)", 0, 20,
                   Color(0.58f, 0.52f, 1.0f), 1.0f);
  Add_Credits_Line(" - Graphic Designer", 0, -3);

  Add_Credits_Line("Fabian ... (Fabianius)", 0, 20, Color(0.5f, 0.9f, 0.0f),
                   1.0f);
  Add_Credits_Line(" - Graphic Designer", 0, -3);
  ...
  Add_Credits_Line("Norbu Tsering (Naerbu)", 0, 20, Color(0.8f, 0.0f, 0.0f),
                   1.0f);
  ...
  Add_Credits_Line("David Hernandez (vencabot_teppoo)", 0, 20,
                   Color(0.8f, 0.6f, 0.2f), 1.0f);
  Add_Credits_Line(" - Music Artist", 0, -3);

  Add_Credits_Line("Markus Hiebl (Frostbringer)", 0, 20,
  ...
  Add_Credits_Line("Ursula ... (Pipgirl)", 0, 20, Color(0.2f, 0.9f, 0.2f),
                   1.0f);
  Add_Credits_Line(" - Graphic Designer", 0, -3);

  Add_Credits_Line("Tobias Maasland (Weirdnose)", 0, 20,
...
void cMenu_Credits ::Init_GUI(void) { cMenu_Base::Init_GUI(); }
...
void cMenu_Credits ::Leave(const GameMode next_mode /* = MODE_NOTHING */) {
  ...
  pMenuCore->m_handler->m_level->m_background_manager->Get_Pointer(0)
      ->m_color_1.alpha = 255;
  pMenuCore->m_handler->m_level->m_background_manager->Get_Pointer(0)
      ->m_color_2.alpha = 255;
}
...
void cMenu_Credits ::Update(void) {
  ...
  for (HudSpriteList::iterator itr = m_draw_list.begin();
    ...
    // fading out
      ...
      float new_value =
          obj->m_combine_color[0] - (pFramerate->m_speed_factor * 0.01f);
      ...
      if (new_value > 1.0f) {
        ...
        if (obj->m_combine_color[0] < 1.0f) {
          cParticle_Emitter *anim = new cParticle_Emitter(
              pMenuCore->m_handler->m_level->m_sprite_manager);
          anim->Set_Emitter_Rect(
              Get_Random_Float(game_res_w * 0.1f, game_res_w * 0.8f),
              -Get_Random_Float(game_res_h * 0.8f, game_res_h * 0.9f),
              Get_Random_Float(0.0f, 5.0f), Get_Random_Float(0.0f, 5.0f));
          ...
          anim->Set_Color(Color(static_cast<Uint8>(100 + (rand() % 155)),
                                100 + (rand() % 155), 100 + (rand() % 155)));
...
void cMenu_Credits ::Add_Credits_Line(const std::string &text, float posx,
                                      float posy,
                                      const Color &shadow_color /* = black */,
                                      float shadow_pos /* = 0.0f */) {
  cHudSprite *temp =
      new cHudSprite(pMenuCore->m_handler->m_level->m_sprite_manager);
...
void cMenu_Credits ::Menu_Fade(bool fade_in /* = 1 */) {
  ...
  // get logo
  cSprite *logo =
      pMenuCore->m_handler->m_level->m_sprite_manager->Get_from_Position(
          180, logo_pos_y, TYPE_FRONT_PASSIVE, 2);
  ...
  while (1) {
    ...
    pMenuCore->m_handler->m_level->m_background_manager->Get_Pointer(0)
        ->m_color_1.alpha = static_cast<Uint8>(counter);
    pMenuCore->m_handler->m_level->m_background_manager->Get_Pointer(0)
        ->m_color_2.alpha = static_cast<Uint8>(counter);

...
class GL_point {
public:
  GL_point(void) : m_x(0), m_y(0) {}

  GL_point(float x, float y) : m_x(x), m_y(y) {}

  GL_point(const GL_point &p) : m_x(p.m_x), m_y(p.m_y) {}
  ...
  GL_point rotate(const GL_point &hotspot, float angle) const {
    ...
    const float nw =
        static_cast<float>(atan2(r.m_y, r.m_x) + ((angle + 180) * M_PI / 180));
    r.m_x =
        static_cast<float>((sin(1.5 * M_PI - nw) * c) /*+ 0.5*/) + hotspot.m_x;
  ...
  GL_point rotate3d(float angle, const float &a_x, const float &a_y,
                    const float &a_z) const {
    GL_point tmp;

    float s = static_cast<float>(sin(angle));
    float c = static_cast<float>(cos(angle));

    tmp.m_x = m_x * (a_x * a_x * (1 - c) + c) +
              m_y * (a_x * a_y * (1 - c) - a_z * s) +
              /*z**/ (a_x * a_z * (1 - c) + a_y * s);
    tmp.m_y = m_x * (a_y * a_x * (1 - c) + a_z * s) +
              m_y * (a_y * a_y * (1 - c) + c) +
              /*z**/ (a_y * a_z * (1 - c) - a_x * s);
    // tmp.z = m_x*(a_x*a_z*(1-c)-a_y*s) + m_y*(a_y*a_z*(1-c)+a_x*s) +
    // z*(a_z*a_z*(1-c)+c);

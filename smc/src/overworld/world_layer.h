...
public:
  // constructor
  cLayer_Line_Point(cSprite_Manager *sprite_manager, cOverworld *overworld,
                    SpriteType new_type);
  // destructor
...
public:
  // constructor
  cLayer_Line_Point_Start(cSprite_Manager *sprite_manager,
                          cOverworld *overworld);
  // destructor
  virtual ~cLayer_Line_Point_Start(void);
  // create from stream
  cLayer_Line_Point_Start(CEGUI::XMLAttributes &attributes,
...
class cLayer : public CEGUI::XMLHandler,
               public cObject_Manager<cLayer_Line_Point_Start> {
public:
  ...
  /* Returns the colliding Line from the given position and the added direction
   * size if not found returns a NULL line in the class
   */
  cLine_collision
  Get_Line_Collision_Direction(float x, float y, ObjectDirection dir,
                               float dir_size = 15,
   ...
   */
  cLine_collision Get_Nearest(float x, float y,
  ...
  cLine_collision Get_Nearest_Line(cLayer_Line_Point_Start *map_layer_line,
                                   float x, float y,
                                   ObjectDirection dir = DIR_HORIZONTAL,

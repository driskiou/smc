...
// again if CEGUI 0.8 works with std::string utf8
  ...
  CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
      *this, (const CEGUI::utf8 *)filename.c_str(),
      DATA_DIR "/" GAME_SCHEMA_DIR "/World/Description.xsd", "");
#else
  CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
      *this, filename.c_str(),
      DATA_DIR "/" GAME_SCHEMA_DIR "/World/Description.xsd", "");
...
void cOverworld_description ::Save(void) {
  std::string save_dir =
      pResource_Manager->user_data_dir + USER_WORLD_DIR + "/" + m_path;
...
std::string cOverworld_description ::Get_Full_Path(void) const {
  ...
  if (File_Exists(pResource_Manager->user_data_dir + USER_WORLD_DIR + "/" +
                  m_path + "/description.xml")) {
...
void cOverworld_description ::elementStart(
...
void cOverworld_description ::handle_world(
    const CEGUI::XMLAttributes &attributes) {
  m_name = xml_string_to_string(
      attributes.getValueAsString("name", m_name.c_str()).c_str());
...
// again if CEGUI 0.8 works with std::string utf8
#ifdef _WIN32
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, (const CEGUI::utf8 *)world_filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/World/World.xsd", "");
#else
    CEGUI::System::getSingleton().getXMLParser()->parseXMLFile(
        *this, world_filename.c_str(),
        DATA_DIR "/" GAME_SCHEMA_DIR "/World/World.xsd", "");
#endif
  }
  // catch CEGUI Exceptions
    ...
    printf("Loading World %s CEGUI Exception %s\n", world_filename.c_str(),
           ex.getMessage().c_str());
    pHud_Debug->Set_Text(_("World Loading failed : ") +
  ...
  // set name
  m_hud_world_name->Set_Image(
      pFont->Render_Text(pFont->m_font_normal, m_description->m_name, yellow),
      1, 1);
...
void cOverworld ::Save(void) {
  ...
  std::string save_dir = pResource_Manager->user_data_dir + USER_WORLD_DIR +
                         "/" + m_description->m_path;
...
// fixme : Check if there is a more portable way f.e. with imbue()
  ...
  if (!file) {
    printf(
        "Error : Couldn't open world file for saving. Is the file read-only ?");
  ...
  Write_Property(stream, "color_green",
                 static_cast<int>(m_background_color.green));
  Write_Property(stream, "color_blue",
                 static_cast<int>(m_background_color.blue));
  // end background
  stream.closeTag();
  ...
  // moving state
  Write_Property(stream, "moving_state",
                 static_cast<int>(m_player_moving_state));
  // end player
  stream.closeTag();
...
void cOverworld ::Enter(const GameMode old_mode /* = MODE_NOTHING */) {
  ...
  if (pOverworld_Player->m_current_waypoint ==
      static_cast<int>(m_player_start_waypoint)) {
    // if player state is walk
    if (m_player_moving_state == STA_WALK) {
      // walk to the next Waypoint
      pOverworld_Player->Start_Walk(
          m_waypoints[pOverworld_Player->m_current_waypoint]
              ->m_direction_forward);
    }
  }
...
void cOverworld ::Set_Progress(unsigned int normal_level,
                               bool force /* = 1 */) {
  unsigned int level_num = 0;

  for (WaypointList::iterator itr = m_waypoints.begin();
       itr != m_waypoints.end(); ++itr) {
    cWaypoint *obj = (*itr);
...
int cOverworld ::Get_Level_Waypoint_Num(std::string level_name) {
  ...
  if (level_name.rfind(".txt") != std::string::npos ||
      level_name.rfind(".smclvl") != std::string::npos) {
...
void cOverworld ::Update_Waypoint_text(void) {
  ...
  m_hud_level_name->Set_Image(pFont->Render_Text(pFont->m_font_normal,
                                                 waypoint->Get_Destination(),
                                                 color),
                              1, 1);
}
...
bool cOverworld ::Goto_Next_Level(void) {
  ...
  cLayer_Line_Point_Start *front_line =
      pOverworld_Player->Get_Front_Line(current_waypoint->m_direction_forward);
  ...
  if (!next_waypoint->m_access) {
    ...
    anim->Set_Emitter_Rect(
        next_waypoint->m_rect.m_x + (next_waypoint->m_rect.m_w * 0.5f),
        next_waypoint->m_rect.m_y + (next_waypoint->m_rect.m_h * 0.5f), 1.0f,
        1.0f);
...
void cOverworld ::elementStart(const CEGUI::String &element,
                               const CEGUI::XMLAttributes &attributes) {
  if (element == "property" || element == "Property") {
    m_xml_attributes.add(attributes.getValueAsString("name"),
                         attributes.getValueAsString("value"));
...
void cOverworld ::elementEnd(const CEGUI::String &element) {
  ...
  if (element == "information") {
    ...
    // Music
    m_musicfile = xml_string_to_string(
        m_xml_attributes.getValueAsString("music").c_str());
    // Camera Limits
    // pOverworld_Manager->camera->Set_Limits( GL_rect(
    ...
    // Moving State
    m_player_moving_state = static_cast<Moving_state>(
        m_xml_attributes.getValueAsInteger("moving_state"));
  } else if (element == "background") {
    m_background_color = Color(
        static_cast<Uint8>(m_xml_attributes.getValueAsInteger("color_red")),
        m_xml_attributes.getValueAsInteger("color_green"),
        m_xml_attributes.getValueAsInteger("color_blue"));
  } else {
    // get World object
    cSprite *object = Create_World_Object_From_XML(
        element, m_xml_attributes, m_engine_version, m_sprite_manager, this);
...
cSprite *Create_World_Object_From_XML(const CEGUI::String &element,
  ...
  if (element == "sprite") {
    ...
    if (engine_version < 3) {
      Relocate_Image(attributes, "world/objects/bridge/bridge_1.png",
                     "world/objects/bridge/bridge_1_ver_start.png");
    }
      ...
      if (engine_version < 3) {
        if (sprite->m_image->m_filename.compare(
                DATA_DIR "/" GAME_PIXMAPS_DIR "/"
                         "world/objects/bridge/bridge_1_ver_start.png") == 0) {
          ...
          // middle
          copy->Set_Image(pVideo->Get_Surface(
                              "world/objects/bridge/bridge_1_ver_middle.png"),
                          1);
          ...
          copy = copy->Copy();
          copy->Set_Image(
              pVideo->Get_Surface("world/objects/bridge/bridge_1_ver_end.png"),
              1);
          copy->Set_Pos_Y(copy->m_start_pos_y + 32, 1);
          sprite_manager->Add(copy);
        }
    ...
    if (engine_version < 2) {
      if (attributes.exists("y")) {
        attributes.add("y", CEGUI::PropertyHelper<float>::toString(
                                attributes.getValueAsFloat("y") - 600.0f));
      }
    }
    ...
    if (engine_version < 2) {
      if (attributes.exists("Y1")) {
        attributes.add("Y1", CEGUI::PropertyHelper<float>::toString(
                                 attributes.getValueAsFloat("Y1") - 600.0f));
      }
      if (attributes.exists("Y2")) {
        attributes.add("Y2", CEGUI::PropertyHelper<float>::toString(
                                 attributes.getValueAsFloat("Y2") - 600.0f));
      }
    }

...
cCopyObject ::cCopyObject(void) { m_obj = NULL; }
...
bool cMouseCursor ::Handle_Event(SDL_Event *ev) {
  ...
  case SDL_MOUSEMOTION: {
    CEGUI::System::getSingleton().getDefaultGUIContext().injectMousePosition(
        static_cast<float>(ev->motion.x), static_cast<float>(ev->motion.y));
    Update_Position();
    break;
  }
...
cObjectCollision *
cMouseCursor ::Get_First_Editor_Collsion(float px /* = 0.0f */,
                                         float py /* = 0.0f */) {
  if (m_mover_mode) {
    return NULL;
  }
  ...
  CEGUI::Window *mouse_window =
      pGuiSystem->getDefaultGUIContext().getModalWindow();
...
cObjectCollision *
cMouseCursor ::Get_First_Mouse_Collision(const GL_rect &mouse_rect) {
  cSprite_List sprite_objects;
  m_sprite_manager->Get_Objects_sorted(sprite_objects, 1, 1);

  // check objects
  for (cSprite_List::reverse_iterator itr = sprite_objects.rbegin();
       itr != sprite_objects.rend(); ++itr) {
    cSprite *obj = (*itr);
...
void cMouseCursor ::Update_Hovered_Object(void) {
  if (!editor_enabled || !m_hovering_object->m_obj ||
      (m_mover_mode &&
       (Game_Mode == MODE_LEVEL || Game_Mode == MODE_OVERWORLD))) {
    return;
  }
...
void cMouseCursor ::Add_Copy_Object(cSprite *sprite) {
  ...
  for (CopyObjectList::iterator itr = m_copy_objects.begin();
       itr != m_copy_objects.end(); ++itr) {
    if ((*itr)->m_obj == sprite) {
      return;
    }
  }
...
void cMouseCursor ::Add_Copy_Objects(cSprite_List &spritelist) {
  ...
  // insert all objects
  for (cSprite_List::iterator itr = spritelist.begin(); itr != spritelist.end();
       ++itr) {
    cSprite *obj = (*itr);
...
GL_Vector cMouseCursor ::Get_Copy_Object_Base(float px, float py) {
  ...
  for (CopyObjectList::iterator itr = m_copy_objects.begin();
    ...
    if (copy_obj->m_obj->m_pos_x < base_x ||
        copy_obj->m_obj->m_pos_y < base_y) {
...
void cMouseCursor ::Paste_Copy_Objects(float px, float py) {
  ...
  for (CopyObjectList::iterator itr = m_copy_objects.begin();
    ...
    // set base vector
    GL_Vector base_vec = Get_Copy_Object_Base(copy_obj->m_obj->m_pos_x,
                                              copy_obj->m_obj->m_pos_y);

    // copy
    cSprite *new_object =
        Copy(copy_obj->m_obj, px + base_vec.x, py + base_vec.y);
  ...
  if (!m_copy_objects.empty()) {
    m_hovering_object->m_mouse_offset_y =
        static_cast<int>(m_copy_objects[0]->m_obj->m_col_rect.m_h / 2);
    m_hovering_object->m_mouse_offset_x =
        static_cast<int>(m_copy_objects[0]->m_obj->m_col_rect.m_w / 2);
  }
...
bool cMouseCursor ::Add_Selected_Object(cSprite *sprite,
                                        bool from_user /* = 0 */) {
  if (!sprite) {
    return 0;
  }
  ...
  // check if not already added
  for (SelectedObjectList::iterator itr = m_selected_objects.begin();
       itr != m_selected_objects.end(); ++itr) {
...
void cMouseCursor ::Add_Selected_Objects(cSprite_List &spritelist,
                                         bool from_user /* = 0 */) {
  if (spritelist.empty()) {
    return;
  }
...
bool cMouseCursor ::Remove_Selected_Object(const cSprite *sprite,
                                           bool no_user /* = 0 */) {
  if (!sprite) {
    return 0;
  }
...
void cMouseCursor ::Update_Selected_Objects(void) {
  if (!editor_enabled || (m_mover_mode && (Game_Mode == MODE_LEVEL ||
...
void cMouseCursor ::Update_Selected_Object_Offset(cSelectedObject *obj) {
  ...
  obj->m_mouse_offset_x =
      static_cast<int>(m_pos_x) - static_cast<int>(obj->m_obj->m_start_pos_x);
  obj->m_mouse_offset_y =
      static_cast<int>(m_pos_y) - static_cast<int>(obj->m_obj->m_start_pos_y);
}

bool cMouseCursor ::Is_Selected_Object(const cSprite *sprite,
                                       bool only_user /* = 0 */) {
  if (!sprite) {
    return 0;
  }
...
bool cMouseCursor ::Get_Snap_Pos(GL_point &new_pos, int snap,
                                 cSelectedObject *src_obj) {
  GL_rect src_rect;
  ...
  if (m_selected_objects.size() > 1) {
    ...
    full_snap_rect = GL_rect(src_rect.m_x - snap, src_rect.m_y - snap,
                             src_rect.m_w + snap * 2, src_rect.m_h + snap * 2);
    ...
    // update the top-left co-ord of the new bounding box using the mouse
    // position
    src_rect.m_x =
        (m_pos_x - src_obj->m_mouse_offset_x) - multiselect_offset.m_x;
    src_rect.m_y =
        (m_pos_y - src_obj->m_mouse_offset_y) - multiselect_offset.m_y;
  } else {
    src_rect = GL_rect(m_pos_x - src_obj->m_mouse_offset_x,
                       m_pos_y - src_obj->m_mouse_offset_y,
                       src_obj->m_obj->m_start_rect.m_w,
                       src_obj->m_obj->m_start_rect.m_h);
    full_snap_rect = GL_rect(src_obj->m_obj->m_start_rect.m_x - snap,
                             src_obj->m_obj->m_start_rect.m_y - snap,
                             src_obj->m_obj->m_start_rect.m_w + snap * 2,
                             src_obj->m_obj->m_start_rect.m_h + snap * 2);
  }
  ...
  for (cSprite_List::iterator itr = m_sprite_manager->objects.begin();
    ...
    if (full_snap_rect.Intersects(obj->m_start_rect)) {
      const float obj_distance_left =
          fabs(src_rect.m_x - (obj->m_start_rect.m_x + obj->m_start_rect.m_w));
      const float obj_distance_right =
          fabs((src_rect.m_x + src_rect.m_w) - obj->m_start_rect.m_x);
      const float obj_distance_top =
          fabs(src_rect.m_y - (obj->m_start_rect.m_y + obj->m_start_rect.m_h));
      const float obj_distance_bottom =
          fabs((src_rect.m_y + src_rect.m_h) - obj->m_start_rect.m_y);
      ...
      if (obj_distance_best <= distance_best) {
        ...
        const float obj_distance_left_to_left =
            fabs(src_rect.m_x - obj->m_start_rect.m_x);
        const float obj_distance_right_to_right =
            fabs((src_rect.m_x + src_rect.m_w) -
                 (obj->m_start_rect.m_x + obj->m_start_rect.m_w));
        const float obj_distance_top_to_top =
            fabs(src_rect.m_y - obj->m_start_rect.m_y);
        const float obj_distance_bottom_to_bottom =
            fabs((src_rect.m_y + src_rect.m_h) -
                 (obj->m_start_rect.m_y + obj->m_start_rect.m_h));
  ...
  // vertical
  if ((distance_top < distance_left && distance_top < distance_right) ||
      (distance_bottom < distance_left && distance_bottom < distance_right)) {
    // out of snap range
    ...
    const float distance_right_to_right =
        fabs((src_rect.m_x + src_rect.m_w) -
             (snap_obj_rect.m_x + snap_obj_rect.m_w));
  ...
  // horizontal
    ...
    const float distance_bottom_to_bottom =
        fabs((src_rect.m_y + src_rect.m_h) -
             (snap_obj_rect.m_y + snap_obj_rect.m_h));
...
cSprite *cMouseCursor ::Copy(const cSprite *copy_object, float px,
                             float py) const {
  if (!copy_object) {
    return NULL;
  }
...
void cMouseCursor ::Set_Object_Position(cSelectedObject *sel_obj) {
  ...
  if (m_snap_to_object_mode && m_snap_pos_available) {
    sel_obj->m_obj->Set_Pos(m_snap_pos.m_x - sel_obj->m_mouse_offset_x,
                            m_snap_pos.m_y - sel_obj->m_mouse_offset_y, 1);
  } else {
    // set new position
    sel_obj->m_obj->Set_Pos(static_cast<float>(static_cast<int>(m_pos_x) -
                                               sel_obj->m_mouse_offset_x),
                            static_cast<float>(static_cast<int>(m_pos_y) -
                                               sel_obj->m_mouse_offset_y),
                            1);
  }
...
void cMouseCursor ::Draw_Object_Rects(void) {
  ...
  if (!m_left && m_hovering_object->m_obj) {
    hover_rect.m_x =
        m_hovering_object->m_obj->m_start_pos_x - pActive_Camera->m_x;
    hover_rect.m_y =
        m_hovering_object->m_obj->m_start_pos_y - pActive_Camera->m_y;
    hover_rect.m_w = m_hovering_object->m_obj->m_start_rect.m_w;
    hover_rect.m_h = m_hovering_object->m_obj->m_start_rect.m_h;

    // get object color
    obj_color =
        Get_Massive_Type_Color(m_hovering_object->m_obj->m_massive_type);
  ...
  // draw selected objects if not left mouse is pressed, shift selection or
  // mouse selection
  if (!m_left || (pKeyboard->Is_Shift_Down() && !pKeyboard->Is_Ctrl_Down()) ||
      m_selection_mode) {
    obj_color = white;
    ...
    for (SelectedObjectList::iterator itr = m_selected_objects.begin();
      ...
      if (!object->Is_Visible_On_Screen() ||
          object == m_hovering_object->m_obj) {
        continue;
      }
  ...
  if (m_left && m_snap_to_object_mode && m_selected_objects.size() > 1) {
    ...
    pVideo->Draw_Rect(sel_rect.m_x - pActive_Camera->m_x,
                      sel_rect.m_y - pActive_Camera->m_y, sel_rect.m_w,
                      sel_rect.m_h, 0.51f, &lightgrey, rect_request);
...
void cMouseCursor ::Update_Selection(void) {
  ...
  GL_rect rect =
      GL_rect(m_selection_rect.m_x, m_selection_rect.m_y, 0.0f, 0.0f);
  ...
  // ## inner rect
  cRect_Request *rect_request = new cRect_Request();
  pVideo->Draw_Rect(m_selection_rect.m_x - pActive_Camera->m_x + 0.5f,
                    m_selection_rect.m_y - pActive_Camera->m_y + 0.5f,
                    m_selection_rect.m_w, m_selection_rect.m_h, 0.509f, &white,
                    rect_request);
  ...
  pVideo->Draw_Rect(m_selection_rect.m_x - pActive_Camera->m_x,
                    m_selection_rect.m_y - pActive_Camera->m_y,
                    m_selection_rect.m_w, m_selection_rect.m_h, 0.51f,
                    &lightblue, rect_request);
...
void cMouseCursor ::Mover_Update(Sint16 move_x, Sint16 move_y) {
  ...
  //	SDL_CaptureMouse( static_cast<Uint16>(m_x * global_upscalex),
  // static_cast<Uint16>(m_y * global_upscaley) );
...
void cMouseCursor ::Editor_Update(void) {
  ...
  if (col->m_obj) {
    ...
    if ((!m_left || !m_hovering_object->m_obj) &&
        !(pKeyboard->Is_Shift_Down() && !pKeyboard->Is_Ctrl_Down())) {
  ...
  if (col->m_array == ARRAY_MASSIVE) {
    ...
    if (col->m_obj->m_type == TYPE_OW_LINE_START ||
        col->m_obj->m_type == TYPE_OW_LINE_END ||
        col->m_obj->m_type == TYPE_OW_WAYPOINT) {
      // ignore
  ...
  if (m_hovering_object->m_obj) {
    ...
    std::string info =
        "X : " +
        int_to_string(
            static_cast<int>(m_hovering_object->m_obj->m_start_pos_x)) +
        "  Y : " +
        int_to_string(
            static_cast<int>(m_hovering_object->m_obj->m_start_pos_y));
    ...
    // create text surface
    cGL_Surface *position_info =
        pFont->Render_Text(pFont->m_font_small, info, white);

    // create request
    cSurface_Request *request = new cSurface_Request();
    position_info->Blit(static_cast<float>(m_x + 20),
                        static_cast<float>(m_y + 35), 0.52f, request);
    ...
    if (game_debug) {
      info =
          "Curr.  X : " +
          int_to_string(static_cast<int>(m_hovering_object->m_obj->m_pos_x)) +
          "  Y : " +
          int_to_string(static_cast<int>(m_hovering_object->m_obj->m_pos_y)) +
          "  Z : " + float_to_string(m_hovering_object->m_obj->m_pos_z, 6);
      ...
      if (!Is_Float_Equal(m_hovering_object->m_obj->m_editor_pos_z, 0.0f)) {
        info.insert(
            info.length(),
            _("  Editor Z : ") +
                float_to_string(m_hovering_object->m_obj->m_editor_pos_z, 6));
      }
      ...
      position_info->Blit(static_cast<float>(m_x + 20),
                          static_cast<float>(m_y + 55), 0.52f, request);
      request->m_delete_texture = 1;

...
cThromp ::cThromp(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cThromp::Init();
}
...
void cThromp ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  Set_Direction(Get_Direction_Id(
      attributes
          ...
          .c_str()));
  // max distance
  Set_Max_Distance(static_cast<float>(attributes.getValueAsInteger(
      "max_distance", static_cast<int>(m_max_distance))));
  // speed
...
void cThromp ::Set_Image_Dir(std::string dir) {
  ...
  if (!File_Exists(DATA_DIR "/" GAME_PIXMAPS_DIR "/" + dir + "up.png") &&
      !File_Exists(DATA_DIR "/" GAME_PIXMAPS_DIR "/" + dir + "up.settings")) {
...
void cThromp ::Update(void) {
  ...
  if (m_state == STA_STAY) {
    ...
    if (pLevel_Player->m_maryo_type != MARYO_GHOST &&
        pLevel_Player->m_col_rect.Intersects(final_distance)) {
      Activate();
    }
  }
  // flying ( moving into the destination direction )
    ...
    /* slow down
     * only if the velocity is not too small for the given distance to the final
     * position final velocity should not get smaller on the last 10% to the
     * final position
     */
...
void cThromp ::Update_Images(void) {
  ...
  Clear_Images();
  // set images
  Add_Image(pVideo->Get_Surface(
      m_img_dir + Get_Direction_Name(m_start_direction) + ".png"));
  Add_Image(pVideo->Get_Surface(
      m_img_dir + Get_Direction_Name(m_start_direction) + "_active.png"));
  // set start image
  Set_Image_Num(0, 1);
  ...
  if (collision->m_direction == m_direction) {
    ...
    if (m_state == STA_FLY) {
      ...
      // kill enemy
        ...
        pHud_Points->Add_Points(enemy->m_kill_points,
                                m_pos_x + m_image->m_w / 3, m_pos_y - 5, "",
...
void cThromp ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_thromp_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cThromp::Editor_Direction_Select, this));

  // image dir
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_thromp_image_dir"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cThromp::Editor_Image_Dir_Text_Changed, this));

  // max distance
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_thromp_max_distance"));
  Editor_Add(UTF8_("Distance"), UTF8_("Detection distance into its direction"),
             editbox, 90);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cThromp::Editor_Max_Distance_Text_Changed,
                               this));

  // speed
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_thromp_speed"));
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cThromp::Editor_Speed_Text_Changed, this));

...
void Finished_Sound(const int channel) {
  ...
  for (AudioSoundList::iterator itr = pAudio->m_active_sounds.begin();
       itr != pAudio->m_active_sounds.end(); ++itr) {
...
cAudio_Sound ::~cAudio_Sound(void) { Free(); }
...
void cAudio_Sound ::Finished(void) { m_channel = -1; }
...
bool cAudio ::Init(void) {
  ...
  if (!m_initialised) {
    if (m_debug) {
      printf("Initializing Audio System - Buffer %i, Frequency %i, Speaker "
             "Channels %i\n",
             m_audio_buffer, pPreferences->m_audio_hz, m_audio_channels);
    }

    /*	Initializing preferred Audio System specs with Mixer Standard format
     *(Stereo)
     *
     *	frequency	: Output sampling frequency in samples per second (Hz).
     *	format		: Output sample format.
     *	channels	: Number of sound channels in output. 2 for stereo and 1
     *for mono. chunk size	: Bytes used per output sample.
     */

    if (Mix_OpenAudio(pPreferences->m_audio_hz, MIX_DEFAULT_FORMAT,
                      m_audio_channels, m_audio_buffer) < 0) {
...
void cAudio ::Close(void) {
  if (m_initialised) {
    ...
    if (m_sound_enabled) {
      ...
      // clear sounds
      for (AudioSoundList::iterator itr = m_active_sounds.begin();
           itr != m_active_sounds.end(); ++itr) {
        delete *itr;
      }
...
bool cAudio ::Play_Sound(std::string filename, int res_id /* = -1 */,
                         int volume /* = -1 */, int loops /* = 0 */) {
  if (!m_initialised || !m_sound_enabled) {
    return 0;
  }
...
bool cAudio ::Play_Music(std::string filename, int loops /* = 0 */,
                         bool force /* = 1 */,
                         unsigned int fadein_ms /* = 0 */) {
...
cAudio_Sound *cAudio ::Get_Playing_Sound(std::string filename) {
  ...
  // get all sounds
  for (AudioSoundList::const_iterator itr = m_active_sounds.begin();
       itr != m_active_sounds.end(); ++itr) {
    // get object pointer
    cAudio_Sound *obj = (*itr);
...
void cAudio ::Fadeout_Sounds(unsigned int ms /* = 200 */,
  ...
  if (channel >= 0 && !overwrite_fading &&
      Is_Sound_Fading(channel) == MIX_FADING_OUT) {
    return;
  }
...
void cAudio ::Fadeout_Sounds(unsigned int ms, std::string filename,
                             bool overwrite_fading /* = 0 */) {
  if (!m_sound_enabled || !m_initialised) {
    return;
  }
  ...
  for (AudioSoundList::const_iterator itr = m_active_sounds.begin();
    ...
    // Do not fade out the sound again
    if (!overwrite_fading &&
        Is_Sound_Fading(obj->m_channel) == MIX_FADING_OUT) {
      continue;
    }
...
void cAudio ::Set_Music_Position(float position) const {
  if (!m_music_enabled || !m_initialised ||
      Is_Music_Fading() == MIX_FADING_OUT) {
    return;
  }

...
cBonusBox ::~cBonusBox(void) {
  for (MovingSpriteList::iterator itr = m_active_items.begin();
       itr != m_active_items.end(); ++itr) {
    delete *itr;
  }
...
void cBonusBox ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  if (box_type == TYPE_GOLDPIECE) {
    Set_Goldcolor(Get_Color_Id(
        attributes.getValueAsString("gold_color", Get_Color_Name(m_gold_color))
            .c_str()));
  }
}
...
void cBonusBox ::Activate(void) {
  ...
  // check if lower item should be used if no force best item
  else if (!m_force_best_item &&
           (box_type == TYPE_FIREPLANT || box_type == TYPE_MUSHROOM_BLUE) &&
           (current_maryo_type == MARYO_SMALL ||
            ((current_maryo_type == MARYO_FIRE ||
              current_maryo_type == MARYO_ICE) &&
             !pHud_Itembox->m_item_id))) {
    pAudio->Play_Sound("sprout_1.ogg");

    cMushroom *mushroom = new cMushroom(m_sprite_manager);
    mushroom->Set_Pos(m_start_pos_x - ((m_item_image->m_w - m_rect.m_w) / 2),
    ...
    box_item->Set_Pos(m_start_pos_x - ((m_item_image->m_w - m_rect.m_w) / 2),
                      m_start_pos_y, 1);
  } else if (box_type == TYPE_MUSHROOM_DEFAULT ||
             box_type == TYPE_MUSHROOM_LIVE_1 ||
             box_type == TYPE_MUSHROOM_POISON ||
             box_type == TYPE_MUSHROOM_BLUE ||
             box_type == TYPE_MUSHROOM_GHOST) {
    pAudio->Play_Sound("sprout_1.ogg");
    ...
    star->Set_Pos(m_start_pos_x - ((m_item_image->m_w - m_rect.m_w) / 2),
                  m_start_pos_y, 1);
    ...
    goldpiece->Set_Pos(m_start_pos_x - ((m_item_image->m_w - m_rect.m_w) / 2),
                       m_pos_y, 1);
...
void cBonusBox ::Update(void) {
  ...
  for (MovingSpriteList::iterator itr = m_active_items.begin();
    ...
    float diff = powerup->m_pos_y -
                 (m_pos_y - powerup->m_col_rect.m_h - powerup->m_col_pos.m_y);
...
void cBonusBox ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_bonusbox_animation"));
  Editor_Add(UTF8_("Animation"),
             UTF8_("Use the Power animation if the box has a good or needed "
                   "item for this level"),
             combobox, 120, 100);
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cBonusBox::Editor_Animation_Select, this));

  // Item
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_bonusbox_item"));
  Editor_Add(UTF8_("Item"), UTF8_("The item that gets spawned"), combobox, 160,
             140);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cBonusBox::Editor_Item_Select, this));

  // Force best item
  combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
      "TaharezLook/Combobox", "editor_bonusbox_force_best_item"));
  Editor_Add(UTF8_("Force item"),
             UTF8_("Force best available item when activated"), combobox, 120,
             80);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cBonusBox::Editor_Force_best_item_Select,
                               this));

  // gold color
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_bonusbox_gold_color"));
  Editor_Add(UTF8_("Gold color"),
             UTF8_("Gold color if the item is a goldpiece"), combobox, 100, 80);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cBonusBox::Editor_Gold_Color_Select, this));
...
void cBonusBox ::Editor_State_Update(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          ...
          .getRootWindow()
          ->getChildRecursive("editor_bonusbox_force_best_item"));

  if (box_type == TYPE_UNDEFINED || box_type == TYPE_POWERUP ||
      box_type == TYPE_MUSHROOM_DEFAULT || box_type == TYPE_MUSHROOM_LIVE_1 ||
      box_type == TYPE_MUSHROOM_POISON || box_type == TYPE_MUSHROOM_GHOST ||
      box_type == TYPE_STAR || box_type == TYPE_GOLDPIECE) {
  ...
  combobox = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("editor_bonusbox_gold_color"));

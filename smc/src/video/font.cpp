...
void Font_Delete_Ref(cGL_Surface *surface) { pFont->Delete_Ref(surface); }
...
void cFont_Manager ::Init(void) {
  ...
  // open fonts
  m_font_normal =
      TTF_OpenFont(DATA_DIR "/" GUI_FONT_DIR "/default_bold.ttf", 18);
  m_font_small =
      TTF_OpenFont(DATA_DIR "/" GUI_FONT_DIR "/default_bold.ttf", 11);
  m_font_very_small =
      TTF_OpenFont(DATA_DIR "/" GUI_FONT_DIR "/default_bold.ttf", 9);
...
void cFont_Manager ::Delete_Ref(cGL_Surface *surface) {
  for (ActiveFontList::iterator itr = m_active_fonts.begin();
       itr != m_active_fonts.end(); ++itr) {
    cGL_Surface *obj = (*itr);
...
cGL_Surface *cFont_Manager ::Render_Text(TTF_Font *font,
                                         const std::string &text,
                                         const Color color) {
  // get SDL Color
  SDL_Color sdlcolor = color.Get_SDL_Color();
  // create text surface
  cGL_Surface *surface = pVideo->Create_Texture(
...
void cFont_Manager ::Restore_Textures(void) {
  ...
  for (Saved_Texture_List::iterator itr = m_software_textures.begin();
       itr != m_software_textures.end(); ++itr) {
    // get saved texture

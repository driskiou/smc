...
void cBall ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // direction
  m_direction =
      static_cast<ObjectDirection>(attributes.getValueAsInteger("direction"));
  // origin array and type
  Set_Origin(
      static_cast<ArrayType>(attributes.getValueAsInteger("origin_array")),
      static_cast<SpriteType>(attributes.getValueAsInteger("origin_type")));
  // type
  Set_Ball_Type(
      static_cast<ball_effect>(attributes.getValueAsInteger("ball_type")));
}
...
void cBall ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (m_ball_type == FIREBALL_DEFAULT) {
    Set_Color_Combine(m_glim_counter, m_glim_counter / 1.6f, m_glim_counter / 3,
                      GL_ADD);
  } else if (m_ball_type == ICEBALL_DEFAULT) {
    Set_Color_Combine(m_glim_counter / 6.0f, m_glim_counter / 6.0f,
                      m_glim_counter / 6.0f, GL_ADD);
  }
...
void cBall ::Generate_Particles(cParticle_Emitter *anim /* = NULL */) const {
  ...
  if (m_ball_type == FIREBALL_DEFAULT) {
    ...
    anim->Set_Color(
        Color(static_cast<Uint8>(150 + (m_glim_counter * 100)), 50, 50),
        Color(static_cast<Uint8>(0), 200, 200, 0));
  }
  // ice
  else {
...
void cBall ::Handle_Collision_Enemy(cObjectCollision *collision) {
  ...
  if ((m_ball_type == FIREBALL_DEFAULT && enemy->m_fire_resistant) ||
      (m_ball_type == ICEBALL_DEFAULT && enemy->m_ice_resistance >= 1)) {
    pAudio->Play_Sound("item/fireball_repelled.wav");
  }
  // destroy enemy
  else {
    ...
    for (unsigned int w = 0; w < enemy->m_col_rect.m_w; w += 15) {
      for (unsigned int h = 0; h < enemy->m_col_rect.m_h; h += 15) {
        ...
        if (m_ball_type == FIREBALL_DEFAULT) {
          ...
          anim_color_rand = Color(static_cast<Uint8>(rand() % 5), rand() % 85,
                                  rand() % 25, 0);
        } else {
          anim_color = Color(static_cast<Uint8>(150), 150, 240);
          anim_color_rand = Color(static_cast<Uint8>(rand() % 80), rand() % 80,
                                  rand() % 10, 0);
        }
    ...
    if (m_ball_type == FIREBALL_DEFAULT) {
      // get points
      pHud_Points->Add_Points(enemy->m_kill_points, m_pos_x, m_pos_y, "",
                              static_cast<Uint8>(255), 1);

      // create goldpiece
      cMovingSprite *goldpiece =
          new cFGoldpiece(m_sprite_manager, collision->m_direction);
      goldpiece->Set_Spawned(1);
      // set optimal position
      goldpiece->Set_Pos(enemy->m_rect.m_x + ((enemy->m_rect.m_w / 2) -
                                              (goldpiece->m_rect.m_w / 2)),
                         enemy->m_rect.m_y + ((enemy->m_rect.m_h / 2) -
                                              (goldpiece->m_rect.m_h / 2)),
                         1);
      // add goldpiece

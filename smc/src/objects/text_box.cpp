...
void cText_Box ::Activate(void) {
  ...
  CEGUI::MultiLineEditbox *editbox = static_cast<CEGUI::MultiLineEditbox *>(
      wmgr.createWindow("TaharezLook/MultiLineEditbox", "text_box_text"));
  ...
  // set position
  float text_pos_x =
      m_pos_x - (text_box_window_width * 0.5f) + (m_rect.m_w * 0.5f);
  ...
  if (text_pos_x > pActive_Camera->m_limit_rect.m_x +
                       pActive_Camera->m_limit_rect.m_w -
                       text_box_window_width) {
    // put it on screen
    text_pos_x = pActive_Camera->m_limit_rect.m_x +
                 pActive_Camera->m_limit_rect.m_w - text_box_window_width;
  }

  editbox->setXPosition(
      CEGUI::UDim(0, (text_pos_x - pActive_Camera->m_x) * global_upscalex));
  editbox->setYPosition(
      CEGUI::UDim(0, (text_pos_y - pActive_Camera->m_y) * global_upscaley));
  // set size
  ...
  while (display) {
    ...
    if (pPreferences->m_joy_enabled) {
      joy_ver_axis = SDL_JoystickGetAxis(pJoystick->m_joystick,
                                         pPreferences->m_joy_axis_ver);
    }

    // down
    if (keys[SDL_GetScancodeFromKey(pPreferences->m_key_down)] ||
        joy_ver_axis > pPreferences->m_joy_axis_threshold) {
      editbox->getVertScrollbar()->setScrollPosition(
          editbox->getVertScrollbar()->getScrollPosition() +
          (editbox->getVertScrollbar()->getStepSize() * 0.25f *
           pFramerate->m_speed_factor));
    }
    // up
    if (keys[SDL_GetScancodeFromKey(pPreferences->m_key_up)] ||
        joy_ver_axis < -pPreferences->m_joy_axis_threshold) {
      editbox->getVertScrollbar()->setScrollPosition(
          editbox->getVertScrollbar()->getScrollPosition() -
          (editbox->getVertScrollbar()->getStepSize() * 0.25f *
...
void cText_Box ::Set_Text(const std::string &str_text) { m_text = str_text; }
...
void cText_Box ::Editor_Activate(void) {
  ...
  CEGUI::MultiLineEditbox *editbox = static_cast<CEGUI::MultiLineEditbox *>(
      wmgr.createWindow("TaharezLook/MultiLineEditbox", "text_box_text"));
  Editor_Add(UTF8_("Text"), UTF8_("Text to display when activated"), editbox,
             static_cast<float>(text_box_window_width),
             static_cast<float>(text_box_window_height));

  editbox->setText(reinterpret_cast<const CEGUI::utf8 *>(m_text.c_str()));
  editbox->subscribeEvent(
      CEGUI::MultiLineEditbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cText_Box::Editor_Text_Text_Changed, this));
...
bool cText_Box ::Editor_Text_Text_Changed(const CEGUI::EventArgs &event) {
  ...
  std::string str_text =
      static_cast<CEGUI::MultiLineEditbox *>(windowEventArgs.window)
          ->getText()
          .c_str();

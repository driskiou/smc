...
cFurball ::cFurball(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cFurball::Init();
}
...
void cFurball ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  if (m_type == TYPE_FURBALL_BOSS) {
    // max downgrade count
    Set_Max_Downgrade_Count(attributes.getValueAsInteger(
        "max_downgrade_count", m_max_downgrade_count));
...
void cFurball ::Set_Color(const DefaultColor &col) {
  ...
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_1.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_2.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_3.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_4.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_5.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_6.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_7.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/furball/" + filename_dir + "/walk_8.png"));
  ...
  if (m_type == TYPE_FURBALL_BOSS) {
    Add_Image(
        pVideo->Get_Surface("enemy/furball/" + filename_dir + "/hit.png"));
  } else {
    Add_Image(NULL);
  }
...
void cFurball ::Update(void) {
  ...
  if (m_state == STA_STAY) {
    ...
    if (static_cast<int>(m_counter_hit) % 2 == 1) {
      ...
      cMovingSprite::Set_Color(
          Color(static_cast<Uint8>(255),
                250 - static_cast<Uint8>(m_counter_hit * 1.5f),
                250 - static_cast<Uint8>(m_counter_hit * 4)));
    }
    // default
    else {
...
void cFurball ::Generate_Smoke(unsigned int amount /* = 1 */,
                               float particle_scale /* = 0.4f */) const {
  // animation
  cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
  anim->Set_Pos(m_pos_x + (m_col_rect.m_w * 0.5f),
                m_pos_y + (m_col_rect.m_h * 0.5f), 1);
  anim->Set_Image(
      pVideo->Get_Surface("animation/particles/smoke_grey_big.png"));
...
void cFurball ::Handle_Collision_Player(cObjectCollision *collision) {
  ...
  if (collision->m_direction == DIR_TOP && pLevel_Player->m_state != STA_FLY) {
    ...
    if (m_dead) {
      pHud_Points->Add_Points(m_kill_points, m_pos_x, m_pos_y - 5.0f, "",
...
void cFurball ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_furball_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cFurball::Editor_Direction_Select, this));

  if (m_type == TYPE_FURBALL_BOSS) {
    // max downgrades
    CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
        "TaharezLook/Editbox", "editor_furball_max_downgrade_count"));
    ...
    editbox->subscribeEvent(
        ...
        CEGUI::Event::Subscriber(
            &cFurball::Editor_Max_Downgrade_Count_Text_Changed, this));

    // level ends if killed
    combobox = static_cast<CEGUI::Combobox *>(wmgr.createWindow(
        "TaharezLook/Combobox", "editor_furball_level_ends_if_killed"));
    ...
    combobox->subscribeEvent(
        CEGUI::Combobox::EventListSelectionAccepted,
        CEGUI::Event::Subscriber(&cFurball::Editor_Level_Ends_If_Killed, this));
  }
...
bool cFurball ::Editor_Max_Downgrade_Count_Text_Changed(

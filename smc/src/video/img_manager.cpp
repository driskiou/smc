...
namespace SMC
{
  ...
  cImage_Manager ::cImage_Manager(void) : cObject_Manager<cGL_Surface>()
  {
    m_high_texture_id = 0;
  }

  cImage_Manager ::~cImage_Manager(void) { cImage_Manager::Delete_All(); }
  ...
  cGL_Surface *cImage_Manager ::Get_Pointer(const std::string &path) const
  {
    ...
    for (GL_Surface_List::const_iterator itr = objects.begin();
         itr != objects.end(); ++itr)
    {
      cGL_Surface *obj = (*itr);
  ...
  {
    for (GL_Surface_List::iterator itr = objects.begin(); itr != objects.end();
         ++itr)
    {
      // get object
      cGL_Surface *obj = (*itr);
  ...
  void cImage_Manager ::Grab_Textures(bool from_file /* = 0 */,
                                      bool draw_gui /* = 0 */)
  {
    // progress bar
  ...
  void cImage_Manager ::Restore_Textures(bool draw_gui /* = 0 */)
  {
    ...
    for (Saved_Texture_List::iterator itr = m_saved_textures.begin();
         itr != m_saved_textures.end(); ++itr)
    {
      // get saved texture

...
cjStar ::cjStar(cSprite_Manager *sprite_manager) : cPowerUp(sprite_manager) {
  cjStar::Init();
}
...
void cjStar ::Activate(void) {
  ...
  // animation
  Generate_Particles(m_pos_x + m_col_rect.m_w * 0.5f,
                     m_pos_y + m_col_rect.m_h * 0.5f, 1, 20);
...
void cjStar ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  Set_Color_Combine(m_glim_counter / 0.8f, m_glim_counter / 0.9f,
                    m_glim_counter, GL_ADD);

  cPowerUp::Draw();
}

void cjStar ::Generate_Particles(float x /* = 0.0f */, float y /* = 0.0f */,
                                 bool random /* = 1 */,
                                 unsigned int quota /* = 2 */) const {
  ...
  if (collision->m_direction == DIR_RIGHT ||
    ...
    if (m_ground_object) {
      ...
      const float clamped_x = Clamp(
          m_rect.m_x + (m_rect.m_w / 2), m_ground_object->m_col_rect.m_x,
          m_ground_object->m_col_rect.m_x + m_ground_object->m_col_rect.m_w);

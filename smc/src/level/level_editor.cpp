...
cEditor_Level ::~cEditor_Level(void) { delete m_settings_screen; }

const CEGUI::String &cEditor_Level ::getDefaultResourceGroup() const {
...
bool cEditor_Level ::Key_Down(SDL_Keycode key) {
  ...
  // focus last levelexit
    ...
    if (!Is_Float_Equal(new_camera_posx, 0.0f) ||
        !Is_Float_Equal(new_camera_posy, 0.0f)) {
      pActive_Camera->Set_Pos(new_camera_posx - (game_res_w * 0.5f),
                              new_camera_posy - (game_res_h * 0.5f));
    }
  }
...
cSprite *cEditor_Level ::Get_Object(const CEGUI::String &element,
                                    CEGUI::XMLAttributes &attributes,
                                    int engine_version) {
  return Create_Level_Object_From_XML(element, attributes, engine_version,
                                      m_sprite_manager);
}
...
void cEditor_Level ::Function_Load(void) {
  ...
  while (level_name.length()) {
    level_name = Box_Text_Input(level_name, _("Load a Level"),
                                level_name.compare(_("Name")) == 0 ? 1 : 0);

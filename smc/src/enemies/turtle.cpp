...
cTurtle ::cTurtle(cSprite_Manager *sprite_manager) : cEnemy(sprite_manager) {
  cTurtle::Init();
}
...
void cTurtle ::Load_From_Savegame(cSave_Level_Object *save_object) {
  ...
  if (save_object->exists("turtle_state")) {
    Turtle_state mov_state = static_cast<Turtle_state>(
        string_to_int(save_object->Get_Value("turtle_state")));
...
cSave_Level_Object *cTurtle ::Save_To_Savegame(void) {
  ...
  if (m_turtle_state != TURTLE_WALK) {
    save_object->m_properties.push_back(cSave_Level_Object_Property(
        "turtle_state", int_to_string(m_turtle_state)));
  }

  return save_object;
}

void cTurtle ::Set_Direction(const ObjectDirection dir,
...
void cTurtle ::Set_Color(DefaultColor col) {
  ...
  // Walk
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_1.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_2.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_3.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_4.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_5.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_6.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_7.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_8.png"));
  Add_Image(
      pVideo->Get_Surface("enemy/turtle/" + filename_dir + "/walk_9.png"));
  // Walk Turn
  ...
  Add_Image(pVideo->Get_Surface("enemy/turtle/" + filename_dir +
                                "/shell_look_1.png"));
  Add_Image(pVideo->Get_Surface("enemy/turtle/" + filename_dir +
                                "/shell_look_2.png"));
  Add_Image(pVideo->Get_Surface("enemy/turtle/" + filename_dir +
...
void cTurtle ::DownGrade(bool force /* = 0 */) {
  if (!force) {
    ...
    if (m_turtle_state == TURTLE_WALK) {
      ...
      Col_Move(0, m_images[0].m_image->m_col_h - m_images[5].m_image->m_col_h,
               1, 1);
      m_counter = 0.0f;
    }
    // staying
...
void cTurtle ::Update(void) {
  ...
  if (m_turtle_state == TURTLE_SHELL_STAND ||
      m_turtle_state == TURTLE_SHELL_RUN) {
    // update rotation
    if (!Is_Float_Equal(m_velx, 0.0f)) {
      Add_Rotation_Z((m_velx / (m_image->m_w * 0.009f)) *
...
void cTurtle ::Handle_Collision_Player(cObjectCollision *collision) {
  if (collision->m_direction == DIR_UNDEFINED ||
      (m_turtle_state == TURTLE_SHELL_RUN && m_player_counter > 0.0f)) {
    return;
  }
...
void cTurtle ::Handle_Collision_Enemy(cObjectCollision *collision) {
  if (m_turtle_state == TURTLE_SHELL_STAND) {
    cEnemy *enemy = static_cast<cEnemy *>(
    ...
    if (Hit_Enemy(enemy)) {
      // create animation
      cParticle_Emitter *anim = new cParticle_Emitter(m_sprite_manager);
      anim->Set_Emitter_Rect(m_col_rect.m_x + (m_col_rect.m_w * 0.2f),
                             m_col_rect.m_y + (m_col_rect.m_h * 0.2f),
                             m_col_rect.m_w * 0.6f, m_col_rect.m_h * 0.8f);
      anim->Set_Image(pVideo->Get_Surface("animation/particles/light.png"));
      anim->Set_Quota(5);
  ...
  if (m_turtle_state == TURTLE_WALK) {
    ...
    if (collision->m_direction == DIR_RIGHT ||
      ...
      if (collision->m_direction == DIR_RIGHT) {
        ...
        anim->Set_Pos(col_object->m_pos_x + col_object->m_col_pos.m_x + 4.0f,
        ...
        anim->Set_Pos(col_object->m_pos_x + col_object->m_col_pos.m_x +
                          col_object->m_col_rect.m_w - 4.0f,
                      m_pos_y + (m_col_rect.m_h / 1.35f), 1);
...
void cTurtle ::Editor_Activate(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "editor_turtle_direction"));
  ...
  combobox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cTurtle::Editor_Direction_Select, this));

...
void cMenuHandler ::Add_Menu_Item(
    cMenu_Item *item, float shadow_pos /* = 0 */,
    Color shadow_color /* = static_cast<Uint8>(0) */) {
  if (!item) {
...
void cMenuHandler ::Set_Active(int num) {
  ...
  if (num == static_cast<int>(m_active) ||
      num >= static_cast<int>(m_items.size()) || (num >= 0 && !m_items[num])) {
    return;
  }
...
void cMenuHandler ::Update_Mouse(void) {
  ...
  for (unsigned int i = 0; i < m_items.size(); i++) {
    if (m_items[i]->m_col_rect.Intersects(
            static_cast<float>(pMouseCursor->m_x),
            static_cast<float>(pMouseCursor->m_y))) {
      found = i;
      break;
    }
  }
...
cMenuCore ::cMenuCore(void) {
  ...
  // left side
  cParticle_Emitter *anim =
      new cParticle_Emitter(m_handler->m_level->m_sprite_manager);
  anim->Set_Image_Filename("clouds/default_1/1_middle.png");
  anim->Set_Emitter_Rect(-100, static_cast<float>(-game_res_h), 0,
                         game_res_h * 0.5f);
  ...
  anim->Set_Color(Color(static_cast<Uint8>(255), 255, 255, 200),
                  Color(static_cast<Uint8>(0), 0, 0, 55));
  ...
  anim->Set_Image_Filename("clouds/default_1/1_middle.png");
  anim->Set_Emitter_Rect(static_cast<float>(game_res_w) + 100,
                         static_cast<float>(-game_res_h), 0,
                         static_cast<float>(game_res_h) * 0.5f);
  anim->Set_Emitter_Time_to_Live(-1);
  anim->Set_Emitter_Iteration_Interval(16);
...
bool cMenuCore ::Key_Down(SDL_Keycode key) {
  ...
  if (key == SDLK_DOWN || key == pPreferences->m_key_down) {
    if (m_handler->Get_Size() <=
        static_cast<unsigned int>(m_handler->m_active + 1)) {
  ...
  // Fast Debug Level entering
    ...
    if (!CEGUI::System::getSingleton()
      ...
      menu_start->Init();
      // get levels listbox
      CEGUI::Listbox *listbox_levels = static_cast<CEGUI::Listbox *>(
          CEGUI::System::getSingleton()
              .getDefaultGUIContext()
              .getRootWindow()
              ->getChildRecursive("listbox_levels"));
      // select random level
      listbox_levels->setItemSelectState(
          rand() % listbox_levels->getItemCount(), 1);
      // get level name
...
bool cMenuCore ::Mouse_Down(Uint8 button) {
  ...
  if (button == SDL_BUTTON_LEFT) {
    ...
    if (item &&
        item->m_col_rect.Intersects(static_cast<float>(pMouseCursor->m_x),
                                    static_cast<float>(pMouseCursor->m_y))) {
      m_menu_data->m_action = 1;
      return 1;
    }
...
cMenu_Item *cMenuCore ::Auto_Menu(std::string imagename,
                                  std::string imagefilename_menu,
                                  float ypos /* = 0 */,
                                  bool is_quit /* = 0 */) {
  ...
  if (imagefilename_menu.length() > 0) {
    temp_item->m_image_menu->Set_Image(
        pVideo->Get_Surface(DATA_DIR "/" GAME_PIXMAPS_DIR "/menu/items/" +
                            imagefilename_menu),
        1);
  }
  ...
  if (imagename.length() > 0) {
    temp_item->m_image_default->Set_Image(
        pVideo->Get_Surface(DATA_DIR "/" GAME_PIXMAPS_DIR "/menu/" + imagename),
        1);
  }

  // position and initialization
  temp_item->Set_Pos((game_res_w * 0.5f) -
                         (temp_item->m_image_default->m_col_rect.m_w * 0.5f),
                     ypos);
...
void cMenuCore ::Load(const MenuID menu /* = MENU_MAIN */,
                      const GameMode exit_gamemode /* = MODE_NOTHING */) {
  Unload();
  // reset menu handler
  m_handler->Reset();
...
void cMenuCore ::Enter(const GameMode old_mode /* = MODE_NOTHING */) {
  ...
  if (old_mode == MODE_NOTHING || old_mode == MODE_LEVEL ||
      old_mode == MODE_OVERWORLD) {
    // set camera start position
    m_handler->m_camera->Reset_Pos();

    // pre-update animations
    for (cSprite_List::iterator itr =
             m_handler->m_level->m_sprite_manager->objects.begin();
         itr != m_handler->m_level->m_sprite_manager->objects.end(); ++itr) {
      cSprite *obj = (*itr);
    ...
    for (cAnimation_Manager::cAnimation_List::iterator itr =
             m_animation_manager->objects.begin();
         itr != m_animation_manager->objects.end(); ++itr) {
      cAnimation *obj = (*itr);
  ...
  if (!pAudio->Is_Music_Playing() ||
      pAudio->Is_Music_Fading() == MIX_FADING_OUT) {
    if (m_menu_id == MENU_CREDITS) {
      pAudio->Play_Music("land/hyper_1.ogg", -1, 0, 1500);
    } else if (m_menu_data && m_menu_data->m_exit_to_gamemode == MODE_LEVEL &&
               pActive_Level->Is_Loaded()) {
      pAudio->Play_Music(pActive_Level->m_musicfile, -1, 0, 1500);
    } else if (m_menu_data &&
               m_menu_data->m_exit_to_gamemode == MODE_OVERWORLD &&
               pActive_Overworld->Is_Loaded()) {

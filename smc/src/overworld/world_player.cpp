...
cOverworld_Player ::cOverworld_Player(cSprite_Manager *sprite_manager,
...
void cOverworld_Player ::Set_Direction(const ObjectDirection dir,
...
void cOverworld_Player ::Draw_Debug_Text(void) {
  ...
  if (static_cast<int>(m_overworld->m_layer->size()) != m_debug_lines_last) {
    m_debug_lines->Set_Image(
        pFont->Render_Text(
            pFont->m_font_small,
            "Lines : " + int_to_string(m_overworld->m_layer->size()), blue),
        1, 1);
  }
  // Update Current Line Texts
  if (m_current_line != m_debug_current_line_last) {
    m_debug_current_line->Set_Image(
        pFont->Render_Text(pFont->m_font_small,
                           "Curr Line : " + int_to_string(m_current_line),
                           green),
        1, 1);
  }
  // Update Current Waypoint Texts
  if (m_current_waypoint != m_debug_current_waypoint_last) {
    m_debug_current_waypoint->Set_Image(
        pFont->Render_Text(
            pFont->m_font_small,
            "Curr Waypoint : " + int_to_string(m_current_waypoint), green),
        1, 1);
  }
...
void cOverworld_Player ::Activate_Waypoint(void) {
  ...
  if (waypoint->m_waypoint_type == WAYPOINT_NORMAL) {
    ...
    Game_Action_Data_Start.add(
        "screen_fadeout",
        CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_FIXED_COLORBOX));
  ...
  // world link waypoint
    ...
    if (str_world.compare("credits") == 0) {
      ...
      Game_Action_Data_Start.add(
          "screen_fadeout",
          CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_HORIZONTAL_VERTICAL));
    ...
    // world link
      ...
      if (pOverworld_Manager->Get(str_world)) {
        ...
        Game_Action_Data_Start.add(
            "screen_fadeout",
            CEGUI::PropertyHelper<int>::toString(EFFECT_OUT_BLACK_TILED_RECTS));
        Game_Action_Data_Middle.add("enter_world", str_world.c_str());
        Game_Action_Data_Middle.add(
            "world_player_waypoint",
            overworld_origin->m_description->m_path.c_str());
        Game_Action_Data_End.add(
            "screen_fadein",
...
bool cOverworld_Player ::Start_Walk(ObjectDirection new_direction) {
  ...
  // invalid direction
  if (!(new_direction == DIR_UP || new_direction == DIR_DOWN ||
  ...
  if (pOverworld_Manager->m_debug_mode) {
    ...
    if (m_current_waypoint > 0) {
      printf("Waypoint Direction Forward : %d Backward : %d\n",
             Get_Waypoint()->m_direction_forward,
             Get_Waypoint()->m_direction_backward);
    }
  }
  ...
  if (m_current_waypoint >= 0 && m_direction == DIR_UNDEFINED) {
    ...
    if (pOverworld_Manager->m_debug_mode) {
      printf("Waypoint Front line id %d, origin id %d\n",
             m_overworld->m_layer->Get_Array_Num(front_line),
             front_line->m_origin);
    }
  ...
  // if already walking
  if (m_direction != DIR_UNDEFINED) {
    if (m_direction == Get_Opposite_Direction(new_direction) &&
        !m_fixed_walking &&
        m_overworld->Get_Waypoint_Collision(m_col_rect) < 0) {
  ...
  if (m_current_waypoint >= 0 &&
      !m_col_rect.Intersects(Get_Waypoint()->m_rect)) {
...
void cOverworld_Player ::Update_Walk(void) {
  ...
  if (((m_direction == DIR_UP || m_direction == DIR_DOWN) &&
       m_line_hor.m_line_number >= 0 &&
       (m_line_hor.m_difference > 20 || m_line_hor.m_difference < -20)) ||
      ((m_direction == DIR_LEFT || m_direction == DIR_RIGHT) &&
       m_line_ver.m_line_number >= 0 &&
       (m_line_ver.m_difference > 20 || m_line_ver.m_difference < -20)) ||
...
bool cOverworld_Player ::Set_Waypoint(int waypoint,
                                      bool new_startpos /* = 0 */) {
  if (waypoint < 0 ||
      waypoint >= static_cast<int>(m_overworld->m_waypoints.size())) {
    return 0;
  }

  cWaypoint *wp = m_overworld->m_waypoints[waypoint];

  Set_Pos(wp->m_rect.m_x - m_col_pos.m_x +
              ((wp->m_rect.m_w - m_col_rect.m_w) * 0.5f),
          wp->m_rect.m_y - m_col_pos.m_y +
              ((wp->m_rect.m_h - m_col_rect.m_h) * 0.5f),
          new_startpos);
...
cOverworld_Player ::Get_Front_Line(ObjectDirection dir) const {
  return m_overworld->m_layer
      ->Get_Line_Collision_Direction(m_col_rect.m_x + (m_col_rect.m_w * 0.5f),
                                     m_col_rect.m_y + (m_col_rect.m_h * 0.5f),
                                     dir)
      .m_line;
}
...
void cOverworld_Player ::Update_Path_Diff(unsigned int check_size /* = 25 */) {
  ...
  // get nearest lines
  m_line_hor = m_overworld->m_layer->Get_Nearest(
      x, y, DIR_HORIZONTAL, check_size + hor_advance, m_line_waypoint);
  m_line_ver = m_overworld->m_layer->Get_Nearest(
      x, y, DIR_VERTICAL, check_size + ver_advance, m_line_waypoint);
}
...
void cOverworld_Player ::Change_Direction(void) {
  ...
  /* check with a big collision line the difference to the next layer line
   * behind maryo if no line is found turn around if a line is found walk into
   * it's direction
   */
  if (m_direction == DIR_RIGHT || m_direction == DIR_LEFT) {
    // search for a line with increasing distance
    for (unsigned int i = 0; i < 30; i++) {
      m_line_ver = m_overworld->m_layer->Get_Nearest(x - i, y, DIR_VERTICAL, 80,
                                                     m_line_waypoint);
    ...
    // search for a line with increasing distance
    for (unsigned int i = 0; i < 30; i++) {
      m_line_hor = m_overworld->m_layer->Get_Nearest(x, y + i, DIR_HORIZONTAL,
                                                     80, m_line_waypoint);
...
void cOverworld_Player ::Auto_Pos_Correction(float size /* = 1.7f */,

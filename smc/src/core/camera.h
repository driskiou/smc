...
enum Camera_movement {
  ...
  CAMERA_MOVE_BLINK = 2, // Fade out at the current position and fade back in at
...
public:
   ...
   */
  bool Move_to_Position_Gradually(const float pos_x, const float pos_y,
  ...
  // center on the player with the given direction ( DIR_HORIZONTAL,
  // DIR_VERTICAL and DIR_ALL )
  ...
  // reset limits
  inline void Reset_Limits(void) { m_limit_rect = m_default_limits; };
  // set limits
  void Set_Limits(const GL_rect &rect);
  inline void Set_Limit_X(const float val) { m_limit_rect.m_x = val; };
  inline void Set_Limit_Y(const float val) { m_limit_rect.m_y = val; };
  inline void Set_Limit_W(const float val) { m_limit_rect.m_w = val; };
  inline void Set_Limit_H(const float val) { m_limit_rect.m_h = val; };

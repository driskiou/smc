...
GL_Vector ::GL_Vector(float pos_x /* = 0 */, float pos_y /* = 0 */,
                      float pos_z /* = 0 */, float pos_w /* = 1 */) {
  x = pos_x;
  y = pos_y;
  z = pos_z;
  w = pos_w;
...
GL_Vector GL_Vector ::cross(const GL_Vector &v) const {
  GL_Vector tmp =
      GL_Vector(y * v.z - z * v.y, z * v.x - x * v.z, x * v.y - y * v.x);
  return tmp;
}
...
GL_Vector GL_Vector ::rotate(float angle, const GL_Vector &a) const {
  ...
  tmp.x = x * (a.x * a.x * (1 - c) + c) + y * (a.x * a.y * (1 - c) - a.z * s) +
          z * (a.x * a.z * (1 - c) + a.y * s);
  tmp.y = x * (a.y * a.x * (1 - c) + a.z * s) + y * (a.y * a.y * (1 - c) + c) +
          z * (a.y * a.z * (1 - c) - a.x * s);
  tmp.z = x * (a.x * a.z * (1 - c) - a.y * s) +
          y * (a.y * a.z * (1 - c) + a.x * s) + z * (a.z * a.z * (1 - c) + c);

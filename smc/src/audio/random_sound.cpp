...
void cRandom_Sound ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  // position
  Set_Pos(static_cast<float>(attributes.getValueAsInteger("pos_x")),
          static_cast<float>(attributes.getValueAsInteger("pos_y")), 1);
  //
  ...
  // volume reduction
  Set_Volume_Reduction_Begin(attributes.getValueAsFloat(
      "volume_reduction_begin", m_volume_reduction_begin));
  Set_Volume_Reduction_End(attributes.getValueAsFloat("volume_reduction_end",
...
std::string cRandom_Sound ::Get_Filename(void) const { return m_filename; }
...
unsigned int cRandom_Sound ::Get_Delay_Min(void) const { return m_delay_min; }
...
unsigned int cRandom_Sound ::Get_Delay_Max(void) const { return m_delay_max; }
...
float cRandom_Sound ::Get_Volume_Min(void) const { return m_volume_min; }
...
float cRandom_Sound ::Get_Volume_Max(void) const { return m_volume_max; }
...
float cRandom_Sound ::Get_Distance_Volume_Mod(void) const {
  ...
  if (m_distance_to_camera > m_volume_reduction_begin) {
    ...
    return 1.0f -
           distance / (m_volume_reduction_end - m_volume_reduction_begin);
  }
...
void cRandom_Sound ::Update(void) {
  ...
  if (m_continuous) {
    ...
    // update volume
      ...
      // set volume
      pAudio->Set_Sound_Volume(static_cast<Uint8>(sound_volume),
                               sound->m_channel);
    ...
    if (m_next_play_delay <= 0.0f) {
      ...
      if (m_delay_max > m_delay_min) {
        m_next_play_delay += Get_Random_Float(
            0.0f, static_cast<float>(m_delay_max - m_delay_min));
      }
    }
  }
...
void cRandom_Sound ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  pVideo->Draw_Rect(m_col_rect.m_x - pActive_Camera->m_x,
                    ...
                    m_col_rect.m_h, m_editor_pos_z, &greenyellow);
  // volume reduction begin
  cCircle_Request *circle_request = new cCircle_Request();
  pVideo->Draw_Circle(m_col_rect.m_x - pActive_Camera->m_x,
                      m_col_rect.m_y - pActive_Camera->m_y,
                      m_volume_reduction_begin, m_editor_pos_z - 0.0001f,
                      &m_editor_color_volume_reduction_begin, circle_request);
  ...
  pVideo->Draw_Circle(m_col_rect.m_x - pActive_Camera->m_x,
                      m_col_rect.m_y - pActive_Camera->m_y,
                      m_volume_reduction_end, m_editor_pos_z - 0.0002f,
                      &m_editor_color_volume_reduction_end, circle_request);
  circle_request->m_line_width = 3;
  // add request
  pRenderer->Add(circle_request);
}
...
void cRandom_Sound ::Editor_Activate(void) {
  ...
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_sound_filename"));
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Filename_Text_Changed,
                               this));

  // continuous
  CEGUI::ToggleButton *checkbox = static_cast<CEGUI::ToggleButton *>(
      wmgr.createWindow("editor_sound_continuous"));
  Editor_Add(UTF8_("Continuous"),
             UTF8_("Check if the sound should be played continuously instead "
                   "of randomly"),
             checkbox, 50);
  ...
  checkbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Continuous_Changed,
                               this));

  // delay min
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_sound_delay_min"));
  Editor_Add(UTF8_("Delay Minimum"), UTF8_("Minimal delay until played again"),
             editbox, 90);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Delay_Min_Text_Changed,
                               this));

  // delay max
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_sound_delay_max"));
  Editor_Add(UTF8_("Maximum"), UTF8_("Maximal delay until played again"),
             editbox, 90, 28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Delay_Max_Text_Changed,
                               this));

  // volume min
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_sound_volume_min"));
  Editor_Add(UTF8_("Volume Minimum"),
             UTF8_("Minimal random volume for each play"), editbox, 90);

  editbox->setValidationString("^[+]?\\d*$");
  editbox->setText(int_to_string(static_cast<int>(m_volume_min)));
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Volume_Min_Text_Changed,
                               this));

  // volume max
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "editor_sound_volume_max"));
  Editor_Add(UTF8_("Maximum"), UTF8_("Maximal random volume for each play"),
             editbox, 90, 28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cRandom_Sound::Editor_Volume_Max_Text_Changed,
                               this));

  // volume reduction begin
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_sound_volume_reduction_begin"));
  Editor_Add(UTF8_("Volume Reduction Begin"),
             UTF8_("Volume reduction begins gradually at this distance"),
             editbox, 90);
  ...
  editbox->subscribeEvent(
      ...
      CEGUI::Event::Subscriber(
          &cRandom_Sound::Editor_Volume_Reduction_Begin_Text_Changed, this));

  // volume reduction end
  editbox = static_cast<CEGUI::Editbox *>(wmgr.createWindow(
      "TaharezLook/Editbox", "editor_sound_volume_reduction_end"));
  Editor_Add(UTF8_("End"),
             UTF8_("Volume reduction ends at this distance. Sound is not "
                   "played beyond this."),
             editbox, 90, 28, 0);
  ...
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(
          &cRandom_Sound::Editor_Volume_Reduction_End_Text_Changed, this));

  // init
  Editor_Init();
}

bool cRandom_Sound ::Editor_Filename_Text_Changed(
...
bool cRandom_Sound ::Editor_Delay_Min_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cRandom_Sound ::Editor_Delay_Max_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cRandom_Sound ::Editor_Volume_Min_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cRandom_Sound ::Editor_Volume_Max_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cRandom_Sound ::Editor_Volume_Reduction_Begin_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =
...
bool cRandom_Sound ::Editor_Volume_Reduction_End_Text_Changed(
    const CEGUI::EventArgs &event) {
  const CEGUI::WindowEventArgs &windowEventArgs =
      static_cast<const CEGUI::WindowEventArgs &>(event);
  std::string str_text =

...
void cAnimated_Sprite ::Set_Image_Num(const int num,
                                      const bool new_startimage /* = 0 */,
                                      const bool del_img /* = 0 */) {
  if (m_curr_img == num) {
    return;
  }
  ...
  if (m_curr_img < 0) {
    ...
    cMovingSprite::Set_Image(m_images[m_curr_img].m_image, new_startimage,
                             del_img);
  } else {
    debug_print("Warning : Object image number %d bigger as the array size %u, "
                "sprite type %d, name %s\n",
                m_curr_img, static_cast<unsigned int>(m_images.size()), m_type,
                m_name.c_str());
  }
}
...
void cAnimated_Sprite ::Update_Animation(void) {
  ...
  if (static_cast<Uint32>(m_anim_counter * m_anim_mod) >= image.m_time) {
    ...
    m_anim_counter =
        static_cast<Uint32>(m_anim_counter * m_anim_mod) - image.m_time;
  }
}

void cAnimated_Sprite ::Set_Time_All(const Uint32 time,
                                     const bool default_time /* = 0 */) {
  for (cAnimation_Surface_List::iterator itr = m_images.begin();

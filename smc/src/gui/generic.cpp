...
cDialogBox_Text ::cDialogBox_Text(void) : cDialogBox() {
...
void cDialogBox_Text ::Init(std::string title_text) {
  ...
  // get window
  CEGUI::FrameWindow *box_window = static_cast<CEGUI::FrameWindow *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_text_window"));
  box_window->setText(
      reinterpret_cast<const CEGUI::utf8 *>(title_text.c_str()));
  box_window->setSizingEnabled(0);
  box_window->getCloseButton()->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cDialogBox_Text::Button_window_quit_clicked,
                               this));

  // get editbox
  box_editbox = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_text_editbox"));
}
...
std::string cDialogBox_Text ::Enter(std::string default_text,
  ...
  box_editbox->setText(
      reinterpret_cast<const CEGUI::utf8 *>(default_text.c_str()));
  box_editbox->setTooltipText(
      reinterpret_cast<const CEGUI::utf8 *>(title_text.c_str()));
  ...
  while (!finished) {
    while (SDL_PollEvent(&input_event)) {
      if (input_event.type == SDL_KEYDOWN) {
        if (auto_no_text &&
            default_text.compare(box_editbox->getText().c_str()) == 0) {
          box_editbox->setText("");
          // only the first time
          auto_no_text = 0;
        }
        ...
        if (input_event.key.keysym.sym == SDLK_ESCAPE) {
          box_editbox->setText("");
          finished = 1;
        } else if (input_event.key.keysym.sym == SDLK_RETURN ||
                   input_event.key.keysym.sym == SDLK_KP_ENTER) {
          finished = 1;
        } else {
...
bool cDialogBox_Text ::Button_window_quit_clicked(
...
cDialogBox_Question ::cDialogBox_Question(void) : cDialogBox() {
...
void cDialogBox_Question ::Init(bool with_cancel) {
  ...
  // get window
  box_window = static_cast<CEGUI::FrameWindow *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_question_window"));
  box_window->activate();
  ...
  if (with_cancel) {
    box_window->getCloseButton()->subscribeEvent(
        CEGUI::PushButton::EventClicked,
        CEGUI::Event::Subscriber(&cDialogBox_Question::Button_cancel_clicked,
                                 this));
  } else {
    box_window->getCloseButton()->subscribeEvent(
        CEGUI::PushButton::EventClicked,
        CEGUI::Event::Subscriber(&cDialogBox_Question::Button_no_clicked,
                                 this));
  }
}
...
int cDialogBox_Question ::Enter(std::string text, bool with_cancel /* = 0 */) {
  ...
  // get text
  CEGUI::Editbox *box_text = static_cast<CEGUI::Editbox *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_question_text"));
  ...
  if (text_width > 250) {
    ...
    box_window->setXPosition(CEGUI::UDim(
        0, (game_res_w * 0.5f - text_width * 0.5f) * global_upscalex));
  }

  // Button Yes
  CEGUI::PushButton *button_yes = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_question_button_yes"));
  // Button No
  CEGUI::PushButton *button_no = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_question_button_no"));
  // Button Cancel
  CEGUI::PushButton *button_cancel = static_cast<CEGUI::PushButton *>(
      CEGUI::System::getSingleton()
          .getDefaultGUIContext()
          .getRootWindow()
          ->getChildRecursive("box_question_button_cancel"));
  ...
  // events
  button_yes->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cDialogBox_Question::Button_yes_clicked, this));
  button_no->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cDialogBox_Question::Button_no_clicked, this));
...
bool cDialogBox_Question ::Button_cancel_clicked(
    const CEGUI::EventArgs &event) {
  return_value = -1;
  finished = 1;
  return 1;
...
void Draw_Static_Text(const std::string &text,
                      const Color *color_text /* = &white */,
                      const Color *color_bg /* = NULL */,
                      bool wait_for_input /* = 1 */) {
  // fixme : Can't handle multiple lines of text. Change to MultiLineEditbox or
  // use HorzFormatting=WordWrapLeftAligned property.
  bool draw = 1;

  // Statictext window
  CEGUI::Window *window_statictext =
      CEGUI::WindowManager::getSingleton().loadLayoutFromFile(
          "statictext.layout");
  CEGUI::System::getSingleton()
      .getDefaultGUIContext()
      .getRootWindow()
      ->addChild(window_statictext);
  // get default text
  CEGUI::Window *text_default =
      static_cast<CEGUI::Window *>(CEGUI::System::getSingleton()
                                       .getDefaultGUIContext()
                                       .getRootWindow()
                                       ->getChildRecursive("text_default"));

  // set text
  text_default->setProperty(
      "TextColours",
      CEGUI::PropertyHelper<CEGUI::Colour>::toString(
          CEGUI::Colour(static_cast<float>(color_text->red) / 255,
                        static_cast<float>(color_text->green) / 255,
                        static_cast<float>(color_text->blue) / 255, 1)));
  ...
  text_default->setXPosition(CEGUI::UDim(
      0, (game_res_w * 0.5f - text_width * 0.5f) * global_upscalex));
  ...
  while (draw) {
    ...
    if (wait_for_input) {
      while (SDL_PollEvent(&input_event)) {
        if (input_event.type == SDL_KEYDOWN ||
            input_event.type == SDL_JOYBUTTONDOWN ||
...
std::string Box_Text_Input(const std::string &default_text,
...
std::string Get_Clipboard_Content(void) {
  ...
  Size bytecount = 0;
  OSStatus status =
      ::GetScrapFlavorSize(scrap, kScrapFlavorTypeText, &bytecount);
  if (status != noErr) {
    return false;
  }

  char *buffer = new char[bytecount];
  if (::GetScrapFlavorData(scrap, kScrapFlavorTypeText, &bytecount, buffer) ==
      noErr) {
  ...
  delete[] buffer;
#elif __unix__
  // only works with the cut-buffer method (xterm) and not with the more recent
  // selections method
...
void Set_Clipboard_Content(std::string str) {
  ...
  if (OpenClipboard(NULL)) {
    ...
    unsigned int length =
        (str.length() + 1) * sizeof(std::string::allocator_type);
    HANDLE h =
        GlobalAlloc((GMEM_MOVEABLE | GMEM_DDESHARE | GMEM_ZEROINIT), length);
...
bool GUI_Copy_To_Clipboard(bool cut) {
  ...
  if (type.find("/MultiLineEditbox") != CEGUI::String::npos) {
    CEGUI::MultiLineEditbox *editbox =
        static_cast<CEGUI::MultiLineEditbox *>(window_active);
...
bool GUI_Paste_From_Clipboard(void) {
  ...
  if (type.find("/MultiLineEditbox") != CEGUI::String::npos) {
    ...
    // get clipboard text
    CEGUI::String clipboard_text =
        reinterpret_cast<const CEGUI::utf8 *>(Get_Clipboard_Content().c_str());
    // set new text

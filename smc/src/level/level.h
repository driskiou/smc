...
public:
  ...
  // Set the level difficulty ( 0 = undefined, 1 = dead easy and 100 = ultimate
  // challenge )
  ...
  static bool Is_Level_Object_Element(const CEGUI::String &element) {
    if (element == "information" || element == "settings" ||
        element == "background" || element == "music" ||
        element == "global_effect" || element == "player" ||
        element == "sound" || element == "particle_emitter" ||
        element == "path" || element == "sprite" || element == "powerup" ||
        element == "item" || element == "enemy" || element == "levelexit" ||
        element == "level_entry" || element == "enemystopper" ||
        element == "box" || element == "moving_platform" ||
        element == "falling_platform" || element == "ball") {
      return 1;
    }
 ...
 * engine_version : engine version of the data and if it's below the current
 * version it converts it sprite_manager : needed if the engine version is below
 * the current version and data conversion creates multiple objects
 */

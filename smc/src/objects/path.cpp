...
void cPath_State ::Load_From_Savegame(cSave_Level_Object *save_object) {
  ...
  if (save_object->exists("current_segment")) {
    m_current_segment =
        string_to_int(save_object->Get_Value("current_segment"));
  }
  if (save_object->exists("current_segment_pos")) {
    m_current_segment_pos =
        string_to_float(save_object->Get_Value("current_segment_pos"));
  }
...
void cPath_State ::Save_To_Savegame(cSave_Level_Object *save_object) {
  ...
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("new_pos_x", float_to_string(m_pos_x)));
  save_object->m_properties.push_back(
      cSave_Level_Object_Property("new_pos_y", float_to_string(m_pos_y)));

  // current segment
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "current_segment", int_to_string(m_current_segment)));

  // current segment position
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "current_segment_pos", float_to_string(m_current_segment_pos)));

  // forward
  save_object->m_properties.push_back(cSave_Level_Object_Property(
      "forward", int_to_string(static_cast<int>(m_forward))));
}
...
void cPath_State ::Draw(void) {
  ...
  pVideo->Draw_Rect(m_path->m_col_rect.m_x + m_pos_x - 4 - pActive_Camera->m_x,
                    m_path->m_col_rect.m_y + m_pos_y - 4 - pActive_Camera->m_y,
                    8, 8, m_path->m_editor_pos_z + 0.00002f, &orange);
}
...
void cPath_State ::Path_Destroyed_Event(void) { m_path = NULL; }
...
void cPath_State ::Move_Reverse(void) { m_forward = !m_forward; }
...
void cPath ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  while (1) {
    ...
    obj.Set_Pos(attributes.getValueAsFloat("segment_" + str_pos + "_x1"),
                attributes.getValueAsFloat("segment_" + str_pos + "_y1"),
                attributes.getValueAsFloat("segment_" + str_pos + "_x2"),
                attributes.getValueAsFloat("segment_" + str_pos + "_y2"));
...
void cPath ::Set_Identifier(const std::string &identifier) {
  ...
  for (cSprite_List::iterator itr =
           pActive_Level->m_sprite_manager->objects.begin();
       itr != pActive_Level->m_sprite_manager->objects.end(); ++itr) {
    cSprite *obj = (*itr);
    ...
    if (obj->m_type == TYPE_STATIC_ENEMY) {
      ...
      // found
      if (static_enemy->m_path_state.m_path_identifier.compare(m_identifier) ==
          0) {
        // link to me
      ...
      // found
      if (moving_platform->m_path_state.m_path_identifier.compare(
              m_identifier) == 0) {
        // link to me
...
void cPath ::Set_Show_Line(bool show) { m_show_line = show; }
...
void cPath ::Set_Rewind(bool rewind) {
  ...
  for (PathStateList::iterator itr = m_linked_path_states.begin();
       itr != m_linked_path_states.end(); ++itr) {
    cPath_State *obj = (*itr);
...
void cPath ::Remove_Link(cPath_State *path_state) {
  ...
  // get iterator
  PathStateList::iterator itr = std::find(
      m_linked_path_states.begin(), m_linked_path_states.end(), path_state);
...
void cPath ::Draw(cSurface_Request *request /* = NULL */) {
  ...
  if (!m_show_line || editor_enabled || game_debug)
    pVideo->Draw_Rect(m_col_rect.m_x - pActive_Camera->m_x,
                      m_col_rect.m_y - pActive_Camera->m_y, m_col_rect.m_w,
                      m_col_rect.m_h, m_editor_pos_z, &m_editor_color);

  // draw segments
  int count = 0;
  for (PathList::iterator itr = m_segments.begin(); itr != m_segments.end();
       ++itr) {
    ...
    pVideo->Draw_Line(m_col_rect.m_x + obj.m_x1 - pActive_Camera->m_x,
                      m_col_rect.m_y + obj.m_y1 - pActive_Camera->m_y,
                      m_col_rect.m_x + obj.m_x2 - pActive_Camera->m_x,
                      m_col_rect.m_y + obj.m_y2 - pActive_Camera->m_y,
                      m_editor_pos_z + 0.00001f, &line_color, line_request);
...
bool cPath ::Is_Draw_Valid(void) {
  ...
  if (!m_active ||
      (!Is_Visible_On_Screen() && pMouseCursor->m_active_object != this)) {
    return 0;
  }
...
void cPath ::Editor_Activate(void) {
  ...
  CEGUI::Editbox *editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "path_identifier"));
  Editor_Add(UTF8_("Identifier"),
             UTF8_("Identifier name of the Path. This is needed for objects "
                   "that can link to this."),
             editbox, 150);

  editbox->setText(m_identifier.c_str());
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cPath::Editor_Identifier_Text_Changed, this));

  // show path moving line
  CEGUI::Combobox *combobox_line = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "path_show_line"));
  Editor_Add(UTF8_("Show Line"),
             UTF8_("Wether or not to show the path's moving line ingame."),
             combobox_line, 100, 105);
  ...
  combobox_line->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cPath::Editor_Show_Line_Select, this));

  // move type
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "path_move_type"));
  Editor_Add(UTF8_("Move Type"),
             UTF8_("Movement type. Mirror moves forth and back and rewind "
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cPath::Editor_Move_Type_Select, this));

  // selected segment
  combobox = static_cast<CEGUI::Combobox *>(
      wmgr.createWindow("TaharezLook/Combobox", "path_selected_segment"));
  Editor_Add(UTF8_("Segment"), UTF8_("Select the Segment to edit."), combobox,
             100, 105);
  ...
  combobox->subscribeEvent(
      CEGUI::Combobox::EventListSelectionAccepted,
      CEGUI::Event::Subscriber(&cPath::Editor_Selected_Segment_Select, this));

  // button add
  CEGUI::PushButton *pushbutton = static_cast<CEGUI::PushButton *>(
      wmgr.createWindow("TaharezLook/Button", "path_segment_add"));
  Editor_Add("-", UTF8_("Add a Segment after the selected one."), pushbutton,
             60, 28, 0);

  pushbutton->setText(UTF8_("Add"));
  pushbutton->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cPath::Editor_Button_Add_Segment_Clicked,
                               this));

  // button delete
  pushbutton = static_cast<CEGUI::PushButton *>(
      wmgr.createWindow("TaharezLook/Button", "path_segment_delete"));
  Editor_Add("-", UTF8_("Delete the selected Segment."), pushbutton, 70, 28, 0);

  pushbutton->setText(UTF8_("Delete"));
  pushbutton->subscribeEvent(
      CEGUI::PushButton::EventClicked,
      CEGUI::Event::Subscriber(&cPath::Editor_Button_Delete_Segment_Clicked,
                               this));

  // selected segment x1
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "path_segment_x1"));
  Editor_Add(UTF8_("Pos X1"), UTF8_("Line position X1"), editbox, 150);

  editbox->setValidationString("[-+]?[0-9]*\\.?[0-9]*");
  editbox->setText(int_to_string(
      static_cast<int>(m_segments[m_editor_selected_segment].m_x1)));
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cPath::Editor_Pos_X1_Text_Changed, this));

  // selected segment y1
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "path_segment_y1"));
  Editor_Add(UTF8_("Y1"), UTF8_("Line position Y1"), editbox, 150, 28, 0);

  editbox->setValidationString("[-+]?[0-9]*\\.?[0-9]*");
  editbox->setText(int_to_string(
      static_cast<int>(m_segments[m_editor_selected_segment].m_y1)));
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cPath::Editor_Pos_Y1_Text_Changed, this));

  // selected segment x2
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "path_segment_x2"));
  Editor_Add(UTF8_("Pos X2"), UTF8_("Line position X2"), editbox, 150);

  editbox->setValidationString("[-+]?[0-9]*\\.?[0-9]*");
  editbox->setText(int_to_string(
      static_cast<int>(m_segments[m_editor_selected_segment].m_x2)));
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cPath::Editor_Pos_X2_Text_Changed, this));

  // selected segment y2
  editbox = static_cast<CEGUI::Editbox *>(
      wmgr.createWindow("TaharezLook/Editbox", "path_segment_y2"));
  Editor_Add(UTF8_("Y2"), UTF8_("Line position Y2"), editbox, 150, 28, 0);

  editbox->setValidationString("[-+]?[0-9]*\\.?[0-9]*");
  editbox->setText(int_to_string(
      static_cast<int>(m_segments[m_editor_selected_segment].m_y2)));
  editbox->subscribeEvent(
      CEGUI::Editbox::EventTextChanged,
      CEGUI::Event::Subscriber(&cPath::Editor_Pos_Y2_Text_Changed, this));
...
void cPath ::Editor_State_Update(void) {
  ...
  CEGUI::Combobox *combobox = static_cast<CEGUI::Combobox *>(
      CEGUI::System::getSingleton()
          ...
          .getRootWindow()
          ->getChildRecursive("path_selected_segment"));
  combobox->resetList();
  ...
  // x1
  CEGUI::Editbox *editbox_x1 =
      static_cast<CEGUI::Editbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("path_segment_x1"));
  editbox_x1->setText(
      float_to_string(m_segments[m_editor_selected_segment].m_x1, 6, 0));
  // y1
  CEGUI::Editbox *editbox_y1 =
      static_cast<CEGUI::Editbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("path_segment_y1"));
  editbox_y1->setText(
      float_to_string(m_segments[m_editor_selected_segment].m_y1, 6, 0));
  // x2
  CEGUI::Editbox *editbox_x2 =
      static_cast<CEGUI::Editbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("path_segment_x2"));
  editbox_x2->setText(
      float_to_string(m_segments[m_editor_selected_segment].m_x2, 6, 0));
  // y2
  CEGUI::Editbox *editbox_y2 =
      static_cast<CEGUI::Editbox *>(CEGUI::System::getSingleton()
                                        .getDefaultGUIContext()
                                        .getRootWindow()
                                        ->getChildRecursive("path_segment_y2"));
  editbox_y2->setText(
      float_to_string(m_segments[m_editor_selected_segment].m_y2, 6, 0));
...
bool cPath ::Editor_Button_Add_Segment_Clicked(const CEGUI::EventArgs &event) {
  ...
  new_segment.Set_Pos(new_segment.m_x2, new_segment.m_y2, new_segment.m_x2 + 20,
                      new_segment.m_y2 - 20);
  m_segments.insert(m_segments.begin() + m_editor_selected_segment + 1,
                    new_segment);
...
bool cPath ::Editor_Button_Delete_Segment_Clicked(
...
bool cPath ::Editor_Pos_X1_Text_Changed(const CEGUI::EventArgs &event) {
  ...
  m_segments[m_editor_selected_segment].Set_Pos_Start_X(
...
bool cPath ::Editor_Pos_Y1_Text_Changed(const CEGUI::EventArgs &event) {
  ...
  m_segments[m_editor_selected_segment].Set_Pos_Start_Y(
      string_to_float(str_text));
...
bool cPath ::Editor_Pos_X2_Text_Changed(const CEGUI::EventArgs &event) {
  ...
  m_segments[m_editor_selected_segment].Set_Pos_End_X(
      string_to_float(str_text));
...
bool cPath ::Editor_Pos_Y2_Text_Changed(const CEGUI::EventArgs &event) {
  ...
  m_segments[m_editor_selected_segment].Set_Pos_End_Y(
      string_to_float(str_text));

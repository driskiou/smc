...
cGL_Surface ::~cGL_Surface(void) {
  // don't delete a managed OpenGL image if still in use by another managed
  // cGL_Surface
  if (m_auto_del_img && glIsTexture(m_image) &&
      (!m_managed || !Is_Texture_Use_Multiple())) {
...
void cGL_Surface ::Blit(float x, float y, float z,
                        cSurface_Request *request /* = NULL */) const {
...
void cGL_Surface ::Set_Ground_Type(GroundType gtype) { m_ground_type = gtype; }

bool cGL_Surface ::Is_Texture_Use_Multiple(void) const {
  for (GL_Surface_List::iterator itr = pImage_Manager->objects.begin();
       itr != pImage_Manager->objects.end(); ++itr) {
    cGL_Surface *obj = (*itr);
...
cSaved_Texture *
cGL_Surface ::Get_Software_Texture(bool only_filename /* = 0 */) {
  ...
  if (!only_filename) {
    ...
    // texture data
    soft_tex->m_pixels =
        new GLubyte[soft_tex->m_width * soft_tex->m_height * bpp];

    glGetTexImage(GL_TEXTURE_2D, 0, soft_tex->m_format, GL_UNSIGNED_BYTE,
                  soft_tex->m_pixels);
  }
...
void cGL_Surface ::Load_Software_Texture(cSaved_Texture *soft_tex) {
  ...
  if (soft_tex->m_pixels) {
    ...
    pVideo->Create_GL_Texture(soft_tex->m_width, soft_tex->m_height,
                              soft_tex->m_pixels, mipmaps);
  ...
  // load from file
    ...
    if (!surface_copy) {
      printf(
          "Warning: cGL_Surface :: Load_Software_Texture %s loading failed\n",
          m_filename.c_str());
      return;
    }
...
std::string cGL_Surface ::Get_Filename(int with_dir /* = 2 */,
  ...
  // erase pixmaps directory
           ...
           name.find(DATA_DIR "/" GAME_PIXMAPS_DIR "/") != std::string::npos) {

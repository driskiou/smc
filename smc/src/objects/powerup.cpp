...
cSave_Level_Object *cPowerUp ::Save_To_Savegame(void) {
  ...
  if (!Is_Float_Equal(m_start_pos_x, m_pos_x) ||
      !Is_Float_Equal(m_start_pos_y, m_pos_y)) {
    save_object->m_properties.push_back(cSave_Level_Object_Property(
...
void cPowerUp ::Handle_out_of_Level(ObjectDirection dir) {
  if (dir == DIR_LEFT) {
    ...
    m_pos_x = pActive_Camera->m_limit_rect.m_x +
              pActive_Camera->m_limit_rect.m_w - m_col_pos.m_x -
...
void cMushroom ::Load_From_XML(CEGUI::XMLAttributes &attributes) {
  ...
  Set_Pos(static_cast<float>(attributes.getValueAsInteger("posx")),
          static_cast<float>(attributes.getValueAsInteger("posy")));
  // type
  Set_Type(static_cast<SpriteType>(
      attributes.getValueAsInteger("mushroom_type", TYPE_MUSHROOM_DEFAULT)));
}
...
void cMushroom ::Update(void) {
  ...
  if (m_type == TYPE_MUSHROOM_LIVE_1 || m_type == TYPE_MUSHROOM_BLUE) {
    ...
    if (m_counter > 1.0f) {
      ...
      anim->Set_Emitter_Rect(m_col_rect.m_x, m_col_rect.m_y, m_col_rect.m_w,
                             m_col_rect.m_h);
      ...
      if (m_type == TYPE_MUSHROOM_LIVE_1) {
        ...
        anim->Set_Color(Color(static_cast<Uint8>(0), 100, 0),
                        Color(static_cast<Uint8>(30), 150, 30, 0));
      ...
      // Ice
        ...
        anim->Set_Color(Color(static_cast<Uint8>(180), 180, 255, 128),
                        Color(static_cast<Uint8>(50), 50, 0, 0));
    ...
    if (m_ground_object) {
      ...
      if (m_counter > 1.0f) {
        ...
        anim->Set_Emitter_Rect(m_col_rect.m_x + (m_col_rect.m_w * 0.3f),
                               m_col_rect.m_y + (m_col_rect.m_h * 0.91f),
                               m_col_rect.m_w * 0.4f, 0);
        anim->Set_Quota(static_cast<int>(m_counter));
        anim->Set_Pos_Z(m_pos_z - 0.000001f);
        anim->Set_Time_to_Live(1.4f, 0.4f);
        anim->Set_Scale(0.7f, 0.2f);
        anim->Set_Color(Color(static_cast<Uint8>(120), 190, 0),
                        Color(static_cast<Uint8>(40), 60, 10, 0));
        anim->Set_Blending(BLEND_ADD);
...
void cFirePlant ::Update(void) {
  ...
  if (m_particle_counter > 1.0f) {
    ...
    anim->Set_Scale(0.4f, 0.1f);
    anim->Set_Time_to_Live(0.4f);
    anim->Set_Color(Color(static_cast<Uint8>(255), 50, 0),
                    Color(static_cast<Uint8>(0), 50, 0, 0));
    anim->Set_Speed(0.2f, 0.1f);
    anim->Set_Blending(BLEND_ADD);
    anim->Emit();
    pActive_Animation_Manager->Add(anim);
...
cMoon ::cMoon(cSprite_Manager *sprite_manager) : cPowerUp(sprite_manager) {
  cMoon::Init();
}
